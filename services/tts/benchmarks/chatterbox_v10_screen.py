"""Decision-neutral bounded screen for the corrected Chatterbox CUDA profile."""
...
AUTHORITY_COMMIT_SHA: Final = "4270132cf72811ea9fecae6aaa60de7855ecb54a"
...
type CorrectiveProfile = ChatterboxV9Profile
type CorrectiveConfiguration = ChatterboxV9Configuration


class ChatterboxV10ScreenError(RuntimeError):
    """Fixed content-free screen failure."""

    def __init__(self, code: str) -> None:
        super().__init__(f"tts-chatterbox-v10-screen:{code}")
        self.code = code


def _fail(code: str) -> NoReturn:
    raise ChatterboxV10ScreenError(code)
...
def _load_candidate(
    ...
    if candidate_id != CANDIDATE_ID:
        _fail("candidate")
    profile = load_chatterbox_v10_profile(REPOSITORY_ROOT)
...
def run_corrective_preflight(
    *,
    ...
    load_frozen_v10_authority(REPOSITORY_ROOT)
    ...
    try:
        ...
        profile, configuration, languages, role = (
            load_chatterbox_v10_profile(REPOSITORY_ROOT),
    ...
    if host.total_ram_bytes < 24_576 * MIB:
...
def _raw_payload(
    *,
    ...
    return {
        "schemaVersion": "tts-bilingual-raw-v10",
