"""Disposable bilingual quality review for Chatterbox RTX 50 v11."""
...
RAW_ROOT: Final = REPOSITORY_ROOT / "benchmarks" / "tts" / "raw" / "v11"
...
class ChatterboxV11QualityError(RuntimeError):
    """Fixed content-free quality-workflow failure."""

    def __init__(self, code: str) -> None:
        super().__init__(f"tts-chatterbox-v11-quality:{code}")
        self.code = code


def _fail(code: str) -> NoReturn:
    raise ChatterboxV11QualityError(code)
...
def _write_wave(path: Path, waveform: Sized, sample_rate_hz: int) -> None:
    ...
    except ChatterboxV11QualityError:
        raise
    except Exception:
        _fail("invalid-audio")
...
def _render_html(
    *,
    ...
    scorecard = {
        "schemaVersion": "tts-bilingual-quality-scorecard-v11",
...
document.querySelector("#export").addEventListener("click",()=>{{
 ...
 link.download=`v11-quality-result-${{card.sessionId}}.json`;link.click();
...
def _candidate_configuration(
    ...
    if candidate_id == CANDIDATE_ID:
        chatterbox_profile = load_chatterbox_v11_profile(REPOSITORY_ROOT)
...
def generate_quality_session(
    *,
    ...
    if (
        raw.get("schemaVersion") != "tts-bilingual-raw-v11"
    ...
    _write_json(
        ...
        {
            "schemaVersion": "tts-bilingual-quality-private-v11",
