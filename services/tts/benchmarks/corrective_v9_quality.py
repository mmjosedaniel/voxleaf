"""Disposable bilingual quality review for corrective v9 candidates."""
...
class CorrectiveV9QualityError(RuntimeError):
    """Fixed content-free quality-workflow failure."""

    def __init__(self, code: str) -> None:
        super().__init__(f"tts-corrective-v9-quality:{code}")
        self.code = code


def _fail(code: str) -> NoReturn:
    raise CorrectiveV9QualityError(code)
...
def _write_wave(path: Path, waveform: Sized, sample_rate_hz: int) -> None:
    ...
    except CorrectiveV9QualityError:
        raise
    except Exception:
        _fail("invalid-audio")
...
def _render_html(
    *,
    ...
    scorecard = {
        "schemaVersion": "tts-bilingual-quality-scorecard-v9",
...
document.querySelector("#export").addEventListener("click",()=>{{
 ...
 link.download=`v9-quality-result-${{card.sessionId}}.json`;link.click();
...
def _candidate_configuration(
    ...
    ChatterboxV9Adapter | MossV9Adapter,
    ChatterboxV9Configuration | MossV9Configuration,
    Path,
]:
    if candidate_id == CHATTERBOX_CANDIDATE_ID:
        chatterbox_profile = load_chatterbox_v9_profile(REPOSITORY_ROOT)
    ...
    if candidate_id == MOSS_CANDIDATE_ID:
        ...
        root = REPOSITORY_ROOT / MOSS_ARTIFACT_ROOT
        moss_configuration = MossV9Configuration(root, root / "ephemeral-output")
        return (
            MossV9Adapter(moss_profile, moss_configuration),
            moss_configuration,
            REPOSITORY_ROOT / MOSS_ENVIRONMENT / ".venv" / "Scripts/python.exe",
        )
    _fail("candidate")
...
def generate_quality_session(
    *,
    ...
    if (
        raw.get("schemaVersion") != "tts-bilingual-raw-v9"
    ...
    _write_json(
        ...
        {
            "schemaVersion": "tts-bilingual-quality-private-v9",
