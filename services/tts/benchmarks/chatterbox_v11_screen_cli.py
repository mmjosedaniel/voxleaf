"""Closed-stdin commands for the Chatterbox RTX 50 compatibility v11 screen."""
...
def _read() -> dict[str, object]:
    ...
    if len(raw.encode("utf-8")) > MAXIMUM_STDIN_BYTES:
        raise ChatterboxV11ScreenError("input-limit")
    try:
        value = cast(object, json.loads(raw))
    except json.JSONDecodeError:
        raise ChatterboxV11ScreenError("input") from None
    if not isinstance(value, dict) or set(value) != REQUEST_FIELDS:
        raise ChatterboxV11ScreenError("input")
...
def main() -> int:
    ...
    except (ChatterboxV11ScreenError, OSError, RuntimeError):
        output = {
            "status": "invalid",
