"""Closed-stdin commands for the corrective Chatterbox CUDA v10 screen."""
...
from benchmarks.chatterbox_v10_screen import (
    ChatterboxV10ScreenError,
    ScreenConditions,
...
def _read() -> dict[str, object]:
    ...
    if len(raw.encode("utf-8")) > MAXIMUM_STDIN_BYTES:
        raise ChatterboxV10ScreenError("input-limit")
    try:
        value = cast(object, json.loads(raw))
    except json.JSONDecodeError:
        raise ChatterboxV10ScreenError("input") from None
    if not isinstance(value, dict) or set(value) != REQUEST_FIELDS:
        raise ChatterboxV10ScreenError("input")
    payload = cast(dict[str, object], value)
    if (
        payload.get("candidateId") != CANDIDATE_ID
...
def main() -> int:
    ...
    except (ChatterboxV10ScreenError, OSError, RuntimeError):
        output = {
            "status": "invalid",
