...
const LOCAL_ORIGIN =
  process.env.VOXLEAF_PLAYBACK_V3_LOCAL_ORIGIN ?? "http://127.0.0.1:4175";
const AUTHORITY_COMMIT_SHA =
  process.env.VOXLEAF_PLAYBACK_V3_AUTHORITY_COMMIT_SHA;
const EXECUTION_COMMIT_SHA =
  process.env.VOXLEAF_PLAYBACK_V3_EXECUTION_COMMIT_SHA;
const MEBIBYTE = 1_048_576;
const EXPECTED_TRIALS =
  READER_SETTINGS_PLAYBACK_AUTHORITY_V3.evaluation.syntheticInput
    .toneFrequenciesHz.length *
  READER_SETTINGS_PLAYBACK_AUTHORITY_V3.evaluation.deterministicRatePercent
    .length;
const requestedCandidateId = process.env
  .VOXLEAF_PLAYBACK_V3_COMPARISON_CANDIDATE as
  PitchPreservingProbeCandidateIdV3 | undefined;
const selectedCandidates =
  requestedCandidateId === undefined
    ? READER_SETTINGS_PLAYBACK_AUTHORITY_V3.candidates
    : READER_SETTINGS_PLAYBACK_AUTHORITY_V3.candidates.filter(
        ({ id }) => id === requestedCandidateId,
      );

if (
  selectedCandidates.length === 0 ||
  AUTHORITY_COMMIT_SHA === undefined ||
  EXECUTION_COMMIT_SHA === undefined ||
  !/^[0-9a-f]{40}$/u.test(AUTHORITY_COMMIT_SHA) ||
  !/^[0-9a-f]{40}$/u.test(EXECUTION_COMMIT_SHA) ||
  AUTHORITY_COMMIT_SHA === EXECUTION_COMMIT_SHA
) {
  throw new Error("pitch-probe-v3-invalid-execution-authority");
}
...
function executeText(file: string, args: readonly string[]): Promise<string> {
  return new Promise((resolve, reject) => {
    execFile(file, args, { encoding: "utf8" }, (error, stdout) => {
      if (error !== null) {
        reject(new Error("pitch-probe-v3-process-query-failed"));
        return;
      }
      resolve(stdout);
    });
...
async function browserWorkingSetBytes(browser: Browser): Promise<number> {
  ...
  if (process.platform === "linux") {
    ...
    if (!Number.isSafeInteger(bytes) || bytes <= 0) {
      throw new Error("pitch-probe-v3-working-set-unavailable");
    }
    return bytes;
  }
...
async function measureCandidate(
  ...
  let candidate: PitchProbeCandidateResultV3;
  try {
    candidate = await page.evaluate(async (id) => {
      const run = window.__voxleafRunPitchPreservingBackendCandidateProbeV3;
      if (run === undefined) {
        throw new Error("pitch-preserving-candidate-probe-v3-missing");
      }
      return run(id);
    }, candidateId);
  } finally {
...
test("loads both frozen v3 capabilities under the exact prospective CSP", async ({
  context,
  page,
}) => {
  ...
  try {
    ...
    const capabilities = await page.evaluate(async () => {
      const inspect =
        window.__voxleafInspectPitchPreservingBackendCapabilitiesV3;
      if (inspect === undefined) {
        throw new Error("pitch-preserving-capability-probe-v3-missing");
      }
      return inspect();
    });
...
test("measures the selected frozen v3 candidates sequentially", async ({
  browser,
  context,
  page,
}, testInfo) => {
  ...
  const unexpectedRequests = await applyFrozenCspAndIsolation(context);
  const results: PitchProbeCandidateResultV3[] = [];
  try {
    await page.goto("/");
    ...
    console.log(
      "Boundary-deferred v3 Chromium evidence:",
      JSON.stringify(
        result.candidates.map((candidate) => ({
          candidateId: candidate.candidateId,
          firstActivationP95Ms: candidate.firstActivationP95Ms,
          recurringUnitHandoffP95Ms: candidate.recurringUnitHandoffP95Ms,
    ...
    await testInfo.attach("pitch-preserving-v3-chromium-result.json", {
