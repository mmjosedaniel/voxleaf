...
const MATRIX_TIMEOUT_MS = 30 * 60 * 1_000;

function fail() {
  throw new Error("pitch-preserving-v3-inference-contention-failed");
}
...
async function run() {
  ...
  try {
    ...
    console.log(
      "Boundary-deferred v3 inference-contention matrix completed with one local Piper CPU process and one sequential playback candidate.",
    );
  } finally {
...
try {
  ...
  console.error(
    "Boundary-deferred v3 inference-contention matrix failed.",
  );
  process.exitCode = 1;
}
