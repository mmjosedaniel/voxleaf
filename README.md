...
> **Status:** pre-alpha. Roadmap Milestones 1 through 10.1, M008.1, and M009.1 are complete. Active M010.2 implements the reader-first Settings shell, English-default bounded preferences, and repository-owned pitch-preserving playback at `1.00x` through `0.75x`. Its local six-arm packaged portfolio and repository checks pass; human confirmation of the intermediate playback rates and required pull-request checks still gate archival. Piper/davefx Spanish and Piper/joe English remain supported CPU profiles; Chatterbox Multilingual V3 remains a supported Spanish/English GPU profile; and Qwen3-TTS Serena/Spanish plus Aiden/English remain gated development-only profiles. Production runtime/model distribution, license fulfillment, installers, signing, updates, and complete-MVP release validation remain M011 work.
