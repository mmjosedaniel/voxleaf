...
> **Status:** pre-alpha. Roadmap Milestones 1 through 10.2, M008.1, and M009.1 are complete. M010.2 implements the reader-first Settings shell, English-default bounded preferences, and repository-owned pitch-preserving playback at `1.00x` through `0.75x`. Its six-arm packaged portfolio, repository checks, renewed human all-rate confirmation, and pull request #170 Ubuntu/Windows checks pass. Piper/davefx Spanish and Piper/joe English are supported Windows x64 CPU profiles; Chatterbox Multilingual V3 is a supported Spanish/English GPU profile on systems that pass its published CUDA/VRAM/RAM/CPU gate; and Qwen3-TTS Serena/Spanish plus Aiden/English remain gated development-only profiles. M011 Milestone 7 records GO for the Piper local/portfolio core and conditional GO for Chatterbox based on those requirements. These decisions use representative compatible-host evidence rather than an impossible test of every PC. Version `0.1.0` has an exact unsigned local Windows x64/Piper portfolio candidate. The ordinary Chatterbox manifest still withholds Download in this build, and trusted public Windows publication remains pending external signing authorization.
