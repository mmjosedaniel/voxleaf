...
> **Status:** pre-alpha. Roadmap Milestones 1 through 7 are complete. Milestone 6 implemented a candidate-neutral local TTS feasibility harness, and ADR-0013 records that no standard profile passed. Completed Milestone 6.2 records `selection-v5`, which rejects CPU-only and dual-worker scheduling and retains the exact Qwen3-TTS 1.7B CustomVoice/Serena GPU identity only for a constrained development demo. ADR-0015 approves bounded one-GPU adaptive preparation, not a production or real-time claim. The desktop can open a supported local EPUB, render and navigate its safe reflowable content, apply bounded display preferences, and restore a validated logical reading position after exact-file reselection. The EPUB package also exposes deterministic, bounded, locator-linked narration preparation. Completed M007 implements and validates protocol v1, native supervision, a typed desktop client, a one-unit in-memory handoff, the exact development-only Qwen/Serena adapter, and measured exact-host service behavior. The desktop still does not call narration preparation, and audible playback, synchronization, a standard production profile, general hardware support, and installers remain unimplemented. M008 is the active implementation plan for the first product narration caller and bounded playback scheduler.
