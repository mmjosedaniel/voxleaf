...
Roadmap Milestones 1 through 7 are complete. The visual reader, bounded local restoration, locator-linked narration preparation, candidate-neutral TTS feasibility harness, and constrained local TTS service boundary are implemented and validated within their documented boundaries. The first frozen TTS evaluation selected no viable profile. Milestone 6.1's exact Qwen3-TTS 1.7B CustomVoice/Serena `v3` machine evaluation failed startup, throughput, zero-failure, and mid-generation cancellation gates. Accepted `selection-v3` retains that standard blocker. Completed Milestone 6.2 records `selection-v5`, which rejects the CPU-only and dual-worker alternatives and retains exactly one GPU worker only under a constrained development-demo exception. ADR-0015 therefore authorizes the constrained demo without selecting a production profile. Completed M007 implements and validates protocol v1, the bounded Python service, persistent native supervision, typed one-unit ownership, and the exact development-only Qwen/Serena adapter. M008 is in progress through Milestone 5: an application-level coordinator now prepares narration from the active visual locator, dispatches one exact request at a time, transfers accepted audio into the bounded Web Audio player, and mounts accessible quick/prepared controls only when the exact native development configuration is available. The packaged exact-host matrix passed with 39.238-second quick command-to-audible time, 15.280 seconds of start lead, one observed underrun, 20.91 buffering seconds per playback minute, 160 ms cancellation, a 66.480-second prepared lead, about 4.8 GiB peak dedicated GPU memory, and zero external requests. These results prove the constrained demo and its honest buffering behavior; they do not establish a passing standard profile, uninterrupted playback, synchronization, production distribution, or general hardware support.
...
- [`plans/active/M008-bounded-adaptive-prebuffering.md`](plans/active/M008-bounded-adaptive-prebuffering.md): active implementation plan for quick-start and explicit prepared playback, one-GPU scheduling, adaptive boundary waits, truthful rebuffering, and a 30-minute in-memory ceiling. Milestones 1-5 provide the frozen authority, deterministic scheduler, payload-owning bounded FIFO, Web Audio player, preparation presenter, accessible controls, exact-development application coordinator, and measured packaged quick/prepared flow; policy/repository closeout remains.
