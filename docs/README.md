...
Roadmap Milestones 1 through 6.2 are complete. The visual reader, bounded local restoration, locator-linked narration preparation, and candidate-neutral TTS feasibility harness are implemented and validated within their documented boundaries. The first frozen TTS evaluation selected no viable profile. Milestone 6.1's exact Qwen3-TTS 1.7B CustomVoice/Serena `v3` machine evaluation failed startup, throughput, zero-failure, and mid-generation cancellation gates. Accepted `selection-v3` retains that standard blocker. Completed Milestone 6.2 records `selection-v5`, which rejects the CPU-only and dual-worker alternatives and retains exactly one GPU worker only under a constrained development-demo exception. The same-authority GPU baseline measured RTF 1.467; a low-application-load diagnostic completed both workers but improved aggregate RTF by only about 2.6% while slowing the GPU worker substantially. ADR-0015 therefore authorizes the next constrained-demo work without selecting a production profile. M007 Milestones 1-4 are complete: protocol v1 and its canonical cross-language controls are frozen, the Python service proves the bounded lifecycle, and the native shell owns one persistent child, framed standard streams, process-tree termination, a narrow binary command surface, a typed desktop client, and one-unit memory ownership. Packaged WebView2 validates the model-free lifecycle. Native-only development configuration now activates the exact service-owned Qwen/Serena adapter; its frozen runtime/artifact validation, bounded complete waveform, termination, stale suppression, and clean reload pass on the exact host. The product narration caller, measured service-handoff matrix, playback, and distribution remain unimplemented. M008 owns later bounded playback and adaptive preparation. This is not a passing standard profile, production, native-model-streaming, continuous-playback, or general-hardware claim.
...
- [`plans/active/M007-local-tts-service-and-process-protocol.md`](plans/active/M007-local-tts-service-and-process-protocol.md): active ExecPlan for the constrained one-GPU local service, versioned process protocol, native supervision, complete-unit audio delivery, cancellation containment, and exact-host validation. Milestones 1-4 have complete transport, contracts, Python service, native supervisor, typed client, packaged model-free evidence, and focused exact Qwen/Serena host evidence. It does not authorize production or general-hardware claims.
