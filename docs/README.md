...
Roadmap Milestones 1 through 6.2 are complete. The visual reader, bounded local restoration, locator-linked narration preparation, and candidate-neutral TTS feasibility harness are implemented and validated within their documented boundaries. The first frozen TTS evaluation selected no viable profile. Milestone 6.1's exact Qwen3-TTS 1.7B CustomVoice/Serena `v3` machine evaluation failed startup, throughput, zero-failure, and mid-generation cancellation gates. Accepted `selection-v3` retains that standard blocker. Completed Milestone 6.2 records `selection-v5`, which rejects the CPU-only and dual-worker alternatives and retains exactly one GPU worker only under a constrained development-demo exception. The same-authority GPU baseline measured RTF 1.467; a low-application-load diagnostic completed both workers but improved aggregate RTF by only about 2.6% while slowing the GPU worker substantially. ADR-0015 therefore authorizes the constrained demo without selecting a production profile. M007 Milestones 1-6 implement and locally validate protocol v1, canonical cross-language controls, the bounded Python service, persistent native supervision, narrow binary commands, typed one-unit ownership, and the exact development-only Qwen/Serena adapter. Packaged WebView2 validates the model-free lifecycle. The first actual exact-host matrix passes bounded complete-unit delivery, backpressure, every required invalidation, termination, zero stale audio, cleanup, and explicit reload. Final pull-request CI and plan archival remain. The product narration caller, playback, and distribution remain M008/later work. This is not a passing standard profile, production, native-model-streaming, continuous-playback, or general-hardware claim.
...
- [`plans/active/M007-local-tts-service-and-process-protocol.md`](plans/active/M007-local-tts-service-and-process-protocol.md): active closeout ExecPlan for the constrained one-GPU local service and process protocol. Milestones 1-6 implement and locally validate transport, contracts, Python service, native supervision, typed one-unit ownership, exact Qwen/Serena integration, and measured content-safe exact-host handoff. Required final pull-request CI and plan archival remain. It does not authorize production or general-hardware claims.
