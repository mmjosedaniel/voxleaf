...
Roadmap Milestones 1 through 8 are complete. M008's six implementation milestones provide the constrained one-GPU narration demo: bounded narration preparation from the active visual locator, one-at-a-time M007 synthesis, sole-owner in-memory buffering, Web Audio playback, and accessible quick/prepared controls. The final policy keeps quick start as the default, one minute as the initial prepared and refill target, 10 seconds as low water, `0` ms as the boundary-wait default, `1.0x` playback, and the simultaneous 30-minute ceiling. ADR-0013's standard-profile blocker remains because exact-host buffering exceeds the MVP target of 5 seconds per minute. The path is an exact-development demo, not uninterrupted playback, production distribution, or general hardware support. M009 is in progress: Milestones 1 through 6 freeze segment-level authority, carry bounded audible source ranges through playback, render one non-mutating focus-safe reader highlight/follow projection, synchronize genuine user navigation through identity-first cancellation, persist non-skipping heard checkpoints, and validate the packaged synchronized path on the exact host. The final synchronized run observed nine segment transitions, no stale playback, 0.7 ms p95 follow, one natural underrun/refill, 188 ms cancellation, bounded cleanup, zero generated-audio files, and zero external requests; its 97.76 buffering seconds per playback minute remains exact-host performance evidence and not a production claim. Final M009 closeout remains. Required Ubuntu and Windows pull-request checks passed for completed roadmap milestones.
...
- [`plans/active/M009-synchronized-reading-and-narration.md`](plans/active/M009-synchronized-reading-and-narration.md): active focused ExecPlan for segment-level synchronization, highlighting, focus-safe following, synchronized navigation, and heard-position persistence. Milestones 1 through 6 complete the authority/browser proof, bounded audible-progress projection, reader highlighting/following, identity-first user-navigation integration, non-skipping lifecycle persistence, and exact-host synchronized demo; final closeout remains.
- [`plans/active/synchronized-reader-and-startup-buffer.md`](plans/active/synchronized-reader-and-startup-buffer.md): broad historical context superseded by the focused M009 plan for remaining synchronization work.
