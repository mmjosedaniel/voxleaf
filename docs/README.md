...
- [`plans/active/M005-narration-text-preparation.md`](plans/active/M005-narration-text-preparation.md): active implementation authority for roadmap Milestone 5's deterministic, bounded, locator-linked narration normalization and semantic segmentation pipeline; Tasks 2.1-5.1 implement exhaustive source projection, code-point source-span tokens, bounded canonical source windows/lifecycle, source-mapped neutral/Spanish normalization, deterministic boundary scanning and packing, canonical locator-linked prepared segments, and the closed public `OpenedPublication.prepareNarration` batch operation. The broader synthetic integration matrix and deterministic resource proof remain pending.
