...
- [`plans/active/M006-local-tts-feasibility-and-engine-profiles.md`](plans/active/M006-local-tts-feasibility-and-engine-profiles.md): active ExecPlan for roadmap Milestone 6's privacy-safe local TTS candidate evaluation and engine-profile selection. The evaluation authority and bounded harness are frozen; one official Supertonic CPU run failed first-audio and cancellation gates, while the Qwen CUDA run is blocked by unavailable process-attributed NVML VRAM on the measured WDDM host. No profile is selected.
