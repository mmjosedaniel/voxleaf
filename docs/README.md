...
- [`plans/active/M008-bounded-adaptive-prebuffering.md`](plans/active/M008-bounded-adaptive-prebuffering.md): active implementation plan for quick-start and explicit prepared playback, one-GPU scheduling, adaptive boundary waits, truthful rebuffering, and a 30-minute in-memory ceiling. Milestones 1-4 now provide the frozen authority, deterministic scheduler, payload-owning bounded FIFO, unconnected Web Audio player, content-free preparation presenter, and reusable accessible controls; product mounting/caller, exact-model integration, and closeout remain later milestones.
