...
Roadmap Milestones 1 through 7 are complete. M008's six implementation milestones now provide the constrained one-GPU narration demo: bounded narration preparation from the active visual locator, one-at-a-time M007 synthesis, sole-owner in-memory buffering, Web Audio playback, and accessible quick/prepared controls. The final policy keeps quick start as the default, one minute as the initial prepared and refill target, 10 seconds as low water, `0` ms as the boundary-wait default, `1.0x` playback, and the simultaneous 30-minute ceiling. The packaged exact-host matrix measured 39.238 seconds to audible quick playback, 15.280 seconds of start lead, one underrun, 20.91 buffering seconds per playback minute, 160 ms cancellation, a 66.480-second prepared lead, about 4.8 GiB peak dedicated GPU memory, and zero external requests. Because buffering exceeds the MVP target of 5 seconds per minute, ADR-0013's standard-profile blocker remains. The path is an exact-development demo, not uninterrupted playback, synchronization, production distribution, or general hardware support. Local Milestone 6 closeout is complete; required pull-request CI is the remaining completion gate.
...
- [`plans/active/M008-bounded-adaptive-prebuffering.md`](plans/active/M008-bounded-adaptive-prebuffering.md): active closeout plan for the implemented exact-development quick/prepared path. All six milestones are complete locally; required pull-request CI remains before archival.
