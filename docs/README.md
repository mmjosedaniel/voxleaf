...
- [`plans/active/M010-hardware-profiles-fallback-and-operational-resilience.md`](plans/active/M010-hardware-profiles-fallback-and-operational-resilience.md): active focused ExecPlan for privacy-safe host detection, evidence-backed profile matching, conditional CPU-fallback admission, and identity-safe operational recovery. Milestones 1-3 freeze the authority and implement detection, measured matching, bounded preference reuse, pre-start enforcement, and compatibility UI; standard support, fallback admission, and recovery remain later work.
