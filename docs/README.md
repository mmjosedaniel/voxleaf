...
Roadmap Milestones 1 through 6.1 are complete. The visual reader, bounded local restoration, locator-linked narration preparation, and candidate-neutral TTS feasibility harness are implemented and validated within their documented boundaries. The first frozen TTS evaluation selected no viable profile. Milestone 6.1's exact Qwen3-TTS 1.7B CustomVoice/Serena `v3` machine evaluation failed startup, throughput, zero-failure, and mid-generation cancellation gates. Accepted `selection-v3` retains that standard blocker. One fluent maintainer accepted its audible quality for a near-term demo, so ADR-0014 separately permits that exact profile only as a bounded development-demo exception. Local deterministic, candidate-import, repository, privacy, portable, authoritative Windows, and pull-request CI checks passed, completing the plan. Milestone 6.2 is in progress. Its completed result-blind `v4` short-unit/shared-model batch work produced schema-valid full-GPU and targeted speech-tokenizer CPU results, but both stopped on the frozen zero-shared-GPU-memory rule before producing throughput or reviewable audio evidence. Accepted `selection-v4` selects neither placement and records that the targeted component move was not an independent CPU worker. The separate result-blind `v5` authority freezes one GPU-primary Qwen worker plus one fully CPU-only support worker, approximately 8-16-second complete units, the unchanged approximately 15-second startup gate, and an experimental five-minute bounded maximum. Plan Milestone 7 now implements its benchmark-local dual-worker mechanics, exact CPU/GPU adapter paths, bounded ordered replay, deterministic failure coverage, and reviewed non-promotable command surface. No `v5` pilot, official result, quality finding, or two-worker topology approval exists yet. This is not a passing `v3`, production, native-streaming, continuous-playback, or general-hardware claim. Milestone 7 production completion remains blocked, and production TTS, audio, synchronization, hardware support, and release packaging are not implemented.
...
- [`plans/active/M006-002-qwen-short-segment-batch-feasibility.md`](plans/active/M006-002-qwen-short-segment-batch-feasibility.md): active Milestone 6.2 evaluation. Its completed `v4` arms stopped safely and `selection-v4` selects neither placement. The separate independent GPU-primary/CPU-support `v5` authority and benchmark mechanics are implemented; pending Milestones 8-10 own hardware evidence, bounded playback/quality review, and the final decision.
