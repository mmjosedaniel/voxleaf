...
- [`plans/active/M005-narration-text-preparation.md`](plans/active/M005-narration-text-preparation.md): active implementation authority for roadmap Milestone 5's deterministic, bounded, locator-linked narration normalization and semantic segmentation pipeline; Tasks 2.1-4.2 implement package-internal exhaustive source projection, code-point source-span tokens, bounded canonical source windows/lifecycle, source-mapped neutral/Spanish normalization, cross-category invariants/privacy tests, deterministic sentence/dialogue/clause/protected-token scanning, and block-local profile-bounded semantic packing while oversized-token hardening, prepared locator ranges, and the public operation remain pending.
