...
- [`plans/active/M005-narration-text-preparation.md`](plans/active/M005-narration-text-preparation.md): active closeout authority for roadmap Milestone 5's implemented deterministic, bounded, locator-linked narration normalization and semantic segmentation pipeline. Tasks 2.1-5.3 implement and validate exhaustive source projection, code-point source-span tokens, bounded canonical source windows/lifecycle, source-mapped neutral/Spanish normalization, deterministic boundary scanning and packing, canonical locator-linked prepared segments, the closed public `OpenedPublication.prepareNarration` batch operation, its repository-authored public EPUB-to-segment integration matrix, and deterministic exact-bound/resource evidence. Task 6.1 reconciles the implemented boundary across current documentation; final Task 6.2 validation remains pending.
