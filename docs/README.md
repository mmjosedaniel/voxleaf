...
- [`plans/active/M005-narration-text-preparation.md`](plans/active/M005-narration-text-preparation.md): active implementation authority for roadmap Milestone 5's deterministic, bounded, locator-linked narration normalization and semantic segmentation pipeline; Tasks 2.1-2.3 implement package-internal exhaustive source projection, code-point source-span tokens, bounded canonical source windows, continuation, cancellation, and close ownership while normalization, segmentation, prepared ranges, and the public operation remain pending.
