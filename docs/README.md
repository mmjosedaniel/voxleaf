...
Roadmap Milestones 1 through 6.1 are complete. The visual reader, bounded local restoration, locator-linked narration preparation, and candidate-neutral TTS feasibility harness are implemented and validated within their documented boundaries. The first frozen TTS evaluation selected no viable profile. Milestone 6.1's exact Qwen3-TTS 1.7B CustomVoice/Serena `v3` machine evaluation failed startup, throughput, zero-failure, and mid-generation cancellation gates. Accepted `selection-v3` retains that standard blocker. Milestone 6.2 has reached its local decision checkpoint: `selection-v5` rejects the CPU-only and dual-worker alternatives and retains exactly one GPU worker only under a constrained development-demo exception. The same-authority GPU baseline measured RTF 1.467; a low-application-load diagnostic completed both workers but improved aggregate RTF by only about 2.6% while slowing the GPU worker substantially. ADR-0015 therefore supersedes ADR-0014's scheduling details with an approved plan for bounded one-GPU adaptive preparation: quick start at approximately 15 playable seconds or an explicit 1-, 2-, 5-, or 10-minute preparation target, generation during playback-only pause, honest buffering near the generation frontier, and an in-memory ceiling of approximately 30 minutes. None of that runtime behavior is implemented yet. M006-002 remains active only until required pull-request CI closes Milestone 10; its final local validation passes. This is not a passing standard profile, production, native-streaming, continuous-playback, or general-hardware claim. Production TTS, audio, synchronization, hardware support, and release packaging are not implemented.
...
- [`plans/active/M006-002-qwen-short-segment-batch-feasibility.md`](plans/active/M006-002-qwen-short-segment-batch-feasibility.md): active Milestone 6.2 closeout. `selection-v5` rejects CPU-only and dual-worker scheduling and retains one GPU worker only for the constrained demo. Final local validation passes; Milestone 10 awaits required pull-request CI.
