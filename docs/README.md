...
- [`plans/active/M005-narration-text-preparation.md`](plans/active/M005-narration-text-preparation.md): active implementation authority for roadmap Milestone 5's deterministic, bounded, locator-linked narration normalization and semantic segmentation pipeline; Tasks 2.1-3.3 implement package-internal exhaustive source projection, code-point source-span tokens, bounded canonical source windows/lifecycle, and source-mapped whitespace, line-break, hyphenation, punctuation, quotation, ellipsis, allowlisted-symbol, and Spanish lexical/numeric normalization while complete invariants, segmentation, prepared ranges, and the public operation remain pending.
