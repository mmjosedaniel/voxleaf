...
- Record the failed Milestone 6.2 `v4` decision, then freeze and execute its
  separately versioned `v5` CPU-solo and concurrent GPU-primary/CPU-support
  evaluation before claiming that dual-worker generation supports sustainable
  or continuous narration.
