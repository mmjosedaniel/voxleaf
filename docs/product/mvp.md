...
- Continue the active Milestone 6.1 blocker-resolution plan: run the already-frozen built-in-speaker screen, freeze one eligible CustomVoice speaker/instruction in `v3`, prove exact incremental/cancellation behavior, and select the profile only if every applicable gate passes before local TTS integration begins.
