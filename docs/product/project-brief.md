...
The implemented product boundary currently includes private local EPUB ingestion, semantic visual reading/navigation, bounded restoration, deterministic locator-linked narration preparation, the M007 local service, M008's exact-development narration coordinator, buffer, player, and controls, and completed M009's segment-level highlight/follow/navigation plus non-skipping heard-position persistence. M008's final demo policy selects quick mode by default, one minute as the initial prepared/refill target, a 10-second warning, zero default boundary wait, `1.0x` playback, and a simultaneous 30-minute ceiling. Its final policy matrix measured 19.49 buffering seconds per playback minute. M009 subsequently validated the complete synchronized loop on the exact host with no stale playback, bounded cleanup, zero external requests, and 378.46 buffering seconds per playback minute. A later manual real-publication run did not visibly show the active segment highlight and exposed an outer-page layout that could place the reader below controls. M009.1 Milestone 2 repairs the reproduced same-chapter materialization defect and passed clean-host validation. Milestone 3 implements one dedicated reader scroll viewport, stable compact application/publication/narration chrome, collapsible narration detail, and exact loaded/target/estimate text without a progress bar. Milestone 4 implements one bounded locator-backed paragraph leaf. Milestone 5 exact-host work validates the integrated reader and separates passive viewport inspection from active narration: scrolling updates the leaf's visual-line default, pointer hover temporarily previews the exact eligible paragraph, and only activating the leaf changes the narration start. The corrected private-EPUB interaction and amended exact-host matrix pass; final repository, clean-host, and pull-request closeout remains before M010 adds compatibility and recovery UI. The standard-profile blocker therefore remains. A supported production TTS profile, general hardware profiles, validated fallback and recovery, and installers remain deferred. The rest of this brief describes the intended complete product unless it explicitly identifies implemented behavior.
