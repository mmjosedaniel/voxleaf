...
| TTS process                       | Approved plan for constrained demo; not implemented       | Persistent local process under native desktop supervision                                                                                         | M007 must select and validate lifecycle, transport, framing, recovery, cancellation containment, and local exposure; production and general-hardware support remain blocked |
...
| Process transport                 | Unselected; M007 decision gate                            | Rust-owned child-process standard streams are the first prototype; local socket and loopback WebSocket remain comparison alternatives             | Freeze transport, framing, allocation bounds, backpressure, exposure, and crash behavior in an ADR before completing Milestone 7                                            |
