...
The implemented product boundary currently includes private local EPUB ingestion, semantic visual reading/navigation, bounded restoration, deterministic locator-linked narration preparation, the M007 local service, M008's exact-development narration coordinator, buffer, player, and controls, completed M009's segment-level highlight/follow/navigation plus non-skipping heard-position persistence, and completed M009.1 reader stabilization. M009.1 repairs the reproduced same-chapter highlight-materialization defect, implements one dedicated reader scroll viewport with compact/collapsible narration and exact loaded-duration text, adds one bounded locator-backed paragraph leaf, and separates passive viewport inspection from explicit narration replacement. Its corrected private-EPUB interaction, amended exact-host matrix, repository/privacy audit, portable and packaged checks, and required Ubuntu/Windows CI pass; pull request #142 merged the final closeout. M010 is in progress with Milestones 1-5 complete and Milestone 6 implemented and exact-host validated; final corrective closure awaits the private-book rerun. Privacy-safe host detection, four-entry measured matching/preference, compatibility UI, identity-safe explicit recovery, the supported Piper/davefx CPU adapter, native profile selection, Piper-only locator-safe spoken-expansion-aware narration sizing, Piper-only omission of punctuation units that produce no waveform, and retained development-only Qwen/Serena are present. That omission inserts no silence and preserves locator continuation. The corrective packaged Piper resilience arm passes its ordinary-prose and compact-form regressions. Qwen passes the outbound-blocked service lifecycle and the packaged host correctly rejects it at the frozen available-VRAM gate without starting the model or making an external request. Milestone 7 closeout remains. Installer distribution remains deferred to M011. The rest of this brief describes the intended complete product unless it explicitly identifies implemented behavior.
...
| CPU fallback                      | Integrated; packaged resilience arm passed                              | Piper 1.4.2, `es_ES-davefx-medium`, ONNX Runtime CPU float32; total sustained RTF 0.0252, 392 MiB measured peak model-tree RAM, overall quality 4.621/5; final packaged quick start 2.722 s with zero underruns | M010 Milestone 7 closeout remains; M011 must fulfill GPL/CC0 packaging obligations                                                                                     |
