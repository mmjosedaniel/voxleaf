...
installation as a separate product-channel decision.
...
ADR-0049 and release authority v2, Milestone 7 records Piper local/portfolio GO
and conditional Chatterbox GO for systems that pass the published gate. The
ordinary Download action remains withheld, and signed public publication is
...
Milestone 7 confirms that the local/portfolio MVP support boundary is closed
using published requirements plus representative evidence. Describe the
unsigned and ordinary-Download limitations explicitly, and keep trusted public
publication prospective until signing is authorized:
