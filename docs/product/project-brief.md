...
support/recovery decision and entered closeout validation. Privacy-safe host detection,
four-entry measured matching/preference, compatibility UI, identity-safe
...
an explicit choice rather than automatic failover. Required closeout
pull-request checks remain before M010 and M008.1 are archived. Installer
distribution and Piper license fulfillment remain deferred to M011.
...
| CPU fallback                      | Supported; integrated packaged resilience arm passed                    | Piper 1.4.2, `es_ES-davefx-medium`, ONNX Runtime CPU float32; total sustained RTF 0.0252, 392 MiB measured peak model-tree RAM, overall quality 4.621/5; final packaged quick start 2.722 s with zero underruns    | Replacement closeout CI remains; M011 must fulfill GPL/CC0 packaging obligations                                                                                       |
...
| Audio-unit transitions            | M008.1 implementation and local repository validation pass              | Boundary-specific scheduled delays between already-buffered units; no silent PCM, fade, model-input rewrite, or delay after real buffering/final completion                                                        | Listening may tune only a future version; replacement closeout CI remains                                                                                             |
