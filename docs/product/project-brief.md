...
The implemented product boundary currently includes private local EPUB ingestion, semantic visual reading/navigation, bounded restoration, deterministic locator-linked narration preparation, the M007 local service, M008's exact-development narration coordinator, buffer, player, and controls, and completed M009's segment-level highlight/follow/navigation plus non-skipping heard-position persistence. M008's final demo policy selects quick mode by default, one minute as the initial prepared/refill target, a 10-second warning, zero default boundary wait, `1.0x` playback, and a simultaneous 30-minute ceiling. Its final policy matrix measured 19.49 buffering seconds per playback minute. M009 subsequently validated the complete synchronized loop on the exact host with no stale playback, bounded cleanup, zero external requests, and 378.46 buffering seconds per playback minute. A later manual real-publication run did not visibly show the active segment highlight and exposed an outer-page layout that can place the reader below controls. M009.1 Milestone 2 now repairs the reproduced same-chapter materialization defect without changing timing, document, storage, or protocol authority; deterministic and Chromium validation pass while clean-host packaged validation remains pending. M009.1 continues with one dedicated reader viewport, a bounded locator-backed paragraph leaf, compact/collapsible narration, and exact loaded-duration text before M010 adds compatibility and recovery UI. The standard-profile blocker therefore remains. A supported production TTS profile, general hardware profiles, validated fallback and recovery, and installers remain deferred. The rest of this brief describes the intended complete product unless it explicitly identifies implemented behavior.
