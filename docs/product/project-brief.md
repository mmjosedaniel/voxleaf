...
M010.1 follow-up may add explicit English narration and at most one newly
admitted engine, but neither capability exists yet. Installer distribution and
...
| Bilingual narration               | Approved planned; not implemented                                      | M010.1 will evaluate explicit Spanish/English selection, versioned shared preparation, and an exact Piper English baseline before any support claim                                                                | Freeze result-blind authority, pass language-specific correctness/quality/privacy gates, then integrate only admitted profiles                                         |
