...
M010.1 follow-up has frozen the result-blind authority that may add explicit
English narration and at most one newly admitted engine, but neither runtime
capability exists yet. Installer distribution and Piper license fulfillment
...
| Bilingual narration               | Authority frozen; runtime not implemented                               | M010.1 Milestone 1 freezes explicit Spanish/English selection, versioned shared normalization, an exact Piper English baseline, and bounded v7 candidate gates before any support claim                                      | Implement the frozen boundary, pass language-specific correctness/quality/privacy gates, then integrate only admitted profiles                                        |
