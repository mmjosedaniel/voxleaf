...
The implemented product boundary currently includes private local EPUB ingestion, semantic visual reading/navigation, bounded restoration, deterministic locator-linked narration preparation, the M007 local service, M008's exact-development narration coordinator, buffer, player, and controls, and M009's segment-level highlight/follow/navigation plus non-skipping heard-position persistence. M008's final demo policy selects quick mode by default, one minute as the initial prepared/refill target, a 10-second warning, zero default boundary wait, `1.0x` playback, and a simultaneous 30-minute ceiling. The final packaged matrix proves an audible constrained demo with cancellation, bounded resources, and zero external requests, while measuring 19.49 buffering seconds per playback minute. The standard-profile blocker therefore remains. M009 exact-host synchronization and final closeout, a supported production TTS profile, general hardware profiles, and installers remain deferred. The rest of this brief describes the intended complete product unless it explicitly identifies implemented behavior.
...
| Adaptive demo buffering           | Exact-development path and final policy implemented                   | Quick default; explicit 1-/2-/5-/10-minute preparation with 1 minute initially selected; 1-minute refill; 10-second warning; zero default boundary wait; approximately 30-minute ceiling     | M009 exact-host synchronization remains; production/general-hardware graduation remains blocked                                                                       |
...
| Internal audio                    | Exact-development product path implemented                            | Bounded 24-kHz mono float32-le units; sole-owner FIFO; one transient active-device copy; Web Audio; content-free controls and metrics                                                        | Exact-host synchronized-demo validation remains M009; no audio persistence or standard-profile claim                                                                  |
...
segment projection, navigation, and heard persistence are implemented through
M009 Milestone 5; exact-host synchronization, a supported production profile,
general-hardware validation, and distribution still require later evidence.
Current
