...
requirement or implemented runtime. M007 is in progress; its completed
model-free first milestone freezes the accepted process boundary, and later milestones must
complete the service before M008 connects the real model.
...
general hardware support. The frozen native-only development keys
`VOXLEAF_TTS_DEV_ENABLED`, `VOXLEAF_TTS_DEV_PYTHON`, and
`VOXLEAF_TTS_DEV_MODEL_ROOT` remain unconsumed until the exact adapter replaces
the model-free child; do not configure them for Milestone 3.
