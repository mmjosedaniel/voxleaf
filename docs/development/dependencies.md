...
M010.1 Milestone 1 adds two evaluation-only isolated projects without changing
the base Python or shipped application graphs:

- `chatterbox_multilingual_v3` locks `chatterbox-tts==0.1.7` and its complete
  transitive graph for the exact Chatterbox Multilingual V3 bounded screen.
- `moss_tts_nano_100m_onnx_cpu` locks MOSS-TTS-Nano source revision
  `cc7bdf19c7639c0870dab22045a33b442760f6be` and its complete transitive graph
  for the exact CPU ONNX bounded screen.

The existing isolated Piper 1.4.2 project supplies the separately frozen
English `en_US-joe-medium` baseline lock. Exact model, codec, tokenizer,
conditioning, and voice artifacts remain outside Git; their repository
revisions, byte sizes, hashes, licenses, offline rules, and native audio
conversion are frozen in
[`candidates-v7.json`](../../benchmarks/tts/candidates-v7.json). CosyVoice
receives no project or lock because the v7 intake found no exact non-personal
default-voice path to authorize. These locks are evaluation inputs only: they
...
separate Chatterbox graph. The measured parts are published, but its ordinary
release manifest is compatibility-gated as a separate channel state. Its
...
distributable product. Under ADR-0049 and release authority v2, renewed
Milestone 7 records Piper local/portfolio GO, compatible-host Chatterbox runtime
and optional-package GO, and gated ordinary Download GO.
Signed public publication remains pending external authorization. These
decisions do not change the already closed dependency graphs.
...
completed exact local English and Spanish generation. Milestone 4B replaces
those planning sizes with reproducible runtime-only and exact model
measurements. They remain package evidence; ordinary Download availability is a
separate manifest/channel result.
...
contains 400 exact core/optional/not-shipped records and is regenerated from
...
| ------------------ | ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@tauri-apps/api`  | `2.11.1`          | Supplies the typed frontend `invoke` boundary used by M007's narrow model-free protocol probe and production-shaped process client. It exposes no shell/process plugin and adds no network listener or capability.                                                                                                                                                                                                                                                                                                                        | Enabling the global Tauri object or calling undocumented internals would create a broader, less typed frontend surface. ADR-0016 accepts the optimized binary response path.                                                                                                                                                                                                                                                                                                       |
| `@voxleaf/epub`    | workspace `0.0.0` | Gives the desktop the public in-memory opener, immutable publication/semantic/navigation/raster types, package-owned locator and semantic-target resolution, lazy bounded resource reads, and fixed EPUB failure contract used by the publication session, reader, and persistence coordinators. It adds no external package or capability beyond the EPUB package's already inventoried dependencies.                                                                                                                                    | Relative source imports would bypass package exports and composite build boundaries; duplicating ingestion, semantic, navigation, resource, locator, or lifecycle contracts in the desktop would create a second authority.                                                                                                                                                                                                                                                        |
| `@voxleaf/shared`  | workspace `0.0.0` | Gives the desktop direct access to canonical locator/index/state types, domain constructors, the strict persisted-reading-state decoder, and fixed operational errors used by the publication, reader-range, save, restore, and repository boundaries.                                                                                                                                                                                                                                                                                    | Reconstructing serialized/domain contracts locally would violate ADR-0006. Relying on the transitive EPUB edge would make the desktop's direct contract use undeclared.                                                                                                                                                                                                                                                                                                            |
...
| `tauri` Rust crate | `2.11.5`          | Creates the native Windows application, embeds the local webview, owns M007's persistent child supervisor, and exposes only fixed lifecycle/synthesis/cancellation commands plus optimized binary responses. The shell registers no plugin and grants no capability.                                                                                                                                                                                                                                                                      | Electron was rejected for its expected runtime footprint; browser-only deployment cannot satisfy local process requirements; a renderer-accessible shell plugin would be broader than the application-owned commands selected by ADR-0016.                                                                                                                                                                                                                                         |
| `serde`            | `1.0.229`         | Derives the closed native cancellation input shape at the Tauri trust boundary. All other protocol payloads remain canonical JSON values validated before use.                                                                                                                                                                                                                                                                                                                                                                            | Hand-parsing command arguments would weaken unknown-field rejection. A broader generated Rust protocol model is not justified while canonical shared schemas and explicit semantic checks already own the wire authority.                                                                                                                                                                                                                                                          |
| `serde_json`       | `1.0.151`         | Parses and writes bounded protocol control values, rejects duplicate/non-finite control input through the repository strict visitor, and validates shared fixtures in native tests.                                                                                                                                                                                                                                                                                                                                                       | A second JSON implementation or handwritten string parser would add more trust-boundary risk. This crate was already resolved transitively and used in tests; direct runtime ownership makes the supervisor's actual use explicit.                                                                                                                                                                                                                                                 |
| `sha2`             | `0.10.9`          | Computes SHA-256 for every file in the fixed Piper core manifest before the native supervisor accepts the packaged interpreter, service, or voice. It receives only installer-owned fixed paths and exposes no renderer command.                                                                                                                                                                                                                                                                                                          | Trusting filenames, sizes, HTTPS, or installer placement alone would not detect substituted runtime bytes. The crate was already transitively locked through Tauri; making it direct records the native verifier's actual production use without adding another resolved package.                                                                                                                                                                                                  |
| `reqwest`          | `0.13.4`          | Provides M011's native-only optional-Chatterbox HTTPS client after explicit user confirmation and a second native host gate. It accepts only exact manifest URLs, HTTPS, no proxy/credentials, and at most two redirects into the frozen GitHub Release or official Hugging Face content-addressed host sets. Every response needs exact content length, streamed byte ceiling, and SHA-256 verification in application-owned staging. The renderer supplies no request data. The v2 manifest is downloadable only after both gates pass. | A renderer HTTP/plugin path would widen WebView authority; browser download would expose artifacts to uncontrolled handling; shelling out would add command/path injection and system-tool dependence. Disabling all redirects would make the official Hugging Face file service unusable; an open redirect client would permit source substitution. The bounded native policy is the narrow accepted middle.                                                                      |
| `zip`              | `2.4.2`           | Reads only the already size-and-digest-verified optional archive in native code. The wrapper requires the exact root, rejects absolute/traversal/link entries, bounds extraction, verifies the exact runtime file set, and promotes atomically only after verification.                                                                                                                                                                                                                                                                   | Invoking an installed archive tool would introduce a shell/tool dependency. A general archive abstraction or handwritten ZIP parser would create a broader or less-reviewed parser surface. The core Piper package does not use this optional-path dependency at runtime.                                                                                                                                                                                                          |
| `windows-sys`      | `0.61.2`          | Uses only Foundation, Security, JobObjects, and Threading bindings to place the supervised child and descendants in a kill-on-close Windows Job Object. It is target-specific and exposes no renderer API.                                                                                                                                                                                                                                                                                                                                | Killing only the immediate child can leave model descendants alive. A general process-management crate or Tauri shell plugin would add a broader surface than the four reviewed Win32 feature groups. Non-Windows builds use the standard child-kill fallback until platform-specific containment is implemented and validated.                                                                                                                                                    |
| `windows`          | `0.61.3`          | Supplies target-specific typed bindings for M010's direct Windows OS, storage, DXGI, D3D12, DirectML, and loader APIs. The native probe loads only the system CUDA driver library from `System32`, requests fixed symbols, uses adapter LUIDs transiently for joins, and serializes only the bounded identity-free compatibility report. It adds no Tauri plugin, renderer permission, shell, network request, model load, or persistence. Licensed MIT OR Apache-2.0.                                                                    | Handwritten COM/vtable and FFI declarations would duplicate ABI-sensitive generated bindings. PowerShell, WMI, `wmic`, `nvidia-smi`, registry enumeration, a Tauri OS/shell plugin, and broad hardware-inventory crates expose larger identity/raw-output surfaces. The version was already present transitively in the lock; direct declaration changes ownership and enabled code only, not the resolved package set. Rollback removes the detector module and this direct edge. |
...
The Rust native shell directly owns `serde`, `serde_json`, `sha2`, the Windows-only
`windows-sys` features above for the model-free supervisor, and the
Windows-only `windows` features for the privacy-safe host detector.
No EPUB renderer, TTS engine, model runtime, audio library, or persistence
library participates in the application runtime.
...
mid-generation cancellation gates. ADR-0015 supersedes ADR-0014's scheduling
and buffering details and permits the exact profile only as one GPU worker for
a future bounded adaptive development demo; it does not add the candidate to
the production graph or approve distribution. The content-free
...
improves aggregate throughput. The completed hardware evidence now answers
those questions: CPU-only generation is too slow for the selected role, the
...
uses only the existing standard library and `tauri` crate; it adds no Tauri
plugin, capability, listener, Python dependency, model runtime, or audio
library. ADR-0016 is accepted after packaged WebView evidence passed. The
...
Milestone 4 uses the WebView's built-in HTML file input, `FileReader`, `AbortController`, `ArrayBuffer`, and `Uint8Array` APIs for ADR-0009's local-file ingress; `Blob`, object URLs, `HTMLImageElement.decode()`, and `IntersectionObserver` for ADR-0010's bounded lazy raster lifecycle; DOM `Range`, caret geometry, `ResizeObserver`, and animation frames for semantic mapping, visual-position tracking, and reflow settlement; and `window.localStorage`, `visibilitychange`, and `pagehide` behind ADR-0011's asynchronous bounded repository/save boundary. These are platform APIs, not package dependencies. The raster boundary adds only `img-src 'self' blob:` to CSP; the complete reader adds no JavaScript or Rust runtime dependency, Tauri plugin/command/capability, host-path contract, network origin, or lockfile change. Official Tauri dialog/filesystem/store plugins, repository-owned Rust file/image/storage commands, third-party raster decoders, and renderer-oriented EPUB frameworks remain rejected alternatives unless a future demonstrated requirement cannot be satisfied by the current WebView boundary.
...
| `ajv`                         | `8.20.0`  | Compiles the canonical Draft 2020-12 schemas into deterministic standalone TypeScript validator artifacts during generation and independently validates the serialized fixture corpus during tests. It performs no network access; no Ajv module or runtime compiler path is included in the production desktop bundle. |
...
These packages are development-only. ESLint with `typescript-eslint`, Prettier, Vitest, Testing Library, jsdom, and Playwright were selected as a compact stack aligned with React and Vite. Biome could combine formatting and linting, Jest could replace Vitest, and Vitest browser mode was a credible alternative to Playwright; Playwright was selected because it supplies a direct, version-coupled Chromium runner and first-class viewport, storage, focus, accessibility-role, network, and trace controls without replacing the fast jsdom component layer. The separate Tauri/WebDriver harness supplies packaged WebView2 evidence that Chromium cannot prove. A monorepo task runner such as Nx or Turborepo remains unjustified for three TypeScript projects.

Playwright `1.61.1` selects Chrome for Testing `149.0.7827.55` / Chromium revision `1228` on Windows, plus its matching headless-shell and support binaries. `pnpm.cmd install --frozen-lockfile` installs only the package graph; CI also sets `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1` for that step. Browser acquisition is a separate, explicit `pnpm.cmd test:browser:install` operation. Playwright stores the acquired binaries in its default per-user cache and ordinary `pnpm.cmd test:browser` execution neither downloads nor updates them. CI does not restore a browser cache: the authoritative Windows job performs the explicit matching install before the offline smoke test. Playwright and its browser binaries are test-only and are not included in the desktop production bundle.

The native startup smoke uses test-only `tauri-driver` `2.0.6`, installed from crates.io with its locked graph, and `msedgedriver-tool` `0.2.2` at Git revision `8c4b34f51b45f5cf08013366d703de464ab871d1`. Both tools are MIT/Apache-2.0 licensed. The pinned tool downloads Microsoft's signed EdgeDriver for the exact installed Evergreen WebView2 version into a disposable CI directory. A small repository-owned Node client uses only built-in `fetch` to issue the narrow W3C WebDriver and Microsoft CDP commands required by the smoke. Selenium would add an unnecessary package abstraction; WebdriverIO's embedded Tauri provider would also add production Rust plugins and automation capabilities. None of the selected tools or the downloaded driver enters a package manifest, lockfile, application bundle, installer, or production process.

`json-schema-to-typescript` is preferred over handwritten TypeScript wire DTOs because generated DTOs cannot silently become an independent contract authority. TypeScript-first runtime-schema libraries were rejected because the cross-language schema should not be generated from a language-specific executable source. Ajv remains the pinned Draft 2020-12 compiler and independent TypeScript conformance validator, but Task 2.5 makes it development-only: `generate:check` owns validator drift, the fixture suite compares standalone results with freshly compiled canonical-schema results, and the desktop production build rejects every Ajv module plus runtime `eval`/`Function` construction. The generated validators are derived artifacts, not another contract authority.
