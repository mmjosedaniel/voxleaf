...
| --- | --- | --- | --- |
| `@voxleaf/epub` | workspace `0.0.0` | Gives the desktop publication-session owner the public in-memory opener, immutable publication handle, and fixed EPUB failure contract. It adds no external package or capability beyond the EPUB package's already inventoried dependencies. | Relative source imports would bypass package exports and composite build boundaries; duplicating ingestion or lifecycle contracts in the desktop would create a second authority. |
| `@voxleaf/shared` | workspace `0.0.0` | Gives the desktop direct access to canonical shared domain/error constructors; Task 2.2 uses it to construct a fixed content-free internal failure when an unexpected application-boundary throw occurs. | Reconstructing serialized/domain contracts locally would violate ADR-0006. Relying on the transitive EPUB edge would make the desktop's direct contract use undeclared. |
