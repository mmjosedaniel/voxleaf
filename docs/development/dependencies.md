...
The approved [Milestone 5 ExecPlan](../plans/active/M005-narration-text-preparation.md) adds no dependency by itself, and narration-preparation implementation has not started. Its default design uses repository-owned deterministic scanners over the existing safe semantic model and shared locator contracts. Task 1.1 must document a concrete unmet requirement, deterministic runtime behavior, source-range support, Spanish coverage, license/transitive/install/runtime impact, and alternatives before any new production dependency is proposed. No segmentation library, language detector, model tokenizer, pronunciation package, or model-specific preprocessor is currently selected or approved.
