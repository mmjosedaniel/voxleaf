...
The package-internal executable probes are intentionally not exported from `@voxleaf/epub`; Tasks 2.2 through 2.4 build the production archive and XML security adapters over these proven primitives. The probes and production adapters establish the following behavior with synthetic in-memory input:
