...
| `@zip.js/zip.js` | `2.8.30` | Supplies narrowly wrapped, in-memory ZIP metadata iteration and entry decompression for future EPUB ingestion. VoxLeaf selects the pure-JavaScript core subpath, disables workers and native compression streams, enables strict ambiguity, overlap, and signature checks, and never instantiates its HTTP, Blob, or filesystem-oriented APIs. | JSZip and fflate were credible low-level alternatives, but adopting either would require more repository-owned ZIP metadata, overlap, integrity, streaming, and cancellation machinery. A renderer-oriented EPUB framework is prohibited by ADR-0007. Version `2.8.30` is the first release with the required strict canonical archive selection and had crossed the workspace's seven-day release-age gate; `2.8.31` through `2.8.33` had not. |
...
The package-internal executable probes are intentionally not exported from `@voxleaf/epub`; Tasks 2.2 through 2.4 will build the production security adapters over these proven primitives. The probes establish the following behavior with synthetic in-memory input:

| Concern | Evidence and accepted boundary |
| --- | --- |
| ESM and TypeScript | Both package imports compile under the repository's strict ESM composite project and execute under Vitest. The public `@voxleaf/epub` API remains empty. |
...
| XML security | `saxes` parses chunks with `xmlns: true`, forced XML 1.0, no DOM, and no resolver callback. Internal and external `DOCTYPE` declarations and undeclared custom entities fail with only `malformed-xml`; a network spy remains untouched. Built-in XML character references remain supported. |
| XML cancellation | The synchronous parser is fed bounded chunks, with an `AbortSignal` checkpoint before each write and before close. Task 2.4 will add the authoritative byte, depth, attribute, node, text, and deadline budgets. |
...
`@zip.js/zip.js` is BSD-3-Clause licensed, declares no runtime transitive dependencies, has no install lifecycle hook, and reports 5,069,239 unpacked bytes for the complete multi-runtime package. The application does not currently import the internal probe, so it adds no bytes to the current desktop bundle. Future production ingestion will retain the narrow pure-JavaScript core import and must be measured again when it becomes reachable from the desktop application.
