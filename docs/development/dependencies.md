...
Piper graph as the standalone core payload. Milestone 4 now implements the
native archive lifecycle and deterministic source-package builder for the
separate Chatterbox graph, but its release manifest is intentionally withheld
until a real artifact has passed publication, licence, and clean-host gates. Its
...
| `reqwest`          | `0.13.4`          | Provides M011's native-only optional-Chatterbox HTTPS client after a separate explicit user confirmation and second native host gate. It allows only a fixed `https` origin, disables redirects and proxy use, requires an exact content length, and streams into an application-owned staging file before SHA-256 verification. The renderer cannot supply request data. The checked-in manifest is currently withheld, so no product download is currently reachable. | A renderer HTTP/plugin path would widen WebView authority; browser download would expose the archive to uncontrolled handling; shelling out to a downloader would add command/path injection and system-dependency risk. Handwritten WinHTTP bindings would duplicate TLS/streaming failure handling. Native TLS is selected for the Windows-only package boundary. |
