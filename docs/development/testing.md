...
The white-window defect demonstrated that the Chromium production-preview smoke does not apply the packaged Tauri CSP: the former Ajv runtime compilation was blocked before React mounted even though browser and unit tests passed. Run `pnpm.cmd test:native-startup` on native Windows to build the release executable, launch it with a disposable isolated WebView2 profile and loopback-only debugging endpoint, and verify root mount, the visible main landmark, a repository-authored deterministic EPUB open/close, zero page/console errors, and zero external requests. The harness exposes only fixed stage failures, never logs the fixture, filename, URL, page error, console value, or publication content, and removes its temporary profile and EPUB.
