...
- `apps/desktop/tests/browser/foundation.smoke.spec.ts` runs the production Vite build in Playwright's pinned Chromium and proves the local open/render/image/navigation/layout/focus/storage-isolation path with zero non-loopback requests. `active-visual-locator.smoke.spec.ts` drives the production tracker through top, partial-crossing, between-block, and terminal-block geometry with native caret invocation and no focus, URL, storage, page-error, or network side effects. `reader-reflow-restoration.smoke.spec.ts` opens a repository-authored long reflow fixture, captures one nonzero semantic code-point through test-side Range instrumentation, and proves the same canonical range returns to the reading line across all four approved preferences, synchronous superseding changes, viewport resize, and Chromium CSS zoom while focus, URL, reader-storage keys, page errors, and network activity remain unchanged. `large-chapter.smoke.spec.ts` proves the 10,001-block fallback appears before publisher content and remains closable. These tests do not claim persisted-position restoration, native WebView2 reflow/navigation behavior, or full automated accessibility coverage.
...
The secure EPUB ingestion scenario and boundary matrix is implemented with repository-authored synthetic inputs, the deterministic desktop suite proves bounded local reader-state persistence in isolation, the real-browser smokes open EPUB publications through the production React reader, and the separate benchmark covers accepted prototype and production React limits. No current test schedules or restores a saved locator through the application coordinator/persistence boundary, measures native WebView2 reader performance, starts a TTS model, communicates across a process boundary, generates or plays audio, detects supported hardware, builds an installer, or exercises those later end-to-end flows. The examples below are requirements for later roadmap milestones, not claims about current coverage.
