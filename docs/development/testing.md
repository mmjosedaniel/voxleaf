...
Task 3.5 extends both reader smokes with keyboard/focus evidence: the application-owned skip link focuses the generic reading region without changing the browser URL, normal Tab order reaches the file picker and reader controls without positive `tabindex`, native `End` remains uncancelled while reader focus remains stable, keyboard TOC activation focuses its destination heading, passive preference reflow retains the selected control, and keyboard close returns focus to the local EPUB picker. Chromium and native WebView2 also verify a visible forced-colors focus indicator. The smoke uses only the repository-authored synthetic publication and fixed application labels; it captures no publication text, path, or external request.
