...
- `packages/epub/src/index.test.ts` proves that the isolated EPUB package consumes synthetic book and locator contracts through the public `@voxleaf/shared` workspace boundary while its new type-only document-model exports add no runtime values. `packages/epub/src/document/document-model.test.ts` exercises the public closed block/inline/navigation/resource shapes, readonly recursive collections, opaque identifier separation, spine/non-spine documents, and explicit resource-read/close lifecycle without implementing archive opening, sanitization, resource reads, rendering, or persistence. Package-internal path, archive, processing-budget, XML-event, package, identity, and navigation suites exercise untrusted ingestion with synthetic in-memory inputs, strict byte/count/depth/ratio/text limits, injected cancellation/deadlines, namespace-aware XML, fixed content-free failures, and no filesystem, network, worker, or DOM use. XHTML projection, rendering, CFI parsing, locator resolution, and public EPUB opening remain later work.
