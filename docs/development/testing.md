...
- `apps/desktop/src-tauri/src/tts_protocol_probe.rs` has model-free Rust tests for exact and max-plus-one frame/control/audio/identity/narration bounds, malformed/truncated/version failures before payload acceptance, deterministic parent/child validation, and the one-active/no-queue guard. `apps/desktop/src/tts/transport-probe.test.ts` proves binary response type, exact byte length, finite little-endian float samples, fixed synthetic prefix, and content-free error mapping. Neither suite loads a model or contains narration text.
