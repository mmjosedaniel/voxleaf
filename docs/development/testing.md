...
- `packages/epub/src/index.test.ts` proves that the isolated EPUB package consumes synthetic book and locator contracts through the public `@voxleaf/shared` workspace boundary while keeping its own public API empty. Package-internal path, archive, processing-budget, and XML-event suites exercise untrusted ingestion with synthetic in-memory inputs, strict byte/count/depth/ratio/text limits, injected cancellation/deadlines, namespace-aware XML, fixed content-free failures, and no filesystem, network, worker, or DOM use. Sanitization, rendering, CFI parsing, and locator resolution remain later work.
