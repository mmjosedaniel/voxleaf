...
- The current matrix deliberately expects OPF `version="2.0"` to return
  ...
  begin with a passing characterization of that boundary and replace it
  atomically as support lands. Acceptance requires direct and deprecated OPF 2
  metadata forms, exact inert NCX/XHTML doctype tests,
  guide/target/budget/cancellation/privacy regressions, and equivalent public
  reader/restoration/narration journeys with distinct exact-byte identities.
  Planning adds no test or production behavior.
