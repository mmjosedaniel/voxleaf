...
GitHub Actions runs the same authoritative check in the `Windows native foundation` job on `windows-2025`. That job also explicitly installs the Playwright-managed Chromium revision and runs `pnpm.cmd test:browser` before the aggregate check. The separate `Ubuntu portable foundation` job runs `pnpm check:portable` on `ubuntu-24.04`, covering TypeScript and Python validation plus the browser-only desktop build without installing Rust, Playwright browsers, or Linux desktop dependencies. A portable success does not replace native Windows validation.
...
### Native startup and CSP regression — future implementation

The white-window defect demonstrated that the Chromium production-preview smoke does not apply the packaged Tauri CSP: Ajv runtime validator compilation was blocked before React mounted even though browser and unit tests passed. Before Task 2.3 can be marked complete, add a checked-in Windows packaged-WebView startup regression that launches the release executable with an isolated disposable WebView profile and verifies that the application root mounts, the main landmark is visible, no page or console error occurs, and no external request is made. The test must inspect only content-free startup state and must not load or log a private EPUB. The implementation task must define its repository command; no such automated command exists yet.

Run the current manual content-free startup probe after every Tauri CSP change and after material Ajv/shared-contract bundling changes until that regression is checked in. Keep the integrated local-file interaction matrix above as separate native evidence; a mounted shell does not prove selection, cancellation, replacement, or publication cleanup.

The current `unsafe-eval` allowance is also temporary future work. Task 2.5 must generate standalone validators from the canonical checked-in schemas, prove behavioral equivalence with the existing shared-contract and serialized-conformance suites, rebuild the native shell without `unsafe-eval`, and repeat the packaged startup/no-external-request checks before Task 3.1 implementation begins. If standalone generation is rejected or cannot meet the gate, retain no silent exception: document the alternatives and obtain explicit ADR-backed security approval before proceeding.

### Native raster decode safety probe

ADR-0010 requires native Windows WebView2 evidence because jsdom cannot execute `HTMLImageElement.decode()` or prove the packaged CSP. Build and launch the release executable, then activate **Run synthetic raster safety probe**. The fixed status must change from “Raster safety probe has not run” to “Bounded local raster decoding is available.” Repeat the probe to verify that the prior synthetic source was released and a new Blob URL can be decoded under `img-src 'self' blob:`.

The probe uses one checked-in 68-byte repository-authored static PNG value, never a selected file or publication resource. It creates no visible image, filename, path, URL, bytes, or raw error. The fixed unavailable state is failure evidence and must not be rewritten into success. Deterministic tests, rather than this one-pixel native smoke, own exact/max-plus-one dimensions, pixels, frames, concurrency, live capacity, cancellation, postdecode mismatch, and revocation behavior. Repeat the native probe after material Tauri/WebView2, CSP, Blob/object-URL, image-decoder, or raster-boundary changes.
