...
**Status:** In progress. Milestone 1 Tasks 1.1-1.3 are complete: ADR-0012, the deterministic test-only neutral/Spanish corpus, and the model-independent `narration-v1` chunk/resource profile are accepted. Milestone 2 Tasks 2.1-2.2 implement exhaustive package-internal semantic source projection and Unicode-code-point source-span tokens; bounded continuation/lifecycle behavior, normalization, segmentation, prepared locator-range construction, and the public operation remain pending.
