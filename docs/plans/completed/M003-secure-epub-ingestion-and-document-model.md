...
**Status:** Complete. A package-internal locator index now assigns every final semantic block a deterministic start locator in spine order and nested source-start preorder. XHTML `id` and `xml:id` candidates remain in an internal projection sidecar and are preserved only when they identify exactly one source element and one resulting block and pass the unchanged shared v1 decoder; missing, duplicate, conflicting, invalid, or overlong candidates receive text-free `voxleaf-s<spine-index>-a<anchor-index>` replacements with deterministic collision suffixes. Each immutable entry binds the exact-byte BookV1 identity, spine ID/index, anchor ID/index, block reference, start offset zero, and legal text length. Heading/paragraph lengths count final semantic text by Unicode code point, with one position for line breaks and raster images; structural containers accept only offset zero. Arbitrary legal offsets are reconstructed through the shared decoder, while invalid offsets fail with `locator-unresolved`. Assignment is cancellation/deadline-aware through the shared ingestion budget and returns no partial index. Synthetic tests cover source preorder, nested blocks, reversed projection input, repeatability, decoder round trips, exact/max+1 ID bounds, invalid/duplicate/collision fallback, astral and combining Unicode, offset limits, spine inconsistencies, cancellation, deep immutability, and content-free failures. Task 5.2 resolution, progression, public opening/runtime exports, rendering, persistence, and prose search remain deferred.
...
**Status:** Complete. A package-internal resolver now validates untrusted locator structure through the shared v1 decoder and resolves only against the immutable Task 5.1 index. Exact results require matching book identity, spine ID/index, anchor value/index, and a legal Unicode-code-point offset. Same-spine recovery prefers a matching unique anchor value, otherwise clamps to nearest anchor order, then clamps the offset; an addressless matching spine selects the nearest non-empty spine with an earlier-index tie break, while inconsistent spine identity falls back to book start. Wrong-book, malformed, and addressless-publication inputs fail with fixed `locator-unresolved`; cancellation/deadline checkpoints return no partial result. All outputs are immutable canonical index-derived locators with closed content-free reasons, and resolution never searches prose, rendered pages, layout, metadata, paths, or URLs. Synthetic tests cover exact identity, stale progression removal, offset/anchor/spine/book-start recovery, deterministic repeatability/ties, wrong-book/malformed/empty failure, immutability, cancellation, and error canaries. Public opening/restoration, CFI, rendering, persistence, and progression calculation remain deferred.
...
**Status:** Complete. The public `@voxleaf/epub` root now exposes `openEpubPublication`, which accepts only in-memory bytes plus an optional abort signal and assembles the validated archive, package, navigation, BookV1, semantic-document, raster-catalog, and locator-index stages into one opened publication. Success is a frozen `{ ok: true, publication }` value with manifest-order semantic documents, detailed opaque navigation, lazy path-free raster descriptors, deterministic block start locators, structural exact/recovery resolution, and explicit close. Any stage failure closes retained archive state and returns a frozen `{ ok: false, detail, error }` value. All 13 closed EPUB details map exhaustively to the unchanged `OperationalErrorV1` families; unknown exceptions map to `internal-failure`, and neither exception messages/stacks/causes nor paths, URLs, markup, prose, bytes, or rejected values are retained or logged. Focused tests cover every mapping, decoder acceptance, deep failure immutability, canary redaction, invalid/cancelled value results, one complete synthetic public open, locator resolution, close, and absence of network/worker behavior. Task 6.2's deterministic reusable fixture builder and Task 6.3's complete valid/adversarial/boundary integration matrix remain deferred.
...
**Status:** Complete. Test-only support outside the production source/export tree now builds arbitrary deterministic in-memory ZIP entry sequences, a valid minimal EPUB with explicit entry omission/override controls, and one rich synthetic EPUB spanning multi-spine metadata, nested navigation, fallback and nonlinear spine cases, safe semantic structures, local links, and a raster resource. The writer fixes physical entry order, local DOS timestamps, creator/version fields, MS-DOS/external attributes, compression levels, filename encoding behavior, data-descriptor/ZIP64 choices, and worker/native-stream behavior. A documented mutation API clones inputs and supports ordered prepend, append, truncate, and stale-checked equal-length replacement operations; low-level entries can also provide encoded names and fixed extra fields. Repeated builds are byte-identical, all source arrays remain caller-owned, and invalid or undocumented mutations fail with fixed content-free test-support errors. The public-opener tests now reuse the minimal builder, while focused tests prove metadata determinism, rich public opening, malformed high-level inputs, unsafe low-level names, byte mutation behavior, and no network or worker use. The support uses only repository-authored synthetic content and in-memory zip.js primitives, adds no dependency or runtime export, and leaves Task 6.3's exhaustive scenario/boundary matrix deferred.
...
**Status:** Complete. A public integration/security matrix now drives repository-authored deterministic EPUB bytes through every ingestion stage. Nineteen failed-open scenarios cover every detail family reachable from untrusted input, including malformed archives, path and size limits, package/version/layout/metadata/spine failures, protected resources, invalid navigation, unsafe semantics, missing raster references, and caller cancellation. Each case proves exact detail-to-`OperationalErrorV1` mapping, frozen result shapes, no partial publication, deterministic archive cleanup, sensitive-canary redaction, no console output, and no network or worker use. Repeated rich opening proves stable shared-decoder-valid BookV1 output, immutable multi-spine semantic documents, nested navigation, path-free resources, deterministic locators, exact and structural recovery, lazy independent resource bytes, read-scoped cancellation, and idempotent closed lifecycle behavior. Malformed lazy resources and post-close operations return fixed content-free failures. An exhaustive compile-time map ties every `EpubIngestionPolicy` field to its owner-level exact/max+1 regression tests, and the missing content-document byte boundary proof was added. Exact boundary cases stay at injectable internal owners because the public opener intentionally exposes neither relaxed policy nor clock controls; this preserves the production API and avoids huge default-limit fixtures. No production code, dependency, export, generated artifact, or external capability was added. Milestone 6 is complete; Task 7.1 is next.
...
**Status:** Complete. The architecture overview now identifies the secure EPUB package stages as implemented while keeping desktop selection/rendering, persistence, narration preparation, TTS, buffering, and playback explicitly deferred. ADR-0007 records the completed dependency decision and public locator-resolution boundary instead of describing those outcomes as pending. The dependency inventory now describes the actual production ZIP/XML edges, exact versions, existing public root surface, licenses, transitive graph, test-fixture use, capability restrictions, and future desktop-bundle measurement gate. Setup distinguishes package-level ingestion from unavailable product integration, testing records the complete deterministic matrix without overstating end-to-end behavior, and the roadmap records implementation/documentation completion with Task 7.2 validation still outstanding. The audit changed no product requirement, source code, package manifest, lockfile, public contract, generated file, runtime capability, or dependency. `git diff --check` and `pnpm.cmd format:check` pass. Task 7.2 is next; this plan remains active until its focused, root, CI, privacy, and scope evidence is complete.
...
**Status:** Complete. From merged `main` at `c5db788`, the exact prescribed frozen install, focused EPUB/shared checks, individual root stages, and aggregate Windows foundation check all passed. EPUB reports 22 files and 360 tests; shared reports 18 files and 175 tests; desktop reports one test; Python reports three tests; and the Rust harness compiles and passes with no unit tests because it has no domain behavior. The native release executable plus Python source and wheel distributions build successfully. [Foundation checks run #89](https://github.com/mmjosedaniel/voxleaf/actions/runs/29952849807) passed both `Windows native foundation` and `Ubuntu portable foundation` for PR #39 head `65c4741`; that tested tree is byte-for-byte identical to squash merge `c5db788`. The complete Milestone 3 diff from required base `86db4c4` is confined to EPUB implementation/tests/test support, exact ZIP/XML dependencies and lock data, root package-build ordering, and evidence-backed documentation. It changes no desktop, TTS-service, shared-contract, workflow, native-permission, UI, renderer, persistence, narration, audio, or hardware implementation; adds no binary/publication/audio/model/log artifact; and imports no production filesystem, network, worker, DOM, React, Tauri, process, storage, or logging capability into `@voxleaf/epub`. Generated build outputs remain ignored and introduced no unexpected worktree change. All Milestone 3 acceptance criteria are satisfied, and this completed plan is moved to `docs/plans/completed/`.
...
- 2026-07-22: Completed Task 5.1 on `feat/m3-deterministic-anchors-locators` from merged `main` at PR #33 merge commit `841d68e`. Added a package-internal source-ID sidecar and immutable locator index with deterministic source-preorder assignment, shared-decoder-governed source-ID preservation, collision-free textless replacements, exact book/spine binding, Unicode code-point text bounds, legal offset construction, and shared-budget cancellation. Synthetic tests cover nested order, duplicate/invalid/conflicting/exact/max+1 IDs, generated collisions, repeated output, astral/combining text, image/line-break positions, offset limits, inconsistent spine projections, content-free errors, and unchanged path/text-free public exports.
- 2026-07-22: Completed Task 5.2 on `feat/m3-resolve-locators` from merged `main` at PR #34 merge commit `e184647`. Added package-internal exact and structural recovery resolution over the immutable locator index, shared-decoder input validation, full book/spine/anchor/offset identity checks, deterministic nearest offset/anchor/spine/book-start fallback, canonical immutable results with fixed reasons, and shared-budget cancellation. Synthetic tests cover exact and stale locators, wrong books, malformed input, empty spines/publications, tie ordering, progression removal, repeatability, content-free errors, and no prose/page/layout lookup.
- 2026-07-22: Completed Task 6.1 on `feat/m3-errors-fixtures-integration` from merged `main` at PR #35 merge commit `427d2bf`. Added the first public EPUB runtime opener, exhaustive closed detail-to-operational error mapping, failure redaction and cleanup, complete semantic/navigation/resource/locator assembly, and public deterministic locator access/resolution on the opened lifecycle handle. Focused and full EPUB tests cover all error families, unknown exception canaries, invalid/cancelled result values, one complete synthetic EPUB open, exact locator resolution, release, and no network/worker capability. Reusable deterministic fixtures and the full integration/security matrix remain Tasks 6.2 and 6.3.
- 2026-07-22: Completed Task 6.2 on `feat/m3-deterministic-epub-fixtures` from merged `main`. Added test-only deterministic arbitrary-ZIP, minimal-EPUB, comprehensive-EPUB, and documented byte-mutation support; migrated the public-opener tests to the reusable minimal fixture; and proved repeated byte identity, fixed ZIP metadata, safe malformed construction, rich public opening, caller-owned inputs, and no network/worker capability. The exhaustive valid/adversarial/max/max+1 integration matrix remains Task 6.3.
- 2026-07-22: Completed Task 6.3 on `test/m3-full-ingestion-matrix` from merged `main`. Added a deterministic public ingestion/security matrix spanning every ingestion stage and untrusted-input error family, rich repeatable success, shared contracts, lazy resources, exact/recovered locators, cancellation, cleanup, closure, redaction, and prohibited external capabilities. Added compile-time traceability for every policy maximum and the missing exact/max+1 content-document byte proof. Milestone 6 is complete; Task 7.1 is next.
- 2026-07-22: Completed Task 7.1 on `docs/m3-implemented-boundary` from merged `main` at PR #38 merge commit `ce49853`. Reconciled the architecture overview, ADR-0007, dependency inventory, setup status, testing strategy, and roadmap with the implemented public EPUB boundary and exact deferred desktop/product work. No production or contract behavior changed. Task 7.2 final validation is next.
- 2026-07-22: Completed Task 7.2 on `chore/m3-final-validation` from merged `main` at PR #39 squash merge `c5db788`. Passed every prescribed focused and root command, confirmed both jobs in Foundation checks run #89, reviewed the complete diff from Milestone 2 base `86db4c4`, found no privacy/scope/artifact/capability regression, updated closure evidence, and moved this plan from active to completed.
...
- Task 5.1 treats every final `SemanticBlock` as addressable and orders nested blocks by source-start preorder. Block quotes and lists receive start-only anchors before their descendants; heading and paragraph offsets count final semantic inlines by Unicode code point, with one position each for line breaks and raster images. Optional progression remains omitted because it is recovery/display metadata rather than position authority.
- Task 5.1 preserves an XHTML `id` or `xml:id` only when it maps uniquely across the source document and resulting semantic blocks and the existing shared v1 decoder accepts it. Conflicting IDs on one element, duplicates anywhere in the source, invalid grammar, and max-plus-one values fall back rather than rejecting safe content. Generated anchors use the fixed internal `voxleaf-s<spine-index>-a<anchor-index>` spelling and deterministic suffixes when a valid publisher ID reserves that value.
- Task 5.1 keeps raw source-ID candidates and locator constructors package-internal. The immutable index binds block references to exact-byte book identity, spine identity/order, anchor identity/order, and bounded offsets under the existing processing budget; it changes no shared schema or public EPUB runtime export. Task 5.2 owns exact/stale lookup and structural recovery.
- Task 5.2 revalidates unknown locator input at the package boundary and rebuilds every returned locator from the trusted index. Incoming optional progression is discarded because it is non-authoritative and this milestone does not calculate replacement progression.
- Task 5.2 recovers a matching spine by unique anchor value before anchor order, clamps the selected block's offset, and resets cross-spine or book-start recovery to offset zero. A matching empty spine uses absolute index distance with the earlier non-empty spine winning ties; inconsistent spine ID/index uses book start, while another book never resolves.
- Task 5.2 returns a closed internal discriminated result with `exact`, `nearest-offset`, `nearest-anchor`, `nearest-spine`, and `book-start` reasons. It performs no prose, quotation, CFI, page, layout, metadata, path, or URL search and adds no shared schema or public runtime export.
- Task 6.1 exposes only `openEpubPublication` as a runtime root export. It takes an in-memory `Uint8Array` and optional abort signal; callers cannot relax the immutable ingestion policy or inject a clock through the public boundary. The opener projects every supported local content document in manifest order, builds the locator index before publication, converts detailed navigation paths to opaque document IDs, and transfers archive ownership only after every stage succeeds.
- Task 6.1 returns expected open failures as frozen values containing exactly `ok`, a closed EPUB `detail`, and canonical `OperationalErrorV1`. The exhaustive mapping table is compile-time checked against the complete internal detail union; unknown exceptions become `internal-failure`, and opened archive state is released before failure return. The mapper never reads or retains exception message, stack, cause, raw value, publisher content, paths, or URLs and performs no logging.
- Task 6.1 exposes immutable locator entries and structural resolution on the existing opened-publication handle without changing the shared serialized locator schema. Each resolution receives a fresh default processing budget with optional operation cancellation, avoiding use of the elapsed ingestion deadline after a publication has been held open. Resource reads and close retain their Task 4.3 fixed content-free lifecycle behavior.
- Task 6.2 keeps fixture code under `packages/epub/test-support`, reachable only from the test TypeScript graph and absent from the package root and production build graph. The builder uses the already pinned in-memory zip.js writer with explicit deterministic ZIP metadata and no OS ZIP, filesystem, network, worker, transfer stream, native compression stream, ZIP64, or generated binary fixture.
- Task 6.2 provides high-level minimal and comprehensive synthetic EPUB presets plus a low-level ordered-entry API. States that zip.js cannot directly emit use described prepend, append, truncate, or equal-length replacement mutations; replacements must match expected source bytes before writing so dependency changes fail closed instead of silently patching another field.
- Task 6.3 treats the public matrix as integration evidence for representative failures at every stage, every untrusted-input result family, and all successful runtime operations. Exhaustive low-level malformed variants remain in their archive, path, budget, XML, package, navigation, semantics, resource, and locator owner suites instead of being duplicated through the public opener.
- Task 6.3 proves every policy maximum at its strict injectable owner boundary and keeps a compile-time-exhaustive policy-to-test map in the integration suite. The public opener remains limited to bytes and optional cancellation; no policy or clock test seam is exposed and no massive default-limit fixture is generated.
- Task 7.1 keeps ADR-0007 as the security/support authority while recording the completed low-level dependency selection and public publication lifecycle as implementation outcomes. Exact dependency inventory and bundle-impact evidence remain in development documentation rather than being duplicated into product requirements.
- Task 7.1 distinguishes a usable framework-independent `@voxleaf/epub` package from an EPUB reader application: desktop file access, rendering, resource decoding, persistence/restoration, narration normalization/chunking, TTS, audio, and end-to-end behavior remain later milestones and must not be inferred from package tests.
- Task 7.2 uses PR #39 Foundation checks run #89 as remote evidence because its tested head tree is identical to merged `main` at `c5db788`; the final closure commit changes only plan status/index locations and is covered by the same local root matrix.
- Task 7.2 audits the full milestone against PR #18 merge commit `86db4c4`, not merely the final documentation branch. The only new runtime dependencies are the accepted exact ZIP/XML primitives; no desktop, service, shared-contract, workflow, permission, UI, renderer, persistence, TTS, audio, or hardware area changed.
...
Task 5.1 validation passed on 2026-07-22 with `pnpm --filter @voxleaf/epub typecheck`, `pnpm --filter @voxleaf/epub test` (18 files and 314 tests), `pnpm --filter @voxleaf/epub build`, `pnpm --filter @voxleaf/shared test` (18 files and 175 tests, including generated-contract verification), `pnpm run typecheck:typescript`, `pnpm run format:check:typescript`, `pnpm run lint:typescript`, and `git diff --check`. Tests prove internal globally unique source-ID capture, stable spine/source-preorder assignment, shared-decoder acceptance, exact/max+1 anchor bounds, deterministic invalid/duplicate/conflicting/collision replacement, exact-byte and spine binding, Unicode code-point and structural-container offset limits, repeatability, cancellation, immutability, and content-free failures. The implementation adds no dependency, shared schema/generated-file change, public EPUB runtime export, filesystem/network/native permission, UI, rendering, TTS/audio, logging, or persistence behavior. Milestone-wide implementation and final validation have not run; Task 5.2 is next.

Task 5.2 validation passed on 2026-07-22 with `pnpm --filter @voxleaf/epub typecheck`, `pnpm --filter @voxleaf/epub test` (19 files and 324 tests), `pnpm --filter @voxleaf/epub build`, `pnpm run typecheck:typescript`, `pnpm run format:check:typescript`, `pnpm run lint:typescript`, and `git diff --check`. Tests prove full-tuple exact resolution, canonical progression-free output, nearest offset/anchor/spine/book-start recovery, deterministic anchor-value precedence and earlier-spine ties, wrong-book/malformed/addressless rejection, cancellation/deadline checkpoints, immutability, and content-free failures without prose/page/layout search. The implementation adds no dependency, shared schema/generated-file change, public EPUB runtime export, filesystem/network/native permission, UI, rendering, CFI, TTS/audio, logging, or persistence behavior. Milestone-wide implementation and final validation have not run; Task 6.1 is next.

Task 6.1 validation passed on 2026-07-22 with `pnpm --filter @voxleaf/epub typecheck`, `pnpm --filter @voxleaf/epub test` (20 files and 329 tests), `pnpm --filter @voxleaf/epub build`, `pnpm run typecheck:typescript`, `pnpm run format:check:typescript`, `pnpm run lint:typescript`, and `git diff --check`. Tests prove exhaustive closed EPUB-to-operational mapping, shared-decoder acceptance, unknown-exception and sensitive canary redaction, invalid/cancelled value results, post-open failure cleanup with no partial publication, one complete synthetic public open, immutable semantic/navigation/resource/locator output, exact locator resolution, explicit release, and no network/worker use. The implementation adds no dependency, shared schema/generated-file change, filesystem/native permission, UI, rendering, CFI, TTS/audio, logging, or persistence behavior. Milestone-wide implementation and final validation have not run; Task 6.2 is next.

Task 6.2 validation passed on 2026-07-22 with `pnpm.cmd --filter @voxleaf/epub typecheck`, `pnpm.cmd --filter @voxleaf/epub test` (21 files and 336 tests), `pnpm.cmd --filter @voxleaf/epub build`, `pnpm.cmd run typecheck:typescript`, `pnpm.cmd run format:check:typescript`, `pnpm.cmd run lint:typescript`, and `git diff --check`. Tests prove byte-identical repeated minimal and comprehensive builds, fixed entry order/timestamps/attributes/compression/version metadata, valid rich public opening, explicit omitted/malformed entries, deterministic unsafe encoded names and extra fields, ordered documented byte mutations, stale-patch rejection, caller-owned byte arrays, and no network/worker use. The implementation adds no dependency, production export/build input, generated binary, filesystem/native permission, UI, rendering, CFI, TTS/audio, logging, or persistence behavior. Milestone-wide implementation and final validation have not run; Task 6.3 is next.

Task 6.3 validation passed on 2026-07-22 with `pnpm.cmd --filter @voxleaf/epub typecheck`, `pnpm.cmd --filter @voxleaf/epub test` (22 files and 360 tests), `pnpm.cmd --filter @voxleaf/epub build`, `pnpm.cmd run typecheck:typescript`, `pnpm.cmd run format:check:typescript`, `pnpm.cmd run lint:typescript`, and `git diff --check`. Tests prove deterministic public success and failure across every ingestion stage and untrusted-input error family, immutable shared-decoder-valid outputs, lazy resource success/failure/cancellation, exact and structural locator resolution, idempotent closure, no partial publication, archive cleanup, fixed content-free failures, sensitive-canary redaction, every policy's owner-level exact/max+1 boundary, and no network, worker, filesystem, persistence, rendering, TTS, or audio capability. The implementation adds no dependency, production code/export/build input, shared schema/generated-file change, or native permission. Milestone 6 is complete; Task 7.1 is next.

Task 7.1 validation passed on 2026-07-22 with `pnpm.cmd format:check` and `git diff --check`. The documentation audit reconciles the implemented EPUB package stages, public opener/result/resource/locator lifecycle, exact ZIP/XML versions and dependency restrictions, test-only fixture ownership, deterministic scenario/boundary evidence, and unsupported/deferred reader behavior across architecture, ADR, dependency, setup, testing, roadmap, and plan documentation. The change adds no source code, package or lockfile edit, public contract, dependency, generated artifact, native permission, filesystem/network/worker capability, UI, renderer, persistence, TTS, or audio behavior. Milestone-wide focused, root, CI, privacy, and scope validation has not run; Task 7.2 is next.

Task 7.2 and Milestone 3 final validation passed on 2026-07-22. `git diff --check`, `pnpm.cmd install --frozen-lockfile`, `pnpm.cmd --filter @voxleaf/epub typecheck`, `pnpm.cmd --filter @voxleaf/epub test` (22 files, 360 tests), `pnpm.cmd --filter @voxleaf/epub build`, `pnpm.cmd --filter @voxleaf/shared test` (18 files, 175 tests, including 11 generated-contract checks), `pnpm.cmd format:check`, `pnpm.cmd lint`, `pnpm.cmd typecheck`, `pnpm.cmd test` (also one desktop test, three Python tests, and the zero-test Rust harness), `pnpm.cmd build`, and the aggregate `pnpm.cmd check` all passed on native Windows. The build produced the ignored native release executable and Python source/wheel distributions without changing the worktree. [Foundation checks run #89](https://github.com/mmjosedaniel/voxleaf/actions/runs/29952849807) completed successfully for both `Windows native foundation` and `Ubuntu portable foundation`; PR #39 head `65c474103f349e528c3fec41a2850ee9eb95f34d` and merged `main` tree `c5db7881e688dd49f8c532c02e59e182d9befcc9` are identical.

The final scope/privacy audit compared `86db4c4..c5db788`: 60 text files changed with no binary diff. Changes are limited to `packages/epub`, the exact `@zip.js/zip.js@2.8.30` and `saxes@6.0.0` manifest/lock edges, root package-build ordering, and Milestone 3 documentation. No application, TTS service, shared-contract, workflow, native capability/permission, renderer, UI, persistence, narration, TTS, audio, hardware, or later-milestone implementation changed. No EPUB/book file, generated audio, model weight, secret, private data, or log was added. Production EPUB sources contain no filesystem, network, worker, DOM, React, Tauri, process-spawn, browser-storage, or console-logging API. Expected failures remain content-free; fixture text and raster bytes are repository-authored, synthetic, test-only, in-memory inputs. Generated outputs are ignored, the final worktree contains no unexpected artifact, and Milestone 3 is complete.
