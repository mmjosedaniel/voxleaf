...
Task 3.1 semantic text rendering, Task 3.2 explicit navigation, and Task 3.3 bounded semantic images are implemented and validated. Tasks 1.1 through 1.6, 2.1 through 2.2, 2.4 through 2.5, and 3.1 through 3.3 are complete. Task 2.3 implementation and its checked-in packaged-startup/synthetic-open regression are complete, but the broader native reselection/cancellation/replacement matrix is still required before that task can close. Large-chapter enforcement, passive logical-position mapping, reflow/restoration, persistence, and later application tasks remain `Not started`.
