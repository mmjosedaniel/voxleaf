...
| Persistence and migration | Repository and save lifecycle implemented; restoration pending | ADR-0011's two bounded Web Storage envelopes, strict app-local display-preference adapter, exact-identity lookup, content-free failures, unsupported-version preservation, desktop-owned migration dispatch, 500 ms passive debounce, immediate settled saves, and lifecycle flushes are implemented by Tasks 4.4-4.5. Task 4.6 owns restoration integration. |
...
| Large-chapter policy and reader latency budgets | Implemented; native evidence remains | Task 3.6 implements the Task 1.6 policy with 250-block yielded batches, a 10,000-block/80,000-node ceiling, fixed no-partial-content fallback, and production React revalidation. Task 5.3 retains native WebView2 and restoration evidence. |
...
These are test-host implementation gates, not universal product or minimum-hardware claims. Task 3.6 records file-selection-to-first-content plus real React batch, target, append, reflow, and memory evidence on the reference Chromium host. Native WebView2 and locator-restoration measurements remain Task 5.3 work; that task may propose an explicit evidence-backed amendment rather than silently weakening this gate.
...
The broader native valid-open/reselection/cancellation/replacement interaction matrix remains the outstanding Task 2.3 evidence. Task 2.5 changed no shared schema semantics, EPUB validation behavior, native permission, network origin, renderer trust boundary, or private fixture.
...
**Status:** Complete on 2026-07-23. Added one application-owned `ReaderPositionSaveCoordinator` between the reader and Task 4.4 repository. It accepts only locators that resolve exactly back to themselves through the active publication, retains one latest canonical locator, uses an exact trailing 500 ms passive timer, promotes/coalesces explicit navigation and settled preference-reflow updates through an immediate timer, coalesces independently validated global preference writes, and serializes each latest-only write stream without an unbounded queue. Replacement, explicit close, rendering failure, app cleanup, hidden-document, and `pagehide` paths synchronously request a final write before publication cleanup continues; they do not await storage, use `beforeunload`, change focus, or delay navigation/closure. Closed coordinators reject stale-book callbacks, detach timers/listeners, and contain every fixed repository failure or unexpected rejection without changing the in-memory locator. App integration tests prove final-save invocation precedes replacement open and explicit close; component tests prove save intents occur only after explicit destination/reflow settlement; manual-clock tests cover 499/500 ms, supersession, no per-scroll write, immediate promotion/coalescing, lifecycle flush, serialized latest-only writes, stale/wrong-book rejection, preference independence, failure containment, and content-free records. Production Chromium validates both fixed envelopes and absence of filename/passage data. Task 4.6 still owns reading and restoring saved state.
...
**Status:** Complete on 2026-07-23. Added one publication-scoped `ReaderReflowRestorer` that captures and package-normalizes the current `ReadingLocatorV1` before an approved preference mutation or from the last settled active locator when window/visual-viewport geometry changes. It acquires Task 4.2's suspension token before React/browser reflow can publish transient geometry, coalesces rapid changes by revision, and cancels stale work before explicit navigation, root replacement, or close. A bounded animation-frame transaction waits for two stable frames with no fixed sleep, aligns Task 4.1's exact code-point range to the fixed 24-pixel reading line, falls back to the canonical block start when range geometry is unavailable, and stops after twelve frames when the target DOM is missing. Successful restoration seeds the visual tracker with the canonical locator and resumes without an immediate line-start resample, so changed wrapping cannot replace the preserved code-point; later user scroll/resize signals continue normal tracking. The mapper exposes only its registered application element for block-start geometry. No focus, storage, URL/history, publisher content, shared contract, dependency, Tauri capability, or native command was added. Deterministic tests cover exact/recovered alignment, focus isolation, rapid supersession, viewport capture, bounded missing targets, and lifecycle cancellation. Production Chromium proves one nonzero canonical code-point across all four preferences, rapid changes, viewport resize, and zoom with unchanged focus/URL, empty storage, zero page errors, and zero non-loopback requests.
...
**Mitigation:** Real-browser deterministic tests plus mandatory native Windows release smoke/restart matrix; do not treat Ubuntu/WSL as native acceptance. Task 2.5 checked in the content-free packaged-startup/synthetic-open regression, removed `unsafe-eval`, and placed it in Windows CI. Complete Task 2.3's broader native interaction matrix before closing that task.
...
| 2026-07-22 | Use packaged WebView `localStorage` behind a replaceable asynchronous desktop repository; keep at most 128 exact-book states and one global display-preference record in two fixed size-bounded v1 envelopes. | Accepted by ADR-0011 and implemented by Task 4.4; Task 4.5 integrates writes and Task 4.6 retains restoration. |
| 2026-07-22 | Preserve unsupported envelopes, migrate only through explicit validated atomic replacement, and never map state across exact-byte identities. | Accepted by ADR-0011; repository preservation/migration dispatch is implemented by Task 4.4, while restoration remains Task 4.6. |
...
Task 3.1 semantic text rendering, Task 3.2 explicit navigation, Task 3.3 bounded semantic images, Task 3.4 reader layout/preferences, Task 3.6 large-chapter enforcement, Task 4.1 semantic code-point/DOM range mapping, Task 4.2 passive normalized visual-position tracking, Task 4.3 reflow preservation, Task 4.4's versioned local repository, and Task 4.5's validated save lifecycle are implemented and validated. Tasks 1.1 through 1.6, 2.1 through 2.2, 2.4 through 2.5, 3.1 through 3.4, 3.6, and 4.1 through 4.5 are complete. Task 2.3 implementation and its checked-in packaged-startup/synthetic-open regression are complete, but the broader native reselection/cancellation/replacement matrix is still required before that task can close. Task 3.5 keyboard closeout, Task 4.6 restoration integration, and later application tasks remain `Not started`.
