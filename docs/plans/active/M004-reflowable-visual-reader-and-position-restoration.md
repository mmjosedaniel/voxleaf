...
**Status:** Complete on 2026-07-23. Added a focus-visible application-owned skip link that targets one generic reader article without exposing publisher identifiers or changing browser URL/history; converted chapter controls into a named navigation landmark; and kept all activation on native controls with no global key handling or positive `tabindex`. Explicit TOC/internal/chapter navigation continues to focus its heading or reader fallback, while passive preference changes and native scrolling keys retain current focus. Successful keyboard close now restores focus to the local EPUB picker after the reader unmounts. Focused tests cover the generic ID boundary, reader/chapter landmark names, passive preference focus retention, skip activation, and close restoration. Chromium and packaged WebView2 smokes cover native Tab/Enter/End behavior, destination focus, forced-colors focus visibility, unchanged URL, and zero external requests.
...
- 2026-07-23: Completed Task 3.5. Added an application-owned visible-on-focus skip link, a generic reader focus target, named chapter navigation, passive-focus preservation, and successful-close focus return to the local file picker. The reader keeps native Tab/Shift+Tab, button Enter/Space, scrolling keys, Page Up/Down, Home/End, and zoom unhandled; no global shortcut, browser route/history, publisher ID, or content-derived identifier was introduced. Focused desktop tests pass 12 files/109 tests. Production Chromium and packaged WebView2 both prove keyboard skip, tab ordering, uncancelled End, destination-heading focus, forced-colors focus, keyboard close return, unchanged URL, no errors, and zero external requests. Large-chapter scheduling, passive locator mapping/reflow restoration, persistence, narration, TTS, and audio remain deferred.
...
Task 3.1 semantic text rendering, Task 3.2 explicit navigation, Task 3.3 bounded semantic images, Task 3.4 reader layout/preferences, and Task 3.5 keyboard/focus behavior are implemented and validated. Tasks 1.1 through 1.6, 2.1 through 2.2, 2.4 through 2.5, and 3.1 through 3.5 are complete. Task 2.3 implementation and its checked-in packaged-startup/synthetic-open regression are complete, but the broader native reselection/cancellation/replacement matrix is still required before that task can close. Large-chapter enforcement, passive logical-position mapping, reflow restoration, persistence, and later application tasks remain `Not started`.

Task 3.5 validation completed on 2026-07-23:

- `pnpm.cmd --filter @voxleaf/desktop typecheck` passed.
- `pnpm.cmd --filter @voxleaf/desktop test` passed: 12 files and 109 tests.
- `pnpm.cmd test:browser` built shared/EPUB and passed the one pinned-Chromium production smoke in 3.8 seconds outside the managed sandbox. The sandboxed run exposed the existing Vite process-tree cleanup timeout; the unrestricted run exited normally. The smoke proves skip-link focus without URL change, logical native tab order and no positive `tabindex`, uncancelled End with retained reader focus, preference-reflow focus retention, keyboard TOC/internal/chapter destination focus, forced-colors focus visibility, keyboard close return to the picker, and zero external requests.
- `pnpm.cmd test:native-startup` passed after building the release executable. Packaged WebView2 opened the repository-authored synthetic EPUB, proved skip-link/End/TOC keyboard focus behavior and forced-colors focus visibility, returned focus to the picker on keyboard close, decoded the local image, emitted no page/console/runtime error, and made no external request.
- `pnpm.cmd run format:check:typescript`, `pnpm.cmd run lint:typescript`, and `git diff --check` passed after unrestricted workspace traversal; the managed sandbox cannot scan the existing `services/tts/.pytest_cache`.
- Final scope/privacy review found no dependency, lockfile, shared/EPUB public contract, Tauri command/plugin/capability/CSP, Web Storage implementation, network origin, publisher style/attribute/fragment, passive locator/DOM mapper, reflow-restoration implementation, narration, TTS, audio, private fixture, generated contract, or generated media change. The new reader DOM ID is fixed application-owned `voxleaf-reader-content`; it contains no publisher or position data.
