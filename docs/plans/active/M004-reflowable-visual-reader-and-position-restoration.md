...
Task 3.1 semantic text rendering and Task 3.2 explicit navigation are implemented and validated. Tasks 1.1 through 1.6, 2.1 through 2.2, 2.4 through 2.5, and 3.1 through 3.2 are complete. Task 2.3 implementation and its checked-in packaged-startup/synthetic-open regression are complete, but the broader native reselection/cancellation/replacement matrix is still required before that task can close. Semantic image integration, large-chapter enforcement, passive logical-position mapping, reflow/restoration, persistence, and later application tasks remain `Not started`.
