...
Task 3.1 semantic text rendering, Task 3.2 explicit navigation, Task 3.3 bounded semantic images, Task 3.4 reader layout/preferences, Task 3.6 large-chapter enforcement, and Task 4.1 semantic code-point/DOM range mapping are implemented and validated. Tasks 1.1 through 1.6, 2.1 through 2.2, 2.4 through 2.5, 3.1 through 3.4, 3.6, and 4.1 are complete. Task 2.3 implementation and its checked-in packaged-startup/synthetic-open regression are complete, but the broader native reselection/cancellation/replacement matrix is still required before that task can close. Task 3.5 keyboard closeout, passive reading-line tracking, reflow restoration, persistence, and later application tasks remain `Not started`.
