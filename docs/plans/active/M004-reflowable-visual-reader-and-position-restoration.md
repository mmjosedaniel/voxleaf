...
| Persistence and migration | Repository implemented; integration pending | ADR-0011's two bounded Web Storage envelopes, strict app-local display-preference adapter, exact-identity lookup, content-free failures, unsupported-version preservation, and desktop-owned migration dispatch are implemented by Task 4.4. Task 4.5 still owns the 500 ms/coalesced save lifecycle, and Task 4.6 owns restoration integration. |
...
Task 3.1 semantic text rendering, Task 3.2 explicit navigation, Task 3.3 bounded semantic images, Task 3.4 reader layout/preferences, Task 3.6 large-chapter enforcement, Task 4.1 semantic code-point/DOM range mapping, Task 4.2 passive normalized visual-position tracking, Task 4.3 reflow preservation, and Task 4.4's versioned local repository are implemented and validated. Tasks 1.1 through 1.6, 2.1 through 2.2, 2.4 through 2.5, 3.1 through 3.4, 3.6, and 4.1 through 4.4 are complete. Task 2.3 implementation and its checked-in packaged-startup/synthetic-open regression are complete, but the broader native reselection/cancellation/replacement matrix is still required before that task can close. Task 3.5 keyboard closeout, Task 4.5 save coordination, Task 4.6 restoration integration, and later application tasks remain `Not started`.
