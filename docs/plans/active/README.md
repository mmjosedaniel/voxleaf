...
- [`M007-local-tts-service-and-process-protocol.md`](M007-local-tts-service-and-process-protocol.md): active plan for the constrained one-GPU local service, versioned process protocol, native supervision, complete-unit audio delivery, cancellation containment, and exact-host validation. Milestones 1-5 are complete with model-free packaged evidence and measured exact Qwen/Serena handoff, cancellation, cleanup, and reload evidence; repository closeout remains open. Production and general-hardware support remain blocked.
