...
- The visual reader owns displayed text and the active logical locator. Tasks 2.1-3.3 now implement package-internal exhaustive narration source projection, Unicode-code-point source-span tokens, bounded canonical source windows/lifecycle, and deterministic source-mapped whitespace, line-break, hyphenation, punctuation, quotation, ellipsis, allowlisted Spanish symbol, abbreviation, and numeric-form normalization; complete invariants, segmentation, public preparation, TTS, audio, highlighting, and reader/audio synchronization are not implemented.

Important current limitations are:

- the bounded source-window coordinator and production normalizer slices consume immutable source-span tokens, but no public preparation coordinator or segmenter consumes normalized units yet;
...
- transformed, collapsed, removed, joined, expanded, raster, punctuation, and semantic-line-break positions now retain block-local origin spans as normalized text or omission units plus frozen content-free boundary protections, but prepared locator ranges do not exist before segmentation;
...
- The current safe semantic model and narration source projection expose headings, paragraphs, block quotes, and list boundaries but no scene-break leaf/event. Task 4.1 preserves every available source metadata field and does not widen the ingestion/public semantic contract; Task 4.2 must either recognize a scene-break-only admitted block under an explicit deterministic rule or record a plan/ADR amendment before claiming that behavior.
...
- Task 2.3 records only the canonical relation to source (`at-source-start`, `inside-source`, `before-next-source`, or `publication-end`). The final stable-segment relation cannot exist before Task 4 establishes stable segmentation; Task 5.1 must derive the accepted public relation structurally from those ranges without searching prose.
