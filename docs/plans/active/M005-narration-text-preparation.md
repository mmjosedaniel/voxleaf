...
- The visual reader owns displayed text and the active logical locator. Tasks 2.1-3.1 now implement package-internal exhaustive narration source projection, Unicode-code-point source-span tokens, bounded canonical source windows/lifecycle, and deterministic source-mapped whitespace, semantic-line-break, soft-hyphen, and conservative line-end-hyphenation normalization; later normalization rules, segmentation, public preparation, TTS, audio, highlighting, and reader/audio synchronization are not implemented.

Important current limitations are:

- the bounded source-window coordinator and first production normalizer slice consume immutable source-span tokens, but no public preparation coordinator or segmenter consumes normalized units yet;
- the deterministic neutral/Spanish normalization corpus and model-independent `narration-v1` limits profile are accepted test-only; Task 3.1 implements only whitespace, line-break, and hyphenation decisions while punctuation, symbol, abbreviation, numeric, date/time, currency, percentage, and complete language-composition rules remain pending;
- transformed, collapsed, removed, joined, raster, and semantic-line-break positions now retain block-local origin spans as normalized text or omission units, but prepared locator ranges do not exist before segmentation;
