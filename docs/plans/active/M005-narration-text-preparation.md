...
- The visual reader owns displayed text and the active logical locator. Task 2.1 now implements package-internal exhaustive narration source projection; source spans, normalization, segmentation, public preparation, TTS, audio, highlighting, and reader/audio synchronization are not implemented.

Important current limitations are:

- the production source projector traverses semantic EPUB content exhaustively, but no source-span token mapper, bounded request coordinator, normalizer, or segmenter consumes its internal events yet;
