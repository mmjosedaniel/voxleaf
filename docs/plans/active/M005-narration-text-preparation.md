...
- The visual reader owns displayed text and the active logical locator. Tasks 2.1-3.2 now implement package-internal exhaustive narration source projection, Unicode-code-point source-span tokens, bounded canonical source windows/lifecycle, and deterministic source-mapped whitespace, line-break, hyphenation, punctuation, quotation, ellipsis, and allowlisted Spanish symbol normalization; later Spanish lexical/numeric rules, segmentation, public preparation, TTS, audio, highlighting, and reader/audio synchronization are not implemented.
...
- the deterministic neutral/Spanish normalization corpus and model-independent `narration-v1` limits profile are accepted test-only; Tasks 3.1-3.2 implement whitespace, line-break, hyphenation, punctuation, quotation, ellipsis, malformed-preservation, and the closed Spanish ampersand/Celsius rules while abbreviation, numeric, date/time, currency, percentage, and complete language-composition rules remain pending;
