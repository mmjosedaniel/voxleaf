...
is complete. M007 is now in progress. Its completed first three milestones
...
and retains at most one bounded binary unit outside React state. The exact
one-GPU Qwen/Serena adapter, product narration caller, and real generated-audio
...
|-- Native persistent-child TTS supervisor [M007 Milestone 3; implemented model-free]
...
8. **In progress — Milestone 7:** The model-free Rust probe proves the selected parent/child frame boundary and a narrow binary Tauri response. Canonical shared control schemas and the bounded model-free Python service prove strict narration input, lifecycle, complete-unit audio framing, cancellation, and failure behavior. The production native shell now owns one persistent model-free Rust child, framed read/write bounds, state/timeouts, process-tree termination, zero automatic restart, application-exit cleanup, and narrow Tauri commands. The typed desktop client validates control order and identity, retains one binary unit outside React state, and zeroes released or stale bytes. The exact Qwen adapter, production narration caller, and real generated audio remain later M007 work.
