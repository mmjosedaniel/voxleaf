...
Mixed implementation status. The Milestone 3 secure EPUB ingestion boundary and framework-independent document model are implemented and validated. The desktop now has explicit EPUB/shared workspace dependencies and a UI-independent publication-session owner in addition to the validated capability-free local-file selection/read probe and bounded static-raster predecode/decode source boundary. The file UI is not connected to that session, so a user still cannot open or read a publication. Rendering and the ADR-0011-approved persistence repository remain planned implementation work; narration preparation, TTS integration, buffering, and playback also require their own implementation evidence. The Python area remains a foundation only.
...
The public EPUB package currently implements the in-memory validation, parsing, semantic projection, resource-descriptor, and locator portions of this flow. The desktop publication-session module now calls that public package boundary and owns cancellation, replacement, late-result rejection, and publication cleanup. The bounded local-file selection/read probe still releases its bytes instead of handing them to the session, and no presentation component consumes an opened publication. Saved-position restoration, visual rendering, narration preparation, synthesis, buffering, playback, and persistence remain planned; the system diagram marks the implemented lifecycle boundary and missing UI integration explicitly.
...
The Task 1.2 release probe passed in native Windows WebView2 while the Tauri shell retained zero commands, plugins, and capabilities and the then-current CSP. The current file probe intentionally releases successful bytes. ADR-0010 later added only the image-specific Blob allowance described below. Task 2.2 now provides the separate publication-session lifecycle and call to `openEpubPublication`; Task 2.3 still owns connecting successful file bytes to it and presenting safe open state. The file-ingress decision alone is not evidence that the desktop can open a book.
