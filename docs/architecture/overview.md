...
Mixed implementation status. The Milestone 3 secure EPUB ingestion boundary and framework-independent document model are implemented and validated. The desktop connects its capability-free local-file selection/read boundary to a UI-independent publication-session owner, presents one accessible idle/opening/ready/empty/failure/closing lifecycle surface, contains presentation failures, retains the bounded static-raster predecode/decode source boundary, and renders the starting spine document through an exhaustive application-owned semantic React renderer. A user can open, read the supported text structure at book start, and explicitly close a supported publication. Navigation, semantic image integration, large-chapter enforcement, logical-position ownership/restoration, and the ADR-0011-approved persistence repository remain planned; narration preparation, TTS integration, buffering, and playback also require their own implementation evidence. The Python area remains a foundation only.
...
├── Semantic React text renderer [implemented]
├── Reader navigation and logical-position coordinator
...
[ADR-0008](decisions/ADR-0008-visual-reader-architecture.md) accepts a direct application-DOM reader built from the closed `@voxleaf/epub` semantic model. The initial mode is continuous vertical scrolling over one active spine document. The implemented desktop navigation coordinator owns the active document/locator and explicit table-of-contents, internal-link, and previous/next navigation; later tasks extend it with passive visible-location updates, reflow capture/restoration, and persistence scheduling. Leaf presentation components do not own publication lifecycle or storage. Publisher HTML/CSS/scripts/URLs do not cross into the renderer, and no iframe or browser route/history integration is part of the initial reader.
...
- The approved Task 3.6 policy will render chapters incrementally in browser-yielding batches of at most 250 semantic blocks; more than 10,000 semantic blocks or 80,000 projected live DOM nodes will produce `chapter-too-large` before partial rendering.

`apps/desktop` contains the local-file open UI/coordinator, ADR-0010 raster metadata/source-lifecycle implementation, publication-session owner, immutable six-state reader lifecycle controller, fixed React presentation-error boundary, exhaustive semantic text renderer, explicit navigation coordinator/presentation, Playwright browser smoke, and a Windows-only synthetic benchmark harness. Ready state is the only state that exposes the active `OpenedPublication`; opening, empty, failure, closing, and unmount cleanup drop it before presentation. The semantic renderer uses React text nodes and application-owned elements/classes, structural numeric keys, inherited `lang`/`dir`, activatable package-resolved internal-link controls, and a fixed raster placeholder without copying publisher markup, attributes, styles, fragments, document/resource identities, or URLs into the DOM. The coordinator caches closed target outcomes, owns the canonical active locator/document, routes all explicit navigation through one commit path, and focuses the destination heading or reader region after successful user navigation. Package target/locator resolution, desktop publication lifecycle, safe metadata presentation, empty-state recovery, explicit close/reopen behavior, semantic text rendering, and explicit TOC/internal/chapter navigation are implemented. Image component integration, passive locator/DOM mapping, reflow/restoration, large-chapter enforcement, and ADR-0011 persistence modules require later tasks and tests. The reference thresholds must be revalidated against the real React/WebView2 reader; the test-only harness is not end-to-end reader evidence.
