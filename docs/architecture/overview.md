...
is complete. M007 is now in progress. Its completed first four milestones
...
stale suppression, and clean reload pass on the exact host. The product
narration caller, measured handoff matrix, playback, and distribution remain
unimplemented.
...
|-- Native persistent-child TTS supervisor [M007 Milestones 3-4; implemented]
...
|   [M007 Milestone 4; implemented and focused exact-host validated]
...
|   [M007 Milestones 1-4 implemented; measured handoff remains open]
...
The final sentence above describes the product data flow: focused M007
Milestone 4 exact-service synthesis now exists, but no desktop publication
caller sends prepared narration to it and no playback path consumes its audio.
...
8. **In progress — Milestone 7:** The model-free Rust probe proves the selected parent/child frame boundary and a narrow binary Tauri response. Canonical shared control schemas and the bounded Python service prove strict narration input, lifecycle, complete-unit audio framing, cancellation, and failure behavior. The production native shell owns one persistent child, framed read/write bounds, state/timeouts, process-tree termination, zero automatic restart, application-exit cleanup, and narrow Tauri commands. The typed desktop client validates control order and identity, retains one binary unit outside React state, and zeroes released or stale bytes. Native-only configuration now selects the implemented exact Qwen/Serena adapter, whose focused host diagnostic passes bounded delivery, termination, stale suppression, reload, and shutdown. The measured handoff matrix, production narration caller, and playback remain later work.
...
- The constrained demo's simultaneous playable-audio ceiling is approximately 30 minutes and must also be bounded by complete units, payload bytes, prepared text, and active work.
