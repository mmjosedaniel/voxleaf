...
[ADR-0008](decisions/ADR-0008-visual-reader-architecture.md) accepts a direct application-DOM reader built from the closed `@voxleaf/epub` semantic model. The implemented initial mode is continuous vertical scrolling over one active spine document. The desktop coordinator owns the active document/locator, explicit table-of-contents, internal-link and previous/next navigation, the closed current display preferences and pre-change preference-reflow intents, and package-normalized passive visible-location updates from the application reading line; later tasks extend it with reflow capture/restoration and persistence scheduling. Leaf presentation components do not own publication lifecycle or storage. Publisher HTML/CSS/scripts/URLs do not cross into the renderer, and no iframe or browser route/history integration is part of the initial reader.
...
`apps/desktop` contains the local-file open UI/coordinator, ADR-0010 raster metadata/source-lifecycle implementation, publication-scoped raster loader/component, publication-session owner, immutable six-state reader lifecycle controller, fixed React presentation-error boundary, exhaustive semantic renderer, bounded large-chapter policy/scheduler, explicit navigation coordinator/presentation, semantic DOM range mapper, active visual-locator tracker, closed reader-preference model/controls, Playwright browser smokes, and a Windows-only benchmark harness. Ready state is the only state that exposes the active `OpenedPublication`; opening, empty, failure, closing, and unmount cleanup drop it before presentation. The semantic renderer uses React text nodes and application-owned elements/classes, structural numeric keys, inherited `lang`/`dir`, activatable package-resolved internal-link controls, and bounded local `blob:` image sources without copying publisher markup, attributes, styles, fragments, document/resource identities, paths, or publisher URLs into the DOM. It preflights accepted recursive block/node limits, renders one immutable document through cancellable 250-block browser-yielded batches, retains completed content in normal DOM order, and presents a fixed no-partial-content fallback above either ceiling. Raster reads/decodes are lazy and serialized; the caller-owned byte copy is cleared after preparation, stale work is aborted, ready handles are released on unmount/replacement, and unavailable images remain fixed accessible placeholders without failing the chapter. The coordinator caches closed target outcomes, owns the canonical active locator/document and current validated display preferences, routes all explicit navigation through one commit path, retains the last valid locator when an attempted chapter exceeds the renderer ceiling, emits a frozen pre-change locator-bearing reflow intent for each preference change, focuses the destination heading or reader region after successful user navigation, and accepts same-spine passive visual-locator updates without changing navigation revision or destination focus state. The lifecycle-owned DOM mapper holds at most the currently rendered addressable-block registrations, uses sparse numeric checkpoints rather than prose copies, converts between Unicode code-point offsets and DOM UTF-16 boundaries, rejects surrogate-interior, noncollapsed, unrelated, detached, and stale ranges, and releases registrations on replacement, chapter change, unmount, clear, or close. The publication-scoped visual tracker associates those same rendered blocks without DOM attributes, maintains source-ordered leaf and structural indexes, coalesces scroll/resize/layout/render signals, applies exact-caret or block-start fallback semantics, normalizes through the package resolver, suppresses duplicate locators, and releases every callback, registration, index, and scheduled frame on replacement or close. A binary vertical probe measures only the nearby visible leaf range during ordinary samples. Explicit navigation holds a tokenized tracker suspension until the destination DOM commit completes; passive updates never move focus or write storage. The app retains the preference snapshot across publication close/reopen in memory only. Application-owned CSS maps closed tokens to bounded type scale, line spacing, content measure, and light/dark/system colors in one continuous responsive layout with narrow, zoom, reduced-motion, forced-color, and long-code handling. Package target/locator resolution, desktop publication lifecycle, safe metadata presentation, empty-state recovery, explicit close/reopen behavior, semantic text/image rendering, explicit TOC/internal/chapter navigation, non-persistent reader layout/preferences, large-chapter enforcement, semantic locator/DOM range mapping, and passive normalized visual-position tracking are implemented. Reflow restoration and ADR-0011 persistence modules require later tasks and tests. Task 3.6 remeasures the production React path on the reference Chromium host; Task 5.3 retains broader native WebView2 and end-to-end restoration ownership.
