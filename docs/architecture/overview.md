...
The accepted limits evidence publishes only measurements and content-free outcomes; an internal test helper compares independently rebuilt narration bytes and source ranges without returning them. It separately accounts for Unicode code points, UTF-8 bytes, sentences, protected tokens, parser lookahead, traversal depth, normalization expansion, work, retained intermediate state, and one lookahead segment. Cancellation is targeted every 512 work units and required by 1,024; deterministic yielding is targeted every 4,096 units and required by 8,192. Exact hard maxima pass, and max-plus-one observations produce one fixed content-free failure without a partial result. These are future production requirements, not claims that the preparation operation or scheduler exists.
