...
Mixed implementation status. The Milestone 3 secure EPUB ingestion boundary and framework-independent document model are implemented and validated. The desktop connects its capability-free local-file selection/read boundary to a UI-independent publication-session owner, presents one accessible idle/opening/ready/empty/failure/closing lifecycle surface, contains presentation failures, and renders the active spine document's supported text plus bounded static raster images through an exhaustive application-owned semantic React reader. A user can open, read, navigate, and explicitly close a supported publication. Large-chapter enforcement, passive logical-position ownership/restoration, and the ADR-0011-approved persistence repository remain planned; narration preparation, TTS integration, buffering, and playback also require their own implementation evidence. The Python area remains a foundation only.
...
`apps/desktop` contains the local-file open UI/coordinator, ADR-0010 raster metadata/source-lifecycle implementation, publication-scoped raster loader/component, publication-session owner, immutable six-state reader lifecycle controller, fixed React presentation-error boundary, exhaustive semantic renderer, explicit navigation coordinator/presentation, Playwright browser smoke, and a Windows-only synthetic benchmark harness. Ready state is the only state that exposes the active `OpenedPublication`; opening, empty, failure, closing, and unmount cleanup drop it before presentation. The semantic renderer uses React text nodes and application-owned elements/classes, structural numeric keys, inherited `lang`/`dir`, activatable package-resolved internal-link controls, and bounded local `blob:` image sources without copying publisher markup, attributes, styles, fragments, document/resource identities, paths, or publisher URLs into the DOM. Raster reads/decodes are lazy and serialized; the caller-owned byte copy is cleared after preparation, stale work is aborted, ready handles are released on unmount/replacement, and unavailable images remain fixed accessible placeholders without failing the chapter. The coordinator caches closed target outcomes, owns the canonical active locator/document, routes all explicit navigation through one commit path, and focuses the destination heading or reader region after successful user navigation. Package target/locator resolution, desktop publication lifecycle, safe metadata presentation, empty-state recovery, explicit close/reopen behavior, semantic text/image rendering, and explicit TOC/internal/chapter navigation are implemented. Passive locator/DOM mapping, reflow/restoration, large-chapter enforcement, and ADR-0011 persistence modules require later tasks and tests. The reference thresholds must be revalidated against the real React/WebView2 reader; the test-only harness is not end-to-end reader evidence.
