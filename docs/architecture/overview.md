...
The public `@voxleaf/epub` root currently exposes only the type-level semantic document and opened-publication boundary. Its closed block, inline, navigation, and raster-resource values contain no publisher HTML, DOM objects, paths, URLs, or eager resource bytes. A package-internal opened-publication handle now retains the validated archive, reads one supported local raster resource lazily under the shared byte and cancellation budgets, validates its declared media type against the byte signature, returns a fresh caller-owned allocation without caching, and releases archive state through idempotent close. The runtime package export remains empty until the later integration task can construct the complete validated publication; public EPUB opening and rendering do not work yet.
