...
Mixed implementation status. The Milestone 3 secure EPUB ingestion boundary and framework-independent document model are implemented and validated. The desktop connects its capability-free local-file selection/read boundary to a UI-independent publication-session owner, presents one accessible idle/opening/ready/empty/failure/closing lifecycle surface, contains presentation failures, and renders the active spine document's supported text plus bounded static raster images through an exhaustive application-owned semantic React reader. Its continuous responsive layout and closed in-memory display preferences are implemented without publisher styling or persistence. A user can open, read, navigate, adjust the visual reader, and explicitly close a supported publication with keyboard-operable controls, visible focus, and application-owned skip/focus behavior. Large-chapter enforcement, passive logical-position ownership/restoration, and the ADR-0011-approved persistence repository remain planned; narration preparation, TTS integration, buffering, and playback also require their own implementation evidence. The Python area remains a foundation only.
...
`apps/desktop` contains the local-file open UI/coordinator, ADR-0010 raster metadata/source-lifecycle implementation, publication-scoped raster loader/component, publication-session owner, immutable six-state reader lifecycle controller, fixed React presentation-error boundary, exhaustive semantic renderer, explicit navigation coordinator/presentation, closed reader-preference model/controls, Playwright browser smoke, and a Windows-only synthetic benchmark harness. Ready state is the only state that exposes the active `OpenedPublication`; opening, empty, failure, closing, and unmount cleanup drop it before presentation. The semantic renderer uses React text nodes and application-owned elements/classes, structural numeric keys, inherited `lang`/`dir`, activatable package-resolved internal-link controls, and bounded local `blob:` image sources without copying publisher markup, attributes, styles, fragments, document/resource identities, paths, or publisher URLs into the DOM. Raster reads/decodes are lazy and serialized; the caller-owned byte copy is cleared after preparation, stale work is aborted, ready handles are released on unmount/replacement, and unavailable images remain fixed accessible placeholders without failing the chapter. The coordinator caches closed target outcomes, owns the canonical active locator/document and current validated display preferences, routes all explicit navigation through one commit path, emits a frozen pre-change locator-bearing reflow intent for each preference change, and focuses the destination heading or reader region after successful user navigation. The app retains the preference snapshot across publication close/reopen in memory only. Application-owned CSS maps closed tokens to bounded type scale, line spacing, content measure, and light/dark/system colors in one continuous responsive layout with narrow, zoom, reduced-motion, forced-color, and long-code handling. Package target/locator resolution, desktop publication lifecycle, safe metadata presentation, empty-state recovery, explicit close/reopen behavior, semantic text/image rendering, explicit TOC/internal/chapter navigation, and non-persistent reader layout/preferences are implemented. Passive locator/DOM mapping, reflow restoration, large-chapter enforcement, and ADR-0011 persistence modules require later tasks and tests. The reference thresholds must be revalidated against the real React/WebView2 reader; the test-only harness is not end-to-end reader evidence.
