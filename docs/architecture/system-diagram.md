...
| Local TTS profiles and release distribution        | **Local profiles and verified Piper payload implemented** | M010.1 implements supported Chatterbox Spanish/English plus language-matched Piper CPU profiles; Qwen Serena/Aiden remain development-only. M011 Milestone 3 builds a private embedded Piper Spanish-English core with complete notices/source and native manifest verification. M011 Milestone 4A implements the native-owned optional Chatterbox lifecycle in a withheld state. Planned 4B uses six exact official Hugging Face model-data files plus a separately verified runtime; Qwen remains outside the first distributable product. |
...
| Release packaging                                  | **M1-3 and fail-closed M4A foundation complete**           | M011 release authority v1 freezes the Windows x64/private embedded Piper core and historical single-archive optional boundary. Milestones 2-3 close the production locks, audits, 400-component inventory, deterministic bilingual Piper payload, compliance bundle, measurements, and native verification. Milestone 4A adds the compiled withheld manifest, native lifecycle, and Settings controls. Milestone 4B must freeze additive official-source authority and implement model/runtime acquisition; installer integration, final optional evidence, signing, and complete-MVP release evidence remain planned. |
...
flowchart LR
    ...
    CHATTERBOX_RUNTIME_SOURCE["Reviewed Chatterbox runtime origin<br/>exact 79-package graph;<br/>M11 M4B authority pending"]:::planned
    PACKAGE["Windows x64 per-user MVP installer<br/>M11 M1 authority frozen;<br/>resource integration planned"]:::planned
    RELEASE_GRAPH["Release locks + audit + inventory<br/>M11 M2-M4A current;<br/>400 components + explicit blind spots"]:::implemented
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            ACQUISITION["Native optional-profile lifecycle<br/>M11 M4A fail-closed foundation implemented;<br/>M4B multi-artifact acquisition planned"]:::progress
        end
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            CHATTERBOX["Exact Chatterbox bilingual CUDA adapter<br/>Spanish/English language binding<br/>24-kHz complete unit; supported"]:::implemented
            ...
            CHATTERBOX_PACKAGE["Optional Chatterbox GPU profile<br/>M11 M4A manifest withheld;<br/>M4B official model files + reviewed runtime planned"]:::progress
    ...
    PROD_TTS -.->|"installer integration + clean-host gates"| PACKAGE
...
The M011 installer remains planned. Milestone 4A implements the fail-closed
optional lifecycle but keeps its network manifest withheld. Milestones 2-3
...
components plus explicit audit blind spots. The deterministic core now combines
the production service, private CPython/Piper runtime, two voices, notices,
model cards, and exact source archives. Its fixed manifest, standalone size,
bilingual offline smoke, and native verifier are implemented; these are not
final installer or clean-host evidence. Chatterbox's implemented local adapter
...
proof, and application-owned removal. Planned Milestone 4B splits that edge:
...
from a separately frozen immutable origin. Model repository code is never
executed, and a model-only transfer is not an installed profile. Selecting an
...
general Windows installer; a clearly labelled local portfolio build may be
...
| MVP packaging and release                               | [Active M011 ExecPlan](../plans/active/M011-package-validate-and-release-mvp.md), frozen historical [release authority v1](mvp-release-authority-v1.md), accepted [ADR-0042](decisions/ADR-0042-freeze-mvp-release-authority.md), [release security/distribution boundary](../development/release-security-and-distribution.md), exact [component inventory](../../services/tts/release/component-inventory-v1.json), and [Piper package evidence](../../services/tts/release/core/core-package-evidence-v1.json). Milestones 1-3 freeze the topology, close the dependency/audit boundary, and implement the verified bilingual Piper payload with compliance fulfillment. Milestone 4A adds the withheld optional Chatterbox lifecycle and source-package boundary. Planned 4B must freeze an additive authority and implement exact official Hugging Face model-file acquisition plus separately verified runtime delivery; enabled acquisition, installer integration, signing, and complete release evidence remain future work. |
...
7. **Milestone 11 — In progress:** Milestone 1 freezes the exact minimum
   ...
   Chatterbox lifecycle and source-package checker. Milestone 4B is planned to
   freeze and implement six exact official Hugging Face model-data downloads,
   separate reviewed runtime delivery, safe loading, and a final multi-artifact
   manifest. Normal-user package/cleanup, complete clean-host validation, and
