...
| TTS feasibility | **In progress** | The evaluation authority and bounded harness are frozen. One official Supertonic CPU run failed the first-audio and cancellation gates; Qwen CUDA measurement is blocked because the measured Windows WDDM host cannot expose mandatory process-attributed NVML VRAM. No profile, production dependency, or general hardware claim exists. |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            FEASIBILITY["TTS feasibility and profiling<br/>In progress: CPU failed gates;<br/>CUDA blocked on WDDM VRAM attribution"]:::progress
...
| Local TTS feasibility authority | [Milestone 6 active plan](../plans/active/M006-local-tts-feasibility-and-engine-profiles.md), [frozen feasibility profile](tts-feasibility-profile-v1.md); official CPU failure evidence and CUDA hardware-attribution blocker recorded, no selection yet |
...
1. **Milestone 6 — In progress:** The evaluation authority and candidate-neutral harness are implemented. The official Supertonic CPU profile failed first-audio and cancellation gates; the Qwen CUDA profile is blocked on mandatory process-attributed NVML VRAM under WDDM. Resolve the measured-hardware blocker, finish quality/audit evidence, and record an explicit viable-profile or no-viable-profile decision before integration.
