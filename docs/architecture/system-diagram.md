...
| Local TTS profiles and release distribution        | **Local/portfolio MVP GO; public signing pending**                                           | M010.1 implements supported Chatterbox Spanish/English plus language-matched Piper CPU profiles; Qwen Serena/Aiden remain development-only. M011 builds the verified Piper core and the currently withheld Chatterbox download lifecycle. Under ADR-0049 and release authority v2, Milestone 7 records Piper portfolio GO and conditional Chatterbox GO for systems that pass the published Windows/CUDA/VRAM/RAM/CPU gate. Representative evidence supports those requirements-defined classes without asserting identical behavior on every PC. The ordinary build still hides Chatterbox Download, and trusted public publication remains pending external signing authorization. Qwen remains outside the first distributable product. |
...
8. **Milestone 11 — Local/portfolio MVP GO; branch closeout pending:** Milestone 1 freezes the exact minimum
   ...
   `chatterbox-runtime-v2`; the ordinary manifest remains withheld as a
   separate distribution state. Milestone 5 implements the exact unsigned local Windows
   candidate and external-credential signing path; Milestone 6A implements
   lifecycle feedback and bounded uninstall choices. Milestone 7 consumes the
   completed Milestone 3.1 result and authority v2's requirements-based
   evidence rule. It records GO for the Piper local/portfolio core and GO for
   Chatterbox on systems that pass the published gate. Trusted public signing
   remains externally pending; M011 stays active only until required branch
   checks and the final closeout merge are recorded.
