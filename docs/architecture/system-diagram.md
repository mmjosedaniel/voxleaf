...
| Narration preparation | Approved but not started under [ADR-0012](decisions/ADR-0012-bounded-narration-preparation.md), [Roadmap Milestone 5](../plans/roadmap.md#milestone-5-prepare-text-for-natural-narration), and the [active Milestone 5 ExecPlan](../plans/active/M005-narration-text-preparation.md). The planned framework-independent `OpenedPublication.prepareNarration` transformation derives narration source text from immutable safe semantic leaf blocks, applies deterministic narration-only normalization and semantic segmentation, bounds each returned batch/segment, and keeps every prepared segment related to a stable half-open `LocatorRangeV1`. One preparation may run independently of one raster read; close cancels and awaits both. It must not mutate displayed text, persist narration, or introduce model-specific preprocessing. Task 1.2 adds the test-only [`narration-v1` normalization corpus](narration-normalization-v1.md); Task 1.3 adds the test-only [`narration-v1` limits profile](narration-preparation-limits-v1.md), deterministic content-free evidence, and exact/max-plus-one tests. Neither task adds a production narration module, public operation, or segmentation behavior. |
...
- Milestone 5: ADR-0012 accepts the narration-preparation ownership, public result, continuation, range, language, dependency, cancellation, and lifecycle boundary; Task 1.2 accepts the test-only deterministic normalization/Spanish corpus; and Task 1.3 accepts exact model-independent profile bounds. Source projection, narration-only normalization, semantic segmentation, bounded continuation/cancellation, and stable locator-range mapping remain unimplemented. Until production validation passes, the status remains approved planned.
