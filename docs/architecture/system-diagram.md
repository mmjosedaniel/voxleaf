...
| Boundary-aware audio transitions                   | **In progress; replacement CI remains**  | M008.1 reduces the completed prepared segment's semantic boundary to one bounded numeric delay and schedules at most one interruptible timer before an already-buffered successor. Hard/token splits remain continuous; real buffering replaces the delay; no silent PCM, model-input change, protocol field, or persistent data is added. Deterministic, portable, and authoritative Windows repository validation pass. The merged implementation's clean-runner smoke exposed a nondeterministic synchronization-probe race; its bounded correction and replacement required checks remain before archival.                                                                                          |
...
| Hardware profiles, fallback, and resilience        | **In progress; closeout CI remains**     | M010 Milestones 1-6 implement detection, matching, preference/recheck UI, identity-safe recovery, the passing Piper/davefx CPU-fallback decision, fourth executable Piper profile, exact isolated service adapter, profile-aware supervision/settings, Piper-only locator-safe spoken-expansion-aware preparation, nonspoken-unit omission, valid zero-sentence-boundary fragment scheduling, native configuration admission, and the resilience runner. Milestone 7 records the final matrix: Piper is the sole supported/recommendable compatible profile, Qwen/Serena stays development-only, automatic failover stays disabled, and distribution stays with M011. Local closeout validation and replacement required checks remain before archival. |
...
4. **Milestone 10 — Closeout validation:** Milestones 1-6 are complete. Milestone 7 records the final support matrix and ADR, exact safety margins, explicit fallback/recovery policy, runtime/license boundary, and limitations. Local closeout validation plus replacement required pull-request checks remain before archival.
