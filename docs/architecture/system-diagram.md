...
| Hardware profiles, fallback, and resilience        | **In progress; Milestone 6 complete**   | M010 Milestones 1-5 implement detection, matching, preference/recheck UI, identity-safe recovery, and the passing Piper/davefx CPU-fallback decision. Milestone 6 adds the fourth executable Piper profile, exact isolated service adapter, profile-aware supervision/settings, Piper-only locator-safe preparation, native configuration-admission gate, and resilience runner. Corrective Piper playback passes through both the long-paragraph and configured-runtime gates; Qwen passes offline service validation and exact fail-closed packaged VRAM compatibility. Milestone 7 remains. |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        ...
        subgraph PACKAGES["TypeScript packages"]
            ...
            PREP["@voxleaf/epub narration preparation<br/>narration-v1 + Piper-only narration-piper-v1<br/>bounded locator-linked batches"]:::implemented
...
product dispatch first applies the narrower `narration-piper-v1` limits,
preserving complete text and contiguous locator ranges while keeping normal
units within protocol v1's 20-second ceiling. Other profiles retain
...
| Hardware profiles, fallback, and operational resilience | [M010 active ExecPlan](../plans/active/M010-hardware-profiles-fallback-and-operational-resilience.md), frozen [hardware/profile/recovery authority v1](hardware-profile-recovery-authority-v1.md), accepted [ADR-0019](decisions/ADR-0019-privacy-safe-hardware-profiles-and-recovery.md), passing [Piper v6 result](../../benchmarks/tts/cpu-fallback-result-v6.json), accepted [selection-v6](../../benchmarks/tts/selection-v6.md), [ADR-0020](decisions/ADR-0020-admit-piper-cpu-fallback.md), frozen [Piper narration preparation profile v1](piper-narration-preparation-profile-v1.md), and frozen [runtime-configuration availability v1](tts-profile-runtime-configuration-availability-v1.md). Milestones 1-6 implement detection, matching, preference/recheck UI, explicit recovery, CPU fallback selection/integration, Piper-only locator-safe preparation, native configuration admission, and the passing corrective profile-specific resilience matrix. |
...
3. **Milestone 10 — In progress:** Milestones 1-6 freeze and implement the privacy-safe report, measured matching, compatibility/recheck UI, identity-safe recovery, passing Piper CPU-fallback decision/integration, Piper-only locator-safe preparation, native exact-runtime configuration admission, and profile-specific resilience matrix. Corrective long-paragraph and configured-runtime Piper playback pass; Qwen passes offline service validation and the exact packaged fail-closed VRAM path. Milestone 7 owns final support decisions and closeout.
