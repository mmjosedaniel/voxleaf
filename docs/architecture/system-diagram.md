...
| Local TTS profiles and release distribution        | **Local profiles implemented; distribution authority frozen** | M010.1 implements supported Chatterbox Spanish/English plus language-matched Piper CPU profiles; Qwen Serena/Aiden remain development-only. M011 Milestone 1 freezes a per-user Windows x64/private embedded Piper Spanish-English core and a separately gated optional Chatterbox GPU download with explicit consent, integrity verification, atomic installation, offline use, and removal. Qwen remains outside the first distributable product.                                     |
