...
| Reader experience stabilization                    | **Approved planned**                    | Active M009.1 will reconcile the later manual real-publication highlight discrepancy, establish one dedicated reader scroll owner, add a bounded locator-backed paragraph leaf, make narration compact/collapsible, and replace the preparation bar with exact loaded-duration text. No implementation is claimed yet.                                                      |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            STABILIZE["Reader experience stabilization<br/>highlight proof, reader viewport,<br/>leaf + compact narration<br/>M9.1 approved planned"]:::planned
...
The dashed M009.1 edges are approved presentation work, not runtime behavior.
They reuse the implemented range, locator, cancellation, and content-free
status boundaries to reconcile visible highlighting, establish one reader
scroll owner, expose a bounded paragraph leaf, and compact the narration
surface. They do not add a protocol field, change buffer policy, or replace
the exact segment text-range authority.
...
| Reader experience stabilization                       | [M009.1 active ExecPlan](../plans/active/M009-001-reader-experience-stabilization.md), frozen [reader-experience authority v1](reader-experience-authority-v1.md), and accepted [ADR-0018](decisions/ADR-0018-reader-experience-stabilization.md). Milestone 1 distinguishes accepted ranges from paint-aware perceivability and freezes one reader scroll owner, compact/collapsible narration, text-only loaded duration, and one bounded locator-backed leaf. Production UI behavior remains planned for Milestones 2-6. |
...
2. **Milestone 9.1 — In progress:** Milestone 1 freezes the [reader-experience authority v1](reader-experience-authority-v1.md) and paint-aware browser/packaged proof. Continue with Milestones 2-6 to restore production highlighting and implement the reader-first shell without changing completed synchronization, protocol, segmentation, or buffer authority.
