...
| Reader settings and playback controls              | **In progress; v3 comparison next**                             | M010.2 Milestones 1-2C are complete. Immutable v1 and frozen v2 comparisons both select no backend. ADR-0037 removes every experiment and retains `1.00x`. ADR-0039 freezes a separate boundary-deferred v3: the active unit keeps its rate, the pending rate applies to the next complete queued unit without TTS/queue invalidation, first activation is bounded to 1,000 ms p95, recurring handoff to 250 ms p95, additional process RAM to 200 MiB, and `1.00x` releases time-stretch ownership. Milestone 2D is next; runtime remains `1.00x`. |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            SETTINGS_SPEED["Reader-first Settings + playback<br/>M10.2 M1-M2 complete; no backend selected;<br/>runtime not implemented"]:::planned
        end
    ...
    SETTINGS_SPEED -.->|"blocked: no selected rate backend"| PLAYBACK
...
5. **Milestone 10.1 — Complete:** Milestones 1/1A-6 freeze and execute the
   ...
   merged the closeout.
6. **Milestone 10.2 — In progress; Milestones 1-2C complete:** v1 authority and
   ...
   boundary-deferred v3; ADR-0039 freezes its exact result-blind authority
   before Milestone 2D candidate work. No M010.2 runtime behavior is implemented
   yet.
