...
| TTS feasibility | **In progress** | The `v2` rerun authority and bounded harness use PID-tagged WDDM dedicated memory plus PyTorch allocator high-water measurement. The official Qwen matrix is complete and fails its balanced-role gates; the Supertonic rerun remains. No profile, production dependency, or general hardware claim exists. |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            FEASIBILITY["TTS feasibility and profiling<br/>In progress: Qwen v2 failed its role gates;<br/>Supertonic rerun pending"]:::progress
...
| Local TTS feasibility authority | [Milestone 6 active plan](../plans/active/M006-local-tts-feasibility-and-engine-profiles.md), [current v2 feasibility profile](tts-feasibility-profile-v2.md); Windows/PyTorch memory proof implemented, Qwen v2 matrix complete and failed, Supertonic rerun pending, no selection yet |
