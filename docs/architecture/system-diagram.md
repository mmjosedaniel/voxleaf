...
4. **Milestone 10 — In progress:** Milestones 1-6 are complete. Milestone 6 implements and exact-host validates the passing Piper CPU-fallback integration, Piper-only locator-safe spoken-expansion-aware preparation, nonspoken-unit omission, valid zero-sentence-boundary fragment scheduling, native exact-runtime configuration admission, profile-specific resilience matrix, and content-safe packaged private-book confirmation. Qwen passes offline service validation and the exact packaged fail-closed VRAM path. Milestone 7 owns final support decisions and closeout.
