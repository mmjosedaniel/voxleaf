...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            SYNC["Playback/reader synchronization<br/>authority + browser/native feasibility proof<br/>M9 Milestone 1; runtime pending"]:::progress
        end
...
user-facing narration path. M009 Milestone 1 freezes and proves the
segment-level decoration/follow approach, but no solid synchronization edge
exists because later milestones still own audible progress, product
highlighting/following, and shared reading position.
...
flowchart TD
    ...
    FOLLOW["Playback, segment highlighting, reader following,<br/>and heard-position persistence<br/>M9 M1 authority/proof; runtime pending"]:::progress
...
2. **Milestone 9 — In progress; Milestone 1 complete:** follow the [M009 ExecPlan](../plans/active/M009-synchronized-reading-and-narration.md) and [frozen authority](synchronization-authority-v1.md) to connect audible progress, highlighting, following, seek, and restoration to the implemented exact-demo path.
