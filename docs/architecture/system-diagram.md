...
- Milestone 5: ADR-0012 now accepts the narration-preparation ownership, public result, continuation, range, language, dependency, cancellation, and lifecycle boundary. Tasks 1.2-1.3 must still accept the deterministic corpus and exact profile bounds before source projection, narration-only normalization, semantic segmentation, bounded continuation/cancellation, and stable locator-range mapping are implemented. Until production validation passes, the status remains approved planned.
