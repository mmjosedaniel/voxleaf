...
| Short-unit and dual-worker feasibility                          | **In progress; v5 concurrency rejected on this host**  | Milestone 6.2's `v5` CPU-solo result passes bounded admission and its same-authority GPU baseline is valid but slower than real time. The hash-bound concurrent arm stopped at `resource-limit` before promotable result creation, so playback/quality is not admitted and no two-worker topology is selected.                                                                                                                         |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            BATCH_PROBE["Short-unit and dual-worker probe<br/>In progress: M6.2<br/>v5 concurrency resource-limited"]:::progress
    ...
    BATCH_PROBE -.->|"failed concurrency evidence; no topology"| TTS
...
flowchart TD
    ...
    BATCH_PROBE -.->|"failed concurrency evidence; no topology"| INFER
    INFER -.-> BUFFER
    BUFFER -.-> FOLLOW
    FOLLOW -.-> DEVICE
```

The current user-visible flow ends at `VISUAL`. `PREP` is usable by package callers and tests but is not wired into the desktop. `NEXT_PROFILE` is completed evidence. `BATCH_PROBE` has schema-valid `v4` safety-stop results and an accepted decision selecting neither placement. Its `v5` CPU-solo result passes bounded admission and its GPU-solo baseline is schema-valid, but the hash-bound concurrent arm stopped at `resource-limit` before a promotable result. The probe is not product runtime behavior or a selected production engine. All inference steps after it are future work. Approximately 15 seconds is a target amount of playable audio held in the future bounded buffer, not a fixed wall-clock delay; a shorter complete remaining range may start when fully ready. The `v5` five-minute value remains an unadmitted maximum-capacity hypothesis.
...
| Short-unit and dual-worker feasibility                 | [Milestone 6.2 active ExecPlan](../plans/active/M006-002-qwen-short-segment-batch-feasibility.md), [v4 authority](tts-feasibility-profile-v4.md), both stopped `v4` results, accepted [`selection-v4`](../../benchmarks/tts/selection-v4.md), frozen [v5 authority](tts-feasibility-profile-v5.md), schema-valid [`v5` CPU admission](../../benchmarks/tts/dual-worker-result-v5-cpu-solo.json), schema-valid [`v5` GPU baseline](../../benchmarks/tts/dual-worker-result-v5-gpu-solo.json), and the active-plan record of the concurrent `resource-limit` stop. No concurrent result, topology selection, or product runtime exists. |
...
1. **Milestone 6.2 — In progress:** Milestone 8 records passing CPU admission, a slower-than-real-time GPU baseline, and a concurrent `resource-limit` stop. Milestone 9 is not admitted. Milestone 10 must record the final decision without changing ADR-0014 meanwhile.
