...
versioned `0.1.0` current-user Windows x64 NSIS installer. Its unsigned-local
package evidence and receipt are regenerated after each installer rebuild;
...
flowchart TD
    ...
    INFER -->|"later product profiles admitted"| PROD
...
8. **Milestone 11 — Renewed Milestone 7 local closeout complete; required pull-request checks pending:** Milestone 1 freezes the exact minimum
