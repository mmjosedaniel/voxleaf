...
flowchart LR
  ...
  subgraph device["User device — local-first boundary"]
    ...
    readingStore["Local reading-state storage<br/>(technology undecided)"]:::planned
...
| --- | --- |
| Desktop shell | [`apps/desktop`](../../apps/desktop/) contains the React/Vite foundation UI, the Task 1.2 local-file probe, and the minimal Tauri shell. [`App.tsx`](../../apps/desktop/src/App.tsx) exposes the application-owned file input and fixed probe states; [`local-epub-file.ts`](../../apps/desktop/src/file-ingress/local-epub-file.ts) enforces the early byte bound and abortable browser read. [`main.rs`](../../apps/desktop/src-tauri/src/main.rs) still registers no commands, and [`tauri.conf.json`](../../apps/desktop/src-tauri/tauri.conf.json) still grants no capabilities. [ADR-0009](decisions/ADR-0009-capability-free-local-file-ingress.md) accepts this boundary; publication integration and reader behavior remain [Roadmap Milestone 4](../plans/roadmap.md#milestone-4-deliver-the-reflowable-visual-reader-and-position-restoration). |
...
| Reader, file boundary, coordinator, and persistence | [ADR-0009](decisions/ADR-0009-capability-free-local-file-ingress.md) and Task 1.2 implement and prove only the capability-free WebView selection/read boundary. [Roadmap Milestone 4](../plans/roadmap.md#milestone-4-deliver-the-reflowable-visual-reader-and-position-restoration), [ADR-0003](decisions/ADR-0003-stable-reading-locators.md), [ADR-0008](decisions/ADR-0008-visual-reader-architecture.md), and the [active Milestone 4 plan](../plans/active/M004-reflowable-visual-reader-and-position-restoration.md) approve the remaining application responsibilities. No desktop-to-EPUB integration, storage adapter, semantic target resolver, renderer, native file access, or application coordinator exists yet. File paths and native permissions remain outside the accepted boundary because `@voxleaf/epub` accepts bytes only. |
...
| Local device systems | Local EPUB selection is accepted through the capability-free WebView boundary and proven as a release probe, without retaining a path or file handle. Reading-state storage and audio-output APIs remain undecided and unimplemented. Normal reading must not require a remote service or persist generated audio. |

## Open decision gates

The roadmap deliberately leaves these matters unresolved until evidence exists:

- Milestone 4: raster decode limits/CSP, persistence technology, save lifecycle/migration, real-browser tooling, and measured large-chapter/reader performance limits. ADR-0008 resolved the visual-reader boundaries, and ADR-0009 resolved local file ingress without native permissions.
