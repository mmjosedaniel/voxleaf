...
The diagrams intentionally omit classes, functions, exhaustive imports, exact schemas, security-budget values, UI layout, deployment packaging, and implementation choices that have not passed their roadmap decision gate. The visual-reader boundary is selected by ADR-0008, the raster safety boundary by ADR-0010, the reader-state backend/ownership boundary by ADR-0011, and the narration-preparation ownership/API/lifecycle boundary by ADR-0012. The approved [Milestone 5 ExecPlan](../plans/active/M005-narration-text-preparation.md) authorizes implementation of that separate narration-preparation transformation. Tasks 1.2-1.3 accept the test-only deterministic neutral/Spanish corpus and exact model-independent resource profile; Tasks 2.1-2.3 implement the package-internal exhaustive source projector, Unicode-code-point source-span token mapper, bounded canonical source-window coordinator, and close-linked lifecycle; Task 3.1 implements the first source-mapped whitespace, semantic-line-break, and hyphenation normalizer slice. Later normalization rules, segmentation, public integration, and prepared ranges remain planned. The diagrams do not select a TTS engine, process transport, audio format, or playback API.
...
| Amber, heavy border and `[In progress]` label | **In progress:** production implementation has begun but applicable milestone validation is incomplete. Milestone 5 narration preparation currently has this status. |
  ...
  subgraph device["User device — local-first boundary"]
    ...
    subgraph libraries["Framework-independent TypeScript packages"]
      ...
      subgraph epubPackage["@voxleaf/epub"]
        ...
        narration["Narration source projection, deterministic normalization,<br/>semantic segmentation and bounded prepared segments<br/>[In progress: Tasks 2.1-3.1 source/lifecycle and first normalization slice]"]:::inprogress
        epubCore -->|Tasks 2.1-3.1 locator-aware source and normalization work| narration
      end
...
flowchart TB
  ...
  normalization["Deterministic narration-only normalization<br/>neutral plus explicit Spanish cases<br/>[In progress: M5 Task 3.1 implemented]"]:::inprogress
  ...
  semantics -->|Task 2.1 immutable located semantics| sourceSelection
  sourceSelection -->|Task 2.2 source units| sourceMapping
  contracts -->|locator endpoint validation| sourceMapping
  sourceMapping -->|Task 2.3 bounded token consumption| sourceWindow
  sourceWindow -.->|M5| normalization
...
Milestone 5 is in progress. Task 2.1 implements a pure package-internal projector that traverses every current safe semantic block/inline variant in locator order and emits frozen heading/paragraph leaves, text/line-break/raster-placeholder units, and quote/list/list-item boundary events without mutating displayed text or reading images. Task 2.2 maps every leaf source position to an immutable Unicode-code-point token with a legal half-open block-local span, validates full locator endpoints through package/shared rules, and retains no raster alternative text in token payloads. Task 2.3 structurally resolves exact/recovered starts, lazily emits bounded frozen source windows with canonical continuation, enforces deterministic checkpoint/yield/depth/retention limits, rejects concurrent narration operations, and makes publication close abort and await active narration independently of raster reads. Task 3.1 converts one block-local token stream into frozen source-mapped normalized text/omission units, collapsing explicit Unicode whitespace, preserving exact code, retaining semantic line breaks, removing accepted formatting/soft-hyphen positions, joining only the closed Spanish line-end case, and preserving genuine compounds and neutral ambiguity. Later normalization categories, segmentation, bounded sensitive prepared segments, stable prepared locator ranges, and the public operation remain pending. ADR-0012 accepts the package-owned `OpenedPublication.prepareNarration` operation, closed results, half-open ranges, containing-segment relation, one-active-operation policy, and close/cancellation behavior. Task 1.2 accepts exact test-only `narration-v1` neutral/Spanish corpus rules, and Task 1.3 accepts exact test-only chunk, batch, work, retention, checkpoint, and yield limits. TTS requests, inference, audio frames, scheduling, playback, highlighting, and visual/audio synchronization remain deferred to Milestones 6 through 9.
...
| Narration preparation | In progress under [ADR-0012](decisions/ADR-0012-bounded-narration-preparation.md), [Roadmap Milestone 5](../plans/roadmap.md#milestone-5-prepare-text-for-natural-narration), and the [active Milestone 5 ExecPlan](../plans/active/M005-narration-text-preparation.md). Task 2.1 adds [`narration-source-projector.ts`](../../packages/epub/src/narration/narration-source-projector.ts), a pure package-internal exhaustive traversal over immutable `PublicationLocatedBlock` values. Task 2.2 adds [`narration-source.ts`](../../packages/epub/src/narration/narration-source.ts), which emits immutable locator-valid Unicode-code-point source tokens. Task 2.3 adds [`narration-source-window.ts`](../../packages/epub/src/narration/narration-source-window.ts) and the package-internal production policy subset: canonical exact/recovered starts, bounded partial-leaf windows, monotonic continuation, deterministic checkpoint/yield work, content-free failures, one narration-operation slot, and close-linked cancellation independent of raster reads. Task 3.1 adds [`narration-normalizer.ts`](../../packages/epub/src/narration/narration-normalizer.ts) and [`spanish-normalization.ts`](../../packages/epub/src/narration/spanish-normalization.ts): a pure block-local normalized-unit stream with nonempty text/typed omissions, retained origin spans, explicit neutral/Spanish language selection, exact code preservation, and the closed whitespace/line-break/hyphenation policy. The planned public `OpenedPublication.prepareNarration` transformation still must complete deterministic narration-only normalization and semantic segmentation, prepared batches/segments, stable `LocatorRangeV1` values, and final result mapping. Task 1.2 supplies the test-only [`narration-v1` normalization corpus](narration-normalization-v1.md), and Task 1.3 supplies the complete [`narration-v1` limits profile](narration-preparation-limits-v1.md). No public operation, dependency, schema, desktop behavior, TTS, audio, persistence, or external capability was added. |
...
- Milestone 5: ADR-0012 accepts the narration-preparation ownership, public result, continuation, range, language, dependency, cancellation, and lifecycle boundary; Task 1.2 accepts the test-only deterministic normalization/Spanish corpus; Task 1.3 accepts exact model-independent profile bounds; Tasks 2.1-2.3 implement exhaustive package-internal source projection, locator-valid Unicode-code-point source-span tokens, bounded canonical source windows, deterministic continuation/cancellation, and publication-close ownership; and Task 3.1 implements the source-mapped whitespace, semantic-line-break, soft-hyphen, and conservative line-end-hyphenation slice. Remaining narration-only normalization, semantic segmentation, public result integration, and stable prepared locator-range mapping remain unimplemented. The milestone remains in progress until its full production validation passes.
