...
| Short-unit shared-model batch feasibility | **In progress; full-GPU arm stopped safely** | Milestone 6.2 Milestones 1 and 2 froze and implemented the development-only `v4` evaluation. Milestone 3's schema-valid full-GPU result stopped on the frozen zero-shared-memory rule before throughput or reviewable audio evidence. The separate targeted-CPU Milestone 4 contingency is admitted but not run; no selection or product runtime exists. |
...
The current user-visible flow ends at `VISUAL`. `PREP` is usable by package callers and tests but is not wired into the desktop. `NEXT_PROFILE` is completed evidence. `BATCH_PROBE` has a schema-valid full-GPU safety-stop result and an admitted but unexecuted targeted-CPU contingency; it is not product runtime behavior or a selected production engine. All inference steps after it are future work. Approximately 15 seconds is a target amount of playable audio held in the future bounded buffer, not a fixed wall-clock delay; a shorter complete remaining range may start when fully ready.
...
1. **Milestone 6.2 — In progress:** Milestone 3 completed the full-GPU safety path and reached the predeclared shared-memory stop. Execute the now-admitted targeted CPU placement in Milestone 4 before evaluating playback/quality or recording a decision.
