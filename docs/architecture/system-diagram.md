...
| Constrained local TTS service and process protocol       | **Approved planned**                    | M007 is the next implementation authority for one development-only GPU Qwen/Serena service, native supervision, a versioned local process boundary, complete bounded units, and identity-first worker termination under ADR-0015. No runtime implementation exists yet.                                                                               |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            TTS["Constrained local TTS service + protocol<br/>Approved plan: M7; not implemented"]:::planned
...
The diagram deliberately has no solid edge from the desktop session to narration preparation: the API is implemented and validated at the package boundary, but the production desktop does not call it. The Python node is also not a running process. M007 will evaluate and freeze the constrained-demo transport, framing, supervision, and exact engine boundary before implementation; this map does not claim that those choices or any runtime already exist.
...
flowchart TD
    ...
    BATCH_PROBE["Short-unit and dual-worker probe<br/>Complete: M6.2<br/>CPU + dual worker rejected"]:::implemented
    INFER["Constrained local inference + protocol<br/>Approved plan: M7; not implemented"]:::planned
...
The current user-visible flow ends at `VISUAL`. `PREP` is usable by package callers and tests but is not wired into the desktop. `NEXT_PROFILE` and `BATCH_PROBE` are completed development evidence. Accepted `selection-v5` rejects CPU-only and dual-worker scheduling and retains the exact GPU candidate only for ADR-0015's constrained demo. M007 is the approved next plan for the service/process boundary; it is not product runtime behavior or a selected production engine. Quick start uses approximately 15 playable seconds rather than a fixed wall-clock delay; an explicit prepared mode may target 1, 2, 5, or 10 playable minutes. The approximately 30-minute in-memory value is a simultaneous ceiling, not a startup target or uninterrupted-playback guarantee.
...
| Constrained local TTS service and process protocol     | [M007 active ExecPlan](../plans/active/M007-local-tts-service-and-process-protocol.md); approved plan only until its implementation, ADR, deterministic/native validation, and exact-host evidence are complete                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
1. **Milestone 7 — Approved plan; implementation not started; production blocked:** follow M007 to freeze and implement one bounded GPU Qwen/Serena development-demo service and typed local process boundary under ADR-0015. Do not claim passing performance, continuous playback, native model streaming, general hardware support, or production distribution.
