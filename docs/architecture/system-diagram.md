...
| Reader settings and playback controls              | **In progress; v3 authority next**                              | M010.2 Milestones 1-2B are complete. Immutable v1 and frozen v2 comparisons both select no backend. ADR-0037 removes every experiment and retains `1.00x`. ADR-0038 authorizes a separate boundary-deferred v3: the active unit keeps its rate, the pending rate applies to the next complete queued unit without TTS/queue invalidation, first activation is bounded to 1,000 ms p95, additional process RAM to 200 MiB, and `1.00x` bypasses time stretching. Milestone 2C must freeze v3 before implementation or measurement. |
...
continue unchanged. Milestone 2C must freeze the executable v3 before any new
candidate work. No CSP, dependency, or runtime rate edge exists.
...
5. **Milestone 10.1 — Complete:** Milestones 1/1A-6 freeze and execute the
   ...
   merged the closeout.
6. **Milestone 10.2 — In progress; Milestones 1-2B complete:** v1 authority and
   ...
   boundary-deferred v3; Milestone 2C must freeze it before candidate work. No
