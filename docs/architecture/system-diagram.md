...
| Hardware profiles, fallback, and resilience        | **In progress; Milestone 6 confirmation pending** | M010 Milestones 1-5 implement detection, matching, preference/recheck UI, identity-safe recovery, and the passing Piper/davefx CPU-fallback decision. Milestone 6 adds the fourth executable Piper profile, exact isolated service adapter, profile-aware supervision/settings, Piper-only locator-safe spoken-expansion-aware preparation, native configuration-admission gate, and resilience runner. Corrective Piper playback passes the ordinary-prose, compact-form, and configured-runtime gates; its private-book confirmation remains pending. Qwen passes offline service validation and exact fail-closed packaged VRAM compatibility. Milestone 7 remains. |
...
3. **Milestone 10 — In progress:** Milestones 1-5 are complete. Milestone 6 implements and exact-host validates the privacy-safe report, measured matching, compatibility/recheck UI, identity-safe recovery, passing Piper CPU-fallback decision/integration, Piper-only locator-safe spoken-expansion-aware preparation, native exact-runtime configuration admission, and profile-specific resilience matrix. Corrective ordinary-prose, compact-form, and configured-runtime Piper playback pass; private-book confirmation remains pending. Qwen passes offline service validation and the exact packaged fail-closed VRAM path. Milestone 7 owns final support decisions and closeout.
