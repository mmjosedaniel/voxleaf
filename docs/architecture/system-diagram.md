  ...
  subgraph device["User device — local-first boundary"]
    ...
    subgraph libraries["Framework-independent TypeScript packages"]
      ...
      subgraph epubPackage["@voxleaf/epub"]
        ...
        narration["Narration source projection, deterministic normalization,<br/>semantic segmentation and bounded prepared segments<br/>[In progress: Tasks 2.1-4.1 through boundary scanning]"]:::inprogress
        epubCore -->|Tasks 2.1-4.1 locator-aware preparation work| narration
      end
...
flowchart TB
  ...
  boundaryScan["Sentence/dialogue/clause/protected-token boundaries<br/>[Implemented: M5 Task 4.1]"]:::implemented
  segmentation["Bounded semantic-unit packing<br/>[Approved planned: M5 Tasks 4.2-4.3]"]:::planned
  ...
  normalization -->|Task 4.1 source-mapped units| boundaryScan
  boundaryScan -.->|M5| segmentation
...
| Narration preparation | In progress under [ADR-0012](decisions/ADR-0012-bounded-narration-preparation.md), [Roadmap Milestone 5](../plans/roadmap.md#milestone-5-prepare-text-for-natural-narration), and the [active Milestone 5 ExecPlan](../plans/active/M005-narration-text-preparation.md). Tasks 2.1-2.3 implement exhaustive source projection, locator-valid Unicode-code-point tokens, bounded canonical source windows, deterministic continuation/cancellation, and close ownership. Tasks 3.1-3.4 implement and validate the pure source-mapped neutral/Spanish normalizer in [`narration-normalizer.ts`](../../packages/epub/src/narration/narration-normalizer.ts) and [`spanish-normalization.ts`](../../packages/epub/src/narration/spanish-normalization.ts). Task 4.1 adds [`narration-boundary-scanner.ts`](../../packages/epub/src/narration/narration-boundary-scanner.ts), which emits immutable source-offset sentence/dialogue-turn/clause boundaries and non-splittable protected-token spans through exactly two normalized-unit visits while enforcing the 256-code-point token ceiling. Semantic-unit packing, prepared batches/segments, stable `LocatorRangeV1` values, and final public result mapping remain planned. Task 1.2 supplies the test-only [`narration-v1` normalization corpus](narration-normalization-v1.md), and Task 1.3 supplies the complete [`narration-v1` limits profile](narration-preparation-limits-v1.md). No public operation, dependency, schema, desktop behavior, TTS, audio, persistence, or external capability was added. |
...
- Milestone 5: ADR-0012 accepts the narration-preparation ownership, public result, continuation, range, language, dependency, cancellation, and lifecycle boundary; Task 1.2 accepts the test-only deterministic normalization/Spanish corpus; Task 1.3 accepts exact model-independent profile bounds; Tasks 2.1-2.3 implement exhaustive package-internal source projection, locator-valid Unicode-code-point source-span tokens, bounded canonical source windows, deterministic continuation/cancellation, and publication-close ownership; Tasks 3.1-3.4 implement and validate source-mapped neutral/Spanish normalization; and Task 4.1 implements deterministic source-offset sentence/dialogue/clause/protected-token scanning. Semantic-unit packing, public result integration, and stable prepared locator-range mapping remain unimplemented. The milestone remains in progress until its full production validation passes.
