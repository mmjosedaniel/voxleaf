  ...
  subgraph device["User device — local-first boundary"]
    ...
    subgraph libraries["Framework-independent TypeScript packages"]
      ...
      subgraph epubPackage["@voxleaf/epub"]
        ...
        narration["Narration source projection, deterministic normalization,<br/>semantic segmentation and bounded prepared segments<br/>[In progress: Tasks 2.1-2.2 source mapping implemented]"]:::inprogress
        epubCore -->|Tasks 2.1-2.2 semantic and locator source mapping| narration
      end
...
flowchart TB
  ...
  sourceMapping -.->|M5| normalization
...
| Narration preparation | In progress under [ADR-0012](decisions/ADR-0012-bounded-narration-preparation.md), [Roadmap Milestone 5](../plans/roadmap.md#milestone-5-prepare-text-for-natural-narration), and the [active Milestone 5 ExecPlan](../plans/active/M005-narration-text-preparation.md). Task 2.1 adds [`narration-source-projector.ts`](../../packages/epub/src/narration/narration-source-projector.ts), a pure package-internal exhaustive traversal over immutable `PublicationLocatedBlock` values. It emits frozen located leaves, exact source-position units, and structural boundaries; preserves semantic context; verifies code-point totals against the locator index; and excludes raster alternative text/image reads. Task 2.2 adds [`narration-source.ts`](../../packages/epub/src/narration/narration-source.ts), which emits one immutable source token per Unicode code point or semantic placeholder, retains compact half-open block-local spans, and constructs locator ranges through existing package/shared validation. The planned framework-independent `OpenedPublication.prepareNarration` transformation still must add deterministic narration-only normalization and semantic segmentation, bounded batches/segments, stable prepared `LocatorRangeV1` values, cancellation, and close ownership. Task 1.2 supplies the test-only [`narration-v1` normalization corpus](narration-normalization-v1.md), and Task 1.3 supplies the test-only [`narration-v1` limits profile](narration-preparation-limits-v1.md). No public operation, dependency, schema, desktop behavior, TTS, audio, persistence, or external capability was added. |
...
- Milestone 5: ADR-0012 accepts the narration-preparation ownership, public result, continuation, range, language, dependency, cancellation, and lifecycle boundary; Task 1.2 accepts the test-only deterministic normalization/Spanish corpus; Task 1.3 accepts exact model-independent profile bounds; and Tasks 2.1-2.2 implement exhaustive package-internal source projection plus locator-valid Unicode-code-point source-span tokens. Narration-only normalization, semantic segmentation, bounded continuation/cancellation, public lifecycle integration, and stable prepared locator-range mapping remain unimplemented. The milestone remains in progress until its full production validation passes.
