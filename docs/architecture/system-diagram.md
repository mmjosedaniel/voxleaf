...
3. **Milestone 10 — In progress:** Milestones 1-6 freeze and implement the privacy-safe report, measured matching, compatibility/recheck UI, identity-safe recovery, passing Piper CPU-fallback decision/integration, and profile-specific resilience matrix. Piper passes full packaged playback; Qwen passes offline service validation and the exact packaged fail-closed VRAM path. Milestone 7 owns final support decisions and closeout.
