...
flowchart LR
  ...
  reader -. planned save and restore coordination .-> stateRepository
...
The capability-free file-open flow now passes one bounded in-memory read to the UI-independent publication-session owner and presents only validated title/authors or fixed safe outcomes. The session provides cancellation, replacement, stale-result rejection, and publication cleanup; the desktop shell adds an immutable idle/opening/ready/empty/failure/closing surface, explicit close/reopen, fixed presentation-error containment, and one continuous responsive reader with closed in-memory display preferences. The reader consumes the bounded static-raster boundary through lazy serialized resource reads, accessible per-image fallback, and lifecycle-owned Blob URL release; the Tauri capability list remains empty. The implemented ADR-0011 repository isolates two fixed bounded `localStorage` keys behind asynchronous validated operations, but reader preference writes, position-save scheduling, and restoration integration remain Tasks 4.5-4.6. No remote runtime service or external network dependency is approved for normal reading; the exact local desktop-to-TTS transport remains undecided.
...
| Local device systems | Local EPUB selection is accepted through the capability-free WebView boundary and proven as a release probe, without retaining a path or file handle. ADR-0011's bounded WebView `localStorage` repository is implemented with no native command, plugin, capability, or path contract; application save/restoration integration remains pending. Audio-output APIs remain undecided and unimplemented. Normal reading must not require a remote service or persist generated audio. |
