...
| Adaptive audio scheduling and playback                   | **In progress**                         | M008 Milestone 1 freezes the exact authority and Milestone 2 implements an unconnected model-free scheduler state machine with deterministic complete-unit, backpressure, ownership, invalidation, depletion, and service-recovery traces. No payload-owning production queue, player, caller, or UI exists.                                                             |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            PLAYBACK["Adaptive scheduler + audio queue/player<br/>M8 scheduler proof implemented;<br/>payload runtime not implemented"]:::progress
...
flowchart TD
    ...
    BUFFER["Bounded adaptive scheduler + audio queue<br/>authority/traces implemented<br/>payload runtime not implemented"]:::progress
...
The current user-visible flow ends at `VISUAL`. `PREP` is usable by package callers and tests but is not wired into the desktop. `NEXT_PROFILE` and `BATCH_PROBE` are completed development evidence. Accepted `selection-v5` rejects CPU-only and dual-worker scheduling and retains the exact GPU candidate only for ADR-0015's constrained demo. Completed M007 provides the model-free transport, protocol authority, canonical contracts, Python service, native supervisor, typed client, one-unit handoff, exact development-only Qwen/Serena adapter, measured exact-host handoff/cancellation/cleanup matrix, and repository decision audit. M008 Milestone 1 freezes the model-independent buffer and UX authority; Milestone 2 implements its unconnected metadata-only scheduler state machine and deterministic traces. Audible product runtime still does not exist, and no production engine is selected. The 30-minute in-memory value is a simultaneous ceiling, not a startup target or uninterrupted-playback guarantee.
...
| Playable-audio startup and adaptive buffering policy   | [ADR-0004](decisions/ADR-0004-start-after-audio-lead.md), [ADR-0015](decisions/ADR-0015-bounded-adaptive-qwen-demo-buffering.md), frozen [adaptive buffer v1 authority](adaptive-buffer-authority-v1.md), and [M008](../plans/active/M008-bounded-adaptive-prebuffering.md). Milestones 1-2 provide authority, arithmetic, and model-free scheduler-trace evidence; product-connected payload runtime remains unimplemented.                                                                                                                                                                                                                                                            |
...
1. **Milestone 8 — In progress:** Milestones 1-2 freeze the adaptive-buffer authority and prove the model-free scheduler traces. Continue M008 with the product narration caller, bounded payload-owning multi-unit buffer, player, quick/prepared UI, playback-only pause continuation, adaptive boundary waits, and truthful frontier buffering. Consume M007 protocol v1 without adding a service-side queue or automatic retry.
