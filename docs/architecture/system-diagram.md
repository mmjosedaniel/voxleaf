...
| TTS profile blocker resolution | **In progress; selection complete** | The exact Serena `v3` matrix passed cold load, RAM, VRAM, offline, artifact, license, packaging, and cleanup but failed startup, throughput, zero-failure, and mid-generation cancellation. `selection-v3` retains the standard blocker while ADR-0014 permits the exact profile only for a bounded development demo. Repository/CI closeout remains; no passing standard or general hardware profile exists. |
...
1. **Milestone 6.1 — In progress:** Milestone 5 selection is complete; run repository/privacy validation and required CI, then close the plan without rewriting the frozen authority.
