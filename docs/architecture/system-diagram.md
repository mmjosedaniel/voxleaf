...
| Tauri native shell                                       | **In progress**                         | M007 Milestone 1 adds one narrow model-free Rust-owned child/std-stream protocol probe and optimized binary-response command. No plugin or capability is granted; packaged WebView acceptance remains pending.                                                                                                                                        |
...
| Constrained local TTS service and process protocol       | **In progress**                         | M007 Milestone 1 has deterministic and release parent/child model-free prototype evidence plus frozen protocol-v1 limits. The Python service, canonical protocol schemas, supervisor, Qwen adapter, and accepted packaged WebView path remain unimplemented or unaccepted.                                                                            |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            TTS["Constrained local TTS service + protocol<br/>M7 in progress: model-free native probe only<br/>service/model not implemented"]:::progress
...
The diagram deliberately has no solid edge from the desktop session to narration preparation: the API is implemented and validated at the package boundary, but the production desktop does not call it. The Python node is also not a running process. M007 Milestone 1 now contains a model-free Rust-owned synthetic child/std-stream and optimized binary-response probe. It is not a Python service, supervisor, Qwen adapter, production runtime, or accepted packaged WebView boundary; ADR-0016 remains proposed until the native smoke reaches the application.
...
flowchart TD
    ...
    BATCH_PROBE["Short-unit and dual-worker probe<br/>Complete: M6.2<br/>CPU + dual worker rejected"]:::implemented
    INFER["Constrained local inference + protocol<br/>M7 in progress: native model-free probe only<br/>service/model not implemented"]:::progress
...
The current user-visible flow ends at `VISUAL`. `PREP` is usable by package callers and tests but is not wired into the desktop. `NEXT_PROFILE` and `BATCH_PROBE` are completed development evidence. Accepted `selection-v5` rejects CPU-only and dual-worker scheduling and retains the exact GPU candidate only for ADR-0015's constrained demo. M007 is in progress with a model-free native transport probe and frozen provisional protocol authority; it is not a Python service, Qwen integration, audible product runtime, or selected production engine. Quick start uses approximately 15 playable seconds rather than a fixed wall-clock delay; an explicit prepared mode may target 1, 2, 5, or 10 playable minutes. The approximately 30-minute in-memory value is a simultaneous ceiling, not a startup target or uninterrupted-playback guarantee.
...
| Constrained local TTS service and process protocol     | [M007 active ExecPlan](../plans/active/M007-local-tts-service-and-process-protocol.md), frozen [protocol v1 authority](tts-service-protocol-v1.md), and proposed [ADR-0016](decisions/ADR-0016-rust-owned-stdio-tts-protocol.md). Milestone 1 has deterministic and release parent/child evidence; packaged WebView acceptance and later service/model work remain open.                                                                                                                                                                                                                                                                                                                |
...
1. **Milestone 7 — In progress; production blocked:** complete M007 Milestone 1's packaged WebView gate, then implement the canonical protocol, service, native supervisor, and one bounded GPU Qwen/Serena development-demo adapter under ADR-0015. Do not claim passing performance, continuous playback, native model streaming, general hardware support, or production distribution.
