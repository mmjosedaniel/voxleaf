...
| Local TTS profiles and release distribution        | **Local profiles and verified Piper payload implemented** | M010.1 implements supported Chatterbox Spanish/English plus language-matched Piper CPU profiles; Qwen Serena/Aiden remain development-only. M011 Milestone 3 builds a private embedded Piper Spanish-English core with complete notices/source and native manifest verification. M011 Milestone 4 implements the native-owned optional Chatterbox lifecycle but deliberately withholds its manifest until a real archive, notices, and clean-host proof exist; Qwen remains outside the first distributable product. |
...
| Release packaging                                  | **M1-3 complete; M4 lifecycle in progress**               | M011 release authority v1 freezes the Windows x64/private embedded Piper core and separate optional Chatterbox boundary. Milestones 2-3 close the production locks, audits, 400-component inventory, deterministic bilingual Piper payload, compliance bundle, measurements, and native verification. Milestone 4 adds compiled withheld-manifest, native acquisition/verification/removal, and Settings controls, but no artifact is published. Installer integration, final optional evidence, signing, and complete-MVP release evidence remain planned. |
...
flowchart LR
    ...
    RELEASE_GRAPH["Release locks + audit + inventory<br/>M11 M2-M4 current;<br/>400 components + explicit blind spots"]:::implemented
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            ACQUISITION["Native optional-profile acquisition<br/>fixed manifest + explicit consent;<br/>state and trust authority frozen; implementation planned"]:::planned
        end
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            CHATTERBOX_PACKAGE["Optional Chatterbox GPU package<br/>M11 M4: native lifecycle implemented;<br/>manifest withheld pending audited artifact"]:::progress
    ...
    ARTIFACT_SOURCE -.->|"only after an enabled allowlisted HTTPS manifest"| ACQUISITION
...
The M011 installer and optional-acquisition nodes retain frozen authority and
remain planned. Milestones 2-3 implement the repository dependency identity
and the standalone core payload:
...
final installer or clean-host evidence. Chatterbox's implemented local adapter gains a distribution edge
...
| MVP packaging and release                               | [Active M011 ExecPlan](../plans/active/M011-package-validate-and-release-mvp.md), frozen [release authority v1](mvp-release-authority-v1.md), accepted [ADR-0042](decisions/ADR-0042-freeze-mvp-release-authority.md), [release security/distribution boundary](../development/release-security-and-distribution.md), exact [component inventory](../../services/tts/release/component-inventory-v1.json), and [Piper package evidence](../../services/tts/release/core/core-package-evidence-v1.json). Milestones 1-3 freeze the topology, close the dependency/audit boundary, and implement the verified bilingual Piper payload with compliance fulfillment. Milestone 4 adds the withheld optional Chatterbox lifecycle and source-package boundary; an audited archive, installer integration, signing, and complete release evidence remain future work. |
...
7. **Milestone 11 — In progress:** Milestone 1 freezes the exact minimum
   ...
   notices/source evidence. Milestone 4 now implements the withheld optional
   Chatterbox lifecycle and source-package checker; a real audited archive,
   final manifest, normal-user package/cleanup, complete clean-host validation,
   and separate portfolio-ready local versus signed public-installer decisions
   remain open.
