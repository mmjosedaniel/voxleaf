  ...
  subgraph device["User device — local-first boundary"]
    ...
    subgraph libraries["Framework-independent TypeScript packages"]
      ...
      subgraph epubPackage["@voxleaf/epub"]
        ...
        narration["Narration source projection, deterministic normalization,<br/>semantic segmentation and bounded prepared segments<br/>[In progress: Task 2.1 source projector implemented]"]:::inprogress
        epubCore -->|Task 2.1 semantic model and locator index| narration
      end
...
- Milestone 5: ADR-0012 accepts the narration-preparation ownership, public result, continuation, range, language, dependency, cancellation, and lifecycle boundary; Task 1.2 accepts the test-only deterministic normalization/Spanish corpus; Task 1.3 accepts exact model-independent profile bounds; and Task 2.1 implements exhaustive package-internal source projection. Code-point source spans, narration-only normalization, semantic segmentation, bounded continuation/cancellation, public lifecycle integration, and stable locator-range mapping remain unimplemented. The milestone remains in progress until its full production validation passes.
