...
| Short-unit and dual-worker feasibility | **In progress; v4 rejected, v5 mechanics implemented** | Milestone 6.2's accepted `selection-v4` selects neither stopped placement. The separate result-blind `v5` authority, benchmark-local controller, exact CPU/GPU adapters, bounded replay, and reviewed command surface exist for one GPU-primary and one complete CPU-only float32 support worker. No `v5` pilot, result, topology selection, or product runtime exists. |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            BATCH_PROBE["Short-unit and dual-worker probe<br/>In progress: M6.2<br/>v4 rejected; v5 mechanics implemented"]:::progress
...
The current user-visible flow ends at `VISUAL`. `PREP` is usable by package callers and tests but is not wired into the desktop. `NEXT_PROFILE` is completed evidence. `BATCH_PROBE` has schema-valid `v4` safety-stop results and an accepted decision selecting neither placement. It also has frozen, model-free-validated `v5` authority plus benchmark-local independent-worker mechanics, exact adapter paths, bounded replay, and a reviewed non-promotable command surface, but no pilot or result. The probe is not product runtime behavior or a selected production engine. All inference steps after it are future work. Approximately 15 seconds is a target amount of playable audio held in the future bounded buffer, not a fixed wall-clock delay; a shorter complete remaining range may start when fully ready. The `v5` five-minute value is only a maximum retained capacity after startup and remains subject to duration, count, byte, and active-work bounds.
...
| Short-unit and dual-worker feasibility | [Milestone 6.2 active ExecPlan](../plans/active/M006-002-qwen-short-segment-batch-feasibility.md), [v4 authority](tts-feasibility-profile-v4.md), machine-readable [`profile-v4.json`](../../benchmarks/tts/profile-v4.json), normalized [`corpus-v4.json`](../../benchmarks/tts/corpus-v4.json), both stopped `v4` results, accepted [`selection-v4`](../../benchmarks/tts/selection-v4.md), frozen [v5 authority](tts-feasibility-profile-v5.md), machine-readable [`profile-v5.json`](../../benchmarks/tts/profile-v5.json), scheduled [`corpus-v5.json`](../../benchmarks/tts/corpus-v5.json), and model-free-tested `services/tts/benchmarks/dual_worker_*` mechanics. No `v5` pilot, result, or product runtime exists. |
...
1. **Milestone 6.2 — In progress:** Milestones 3 and 4 completed the stopped `v4` safety paths, Milestone 5 was not admitted, Milestone 6 records the rejection and freezes separate `v5` authority, and Milestone 7 implements the model-free dual-worker benchmark mechanics. Milestones 8-10 must now run the independent GPU-primary/CPU-support experiment, evaluate measured five-minute-bounded replay, and record the final decision without changing ADR-0014 meanwhile.
