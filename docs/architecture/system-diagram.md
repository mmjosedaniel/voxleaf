...
Status applies to the text inside each node. A partial desktop shell, for example, is not evidence that file selection or reading works.
...
flowchart LR
  ...
  epubFile -. planned local selection .-> fileBoundary
...
flowchart LR
  ...
  selection["Desktop selection<br/>and in-memory byte read"]:::planned
...
The solid middle section is implemented as an in-memory package flow and can be called by another TypeScript consumer. It is not reachable from the desktop application yet. Everything before that package call and after the opened-publication boundary remains planned, so VoxLeaf cannot currently open, render, narrate, or play a user-selected book.

## Component notes and evidence

| Component | Responsibility and status evidence |
| --- | --- |
| Desktop shell | [`apps/desktop`](../../apps/desktop/) contains the React/Vite foundation UI and minimal Tauri shell. [`App.tsx`](../../apps/desktop/src/App.tsx) exposes only a development message, [`main.rs`](../../apps/desktop/src-tauri/src/main.rs) registers no commands, and [`tauri.conf.json`](../../apps/desktop/src-tauri/tauri.conf.json) grants no capabilities. Tauri/React adoption is accepted by [ADR-0001](decisions/ADR-0001-local-first-desktop.md); reader behavior remains [Roadmap Milestone 4](../plans/roadmap.md#milestone-4-deliver-the-reflowable-visual-reader-and-position-restoration). |
...
| Local device systems | Local EPUB selection, reading-state storage, and audio output are approved product interactions, but their APIs and permissions are undecided and unimplemented. Normal reading must not require a remote service or persist generated audio. |
