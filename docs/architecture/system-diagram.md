...
| Reader settings and playback controls              | **In progress; v2 authority next**                              | M010.2 Milestones 1-2 freeze and execute v1, which selects no backend. ADR-0035 authorizes a separate fee-free v2 comparison for six rates ending at `0.75x`, a narrowly reviewed media CSP, and three bounded candidates without rewriting v1. Milestone 2A is next. Current runtime remains the completed M010.1 interface, Spanish fallback, and `1.0x`.                                      |
...
but there is still no rate edge into playback. ADR-0035 now authorizes
Milestone 2A to freeze a separate six-rate, fee-free v2 authority before
Milestone 2B compares the media, exact audited Signalsmith, and optimized
repository WSOLA candidates. The planned media candidate may review only
...
| Reader settings and playback controls                   | [M010.2 active ExecPlan](../plans/active/M010-002-reader-settings-and-playback-controls.md), approved [product requirements](../product/reader-settings-and-playback-controls.md), frozen [authority v1](reader-settings-playback-authority-v1.md), [ADR-0033](decisions/ADR-0033-freeze-reader-settings-and-pitch-preserving-playback-authority.md), no-selection [ADR-0034](decisions/ADR-0034-retain-fixed-speed-after-playback-backend-evaluation.md), and reduced-range follow-up [ADR-0035](decisions/ADR-0035-reopen-reduced-range-fee-free-playback-evaluation.md). Milestones 1-2 are complete; Milestone 2A is next. Current app-shell, default-language, preference, effective-lead, and playback remain unchanged, with `1.00x` retained. |
...
5. **Milestone 10.1 — Complete:** Milestones 1/1A-6 freeze and execute the
   ...
   merged the closeout.
6. **Milestone 10.2 — In progress; Milestones 1-2 complete:** v1 authority and
   comparison select no backend and retain `1.00x`. ADR-0035 authorizes a
   separate reduced-range, fee-free v2; Milestone 2A is next and must freeze
   its six rates, candidates, licence/CSP policy, resource gates, and
   validation before results. No runtime behavior is implemented yet.
