...
| Local TTS profiles and release distribution        | **Milestone 6B planned; final release decision reopened**                                    | M010.1 implements supported Chatterbox Spanish/English plus language-matched Piper CPU profiles; Qwen Serena/Aiden remain development-only. M011 builds the verified Piper core and the currently withheld Chatterbox download lifecycle. Under ADR-0049 and release authority v2, the preliminary Milestone 7 record accepts Piper portfolio GO and conditional Chatterbox GO for systems that pass the published Windows/CUDA/VRAM/RAM/CPU gate. Planned Milestone 6B owns compatibility-gated ordinary acquisition, truthful quality/resource disclosure, release-runtime isolation, and the ordinary installed journey. The current build still hides Chatterbox Download, and trusted public publication remains pending external signing authorization. Qwen remains outside the first distributable product. |
...
| Release packaging                                  | **Implemented; unsigned local/portfolio GO**                                                | M011 Milestones 2-5 and 6A implement the closed release graph, deterministic bilingual Piper payload, withheld optional lifecycle, exact current-user NSIS candidate, truthful lifecycle feedback, and bounded uninstall data classes. The exact installer is `181,704,648` bytes with SHA-256 `56b3d0c0d991c8ded3989d6283fdca39e1071765eaf09530c4a59b9152fedc2d`. It is `unsigned-local` and accepted for the local/portfolio MVP under representative evidence authority v2; a trusted general-public channel remains pending external signing authorization. |
...
flowchart LR
    ...
    VALIDATION_PACKAGE["Separate unsigned validation installer<br/>ADR-0045: distinct identity + data root;<br/>local compatible-host use only"]:::implemented
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            ACQUISITION["Native optional-profile lifecycle<br/>M11 M4B controller implemented;<br/>normal manifest currently withheld"]:::implemented
        end
...
installer is `181,704,648` bytes with SHA-256
`56b3d0c0d991c8ded3989d6283fdca39e1071765eaf09530c4a59b9152fedc2d`.
Local static and lifecycle evidence passes; Defender and SmartScreen were not
observed for this exact hash. Representative use on an independent older
...
and lifecycle journeys pass. Download remains unavailable in the ordinary
build because its checked-in manifest is still `withheld`; this channel state
does not negate runtime support. Model repository
...
8. **Milestone 11 — Milestone 6B planned; final release decision reopened:** Milestone 1 freezes the exact minimum
   ...
   `chatterbox-runtime-v2`; the current ordinary manifest remains withheld as a
   separate distribution state. Milestone 5 implements the exact unsigned local
   ...
   gate. Planned Milestone 6B must enable ordinary acquisition only behind that
   gate, disclose quality/resource/startup costs, compile development fallbacks
   out of release, and pass the ordinary installed journey before Milestone 7 is
   renewed. Trusted public signing remains externally pending.
