...
Status applies to the text inside each node. The file-open boundary proves local selection, bounded byte transfer, package opening, safe metadata presentation, semantic text/image rendering, explicit navigation, semantic code-point/DOM range mapping, passive normalized visual-position tracking, viewport/preference reflow preservation, and exact or nearest-valid saved-position restoration.
...
flowchart LR
  ...
  subgraph device["User device — local-first boundary"]
    ...
    subgraph desktop["apps/desktop — desktop process"]
      ...
      webShell["React/Vite open UI<br/>lifecycle state and metadata"]:::implemented
      ...
      reader["Semantic text/image DOM reader, navigation,<br/>layout/preferences, range mapping, passive tracking,<br/>reflow and saved-position restoration implemented"]:::implemented
...
flowchart LR
  ...
  visualReader["Semantic text/images, navigation, layout/preferences,<br/>code-point/DOM mapping, passive normalized tracking,<br/>reflow and saved-position restoration"]:::implemented
  ...
  stateRepository -->|validated preferences and exact-book position| visualReader
...
- Milestone 4 no longer has an unresolved reader-boundary or implementation gate: ADR-0008 plus its Task 1.6 amendment resolve visual rendering, navigation, active position, incremental large-chapter policy, and reference performance limits; ADR-0009 resolves local file ingress; ADR-0010 resolves static-raster decode/CSP/lifetime behavior; ADR-0011 resolves reader-state storage/save/migration ownership; and the desktop implements those boundaries with deterministic, Chromium, and packaged-WebView2 evidence. Task 6.2 still owns final milestone-wide validation and Task 2.3's broader packaged-native reselection/cancellation/replacement matrix before the plan can close.
