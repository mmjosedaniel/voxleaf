...
| Reader settings and playback controls              | **In progress; v2 comparison next**                             | M010.2 Milestones 1-2 execute immutable v1 and select no backend. Milestone 2A freezes separate v2 authority for six rates ending at `0.75x`, exact media/Signalsmith/optimized-WSOLA candidates, fee-free licensing, a media-only CSP delta, unchanged v1 gates, and strict result lineage. Milestone 2B is next. Current runtime remains the completed M010.1 interface, Spanish fallback, and `1.0x`.                                                                                |
...
but there is still no rate edge into playback. ADR-0035 authorizes and
ADR-0036 freezes the separate six-rate, fee-free v2 authority before
Milestone 2B compares the media, exact Signalsmith 1.3.2 after its full audit,
and optimized repository WSOLA candidates. The planned media candidate may review only
`media-src 'self' blob:`; no CSP, dependency, or runtime edge exists yet.
...
5. **Milestone 10.1 — Complete:** Milestones 1/1A-6 freeze and execute the
   ...
   merged the closeout.
6. **Milestone 10.2 — In progress; Milestones 1-2A complete:** v1 authority and
   ...
   results. Milestone 2B is next. No runtime behavior is implemented yet.
