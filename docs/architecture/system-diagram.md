...
For a shorter product-level explanation, see the
[compact system diagram](system-diagram-compact.md). For detailed rationale and
invariants, see the [architecture overview](overview.md). For milestone
authority, see the [roadmap](../plans/roadmap.md) and
[completed plans](../plans/completed/README.md).
...
Solid arrows are implemented runtime or package relationships, including
conditional paths that execute only after an explicit user action or a
fail-closed gate. Dashed arrows are historical/evidence-only, approved planned,
blocked, deferred, or externally unavailable relationships and must not be read
as working product data flow.
...
| EPUB 2/NCX compatibility                           | **Complete**                                                                     | ADR-0048 and completed Milestone 3.1 admit one bounded OPF 2.0/NCX path through the same internal semantic/navigation model. Focused package, shared-target, semantic/resource/locator, reader/restoration, narration, browser, and packaged-native evidence passes. Unsupported legacy content remains outside the bounded profile; M011 Milestone 7 consumes this result without claiming general EPUB 2 conformance.                                                                                                                                                                                                                                                                  |
| Narration preparation                              | **Implemented**                                                                  | `@voxleaf/epub` exposes bounded, cancellable, locator-linked `OpenedPublication.prepareNarration` batches. The product coordinator calls it from the active or settled navigation locator and selects the bounded bilingual, Piper, or Chatterbox preparation profile before local inference.                                                                                                                                                                                                                                                                                                                                                                                          |
...
| TTS profile blocker resolution                     | **Complete historical gate; original blocker retained**                          | The exact Serena `v3` matrix failed startup, throughput, zero-failure, and mid-generation cancellation. `selection-v5` retains that historical standard-profile blocker and ADR-0015 permits one exact GPU worker only for the original bounded adaptive development demo. Later M010/M010.1 decisions independently admit supported Piper and compatibility-gated Chatterbox product profiles; they do not rewrite the failed Serena gate.                                                                                                                                                                                                                              |
...
| Local TTS profiles and release distribution        | **Milestone 7 local decision complete; trusted public signing remains external** | M010.1 implements supported Chatterbox Spanish/English plus language-matched Piper CPU profiles; Qwen Serena/Aiden remain development-only. M011 builds the verified Piper core and the ordinary Chatterbox lifecycle. Under ADR-0049 and release authority v2, Piper is portfolio GO and Chatterbox is GO only for systems that pass the published Windows/CUDA/VRAM/RAM/CPU gate. ADR-0050 records ordinary Download only after renderer presentation and native pre-network recheck, release-runtime isolation, and the hash-bound ordinary journey. Trusted public publication remains pending external signing authorization. Qwen remains outside the first distributable product. |
| Adaptive audio scheduling and playback             | **Implemented and validated**                                                    | Completed M008 provides the frozen authority, scheduler, sole-owner payload FIFO, Web Audio player, preparation presenter, controls, product narration coordinator, measured packaged quick/prepared matrix, final demo policy, and passing required CI. M009 adds bounded source-range projection without changing PCM ownership, and M010/M010.1/M011 reuse the same bounded path for supported Piper and compatibility-gated Chatterbox product narration.                                                                                                                                                                                                                     |
...
| Release packaging                                  | **Implemented; 6B complete; unsigned local/portfolio GO**                        | M011 Milestones 2-6B implement the closed release graph, deterministic bilingual Piper payload, ordinary compatibility-gated optional lifecycle, truthful lifecycle feedback, and bounded uninstall data classes. The per-user NSIS installer embeds Microsoft's WebView2 bootstrapper; only a host missing a compatible WebView2 runtime may require that prerequisite download. Current package evidence and the receipt are regenerable after each installer rebuild; no fixed installer hash is a durable architectural fact. The package is `unsigned-local` and accepted for the local/portfolio MVP under representative evidence authority v2; a trusted general-public channel remains pending external signing authorization. |
...
flowchart LR
    ...
    AUDIO["OS audio device<br/>External product playback"]:::external
    GPU["Compatible CUDA GPU<br/>External host for gated Chatterbox<br/>or development-only Qwen"]:::external
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            SHELL["Tauri native supervisor<br/>one release-locked product child or gated development child;<br/>bounded lifecycle implemented"]:::implemented
            ...
            ACQUISITION["Native optional-profile lifecycle<br/>ordinary manifest downloadable only after both gates;<br/>M11 6B complete"]:::implemented
        end
        ...
        subgraph TTSAREA["services/tts and packaged local runtimes"]
            ...
            CHATTERBOX_PACKAGE["Optional Chatterbox GPU profile<br/>official model + immutable split runtime;<br/>supported behind published host gate"]:::implemented
            CPU_FALLBACK["Piper davefx/joe CPU family<br/>Spanish/English supported;<br/>packaged local/portfolio GO"]:::implemented
    ...
    PACKAGE -->|"installed desktop binary + release resources"| SHELL
    PACKAGE -->|"embedded bootstrapper;<br/>download only if prerequisite absent"| WEBVIEW2
    WEBVIEW2 -->|"hosts the installed Tauri webview"| SHELL
    SETTINGS_SHELL -->|"eligible presentation; native gate before network"| ORDINARY_ACQUISITION
    PROD_TTS -->|"verified private Piper core + both voices"| PACKAGE
    ORDINARY_ACQUISITION -->|"explicit consent; no bundled Chatterbox bytes"| ACQUISITION
    ...
    PREP -->|"profile-specific ephemeral prepared text<br/>+ semantic boundary"| PLAYBACK
...
M008 Milestone 5 originally connected the active publication preparation,
application coordinator, M007 client/service, bounded FIFO, and Web Audio
device for the exact Qwen/Serena development demo. M010, M010.1, and M011 now
reuse that same bounded product path for supported Piper and compatibility-
gated Chatterbox narration, while Qwen remains development-only. The model-free
child remains validation-only, and the release-locked package cannot expose it
or a development-profile fallback as user-facing narration. M009 Milestone 2 adds the solid playback-to-
...
optional lifecycle and multi-artifact acquisition controller, initially with a
withheld network manifest; Milestone 6B promotes the canonical ordinary manifest
behind the unchanged live compatibility gates. Milestones 2-3
...
versioned `0.1.0` current-user Windows x64 NSIS installer. The installer embeds
Microsoft's WebView2 bootstrapper; if the host lacks a compatible runtime, that
prerequisite installation may contact Microsoft without sending EPUB,
narration, or generated-audio data. Its unsigned-local package evidence and
receipt are regenerated after each installer rebuild;
they are not a fixed architectural hash claim. Local static and lifecycle
evidence passes. Representative use on an independent older
Windows computer also confirms real Piper functionality without making GPU a
Piper requirement.
...
per-artifact digest verification, atomic versioned installation, the published
live hardware gate, and application-owned removal. Milestone 4B splits that edge:
...
assets are published and the representative compatible-host Spanish/English
and lifecycle journeys pass. The ordinary build offers Download only after
both renderer presentation and a native pre-network compatibility recheck;
it then uses the same closed verified acquisition path. Model repository
...
the artifact remains an unsigned local/portfolio MVP rather than a trusted
general-public installer. Milestone 7 consumes the completed bounded EPUB
2/NCX evidence and, under ADR-0049 and release authority v2, records GO for
Piper and conditional compatible-host Chatterbox support. Signed public
publication remains pending external authorization. These status changes do
not alter component topology or runtime data flow; they constrain the claims
made about the existing package edges.
...
documentation, and required pull-request validation. Later M010.1 and M011
packaged portfolio matrices consume the same synchronization path with
supported Piper and compatibility-gated Chatterbox profiles, while the Qwen
branch retains its constrained exact-development boundary.
...
flowchart TD
    ...
    PROFILE["Historical engine feasibility + profile decision<br/>Implemented evidence:<br/>both exact v2 profiles rejected"]:::implemented
    NEXT_PROFILE["Profile blocker resolution<br/>Complete: M6.1<br/>batch-one v3 failed; demo exception"]:::implemented
    BATCH_PROBE["Short-unit and dual-worker probe<br/>Complete: M6.2<br/>CPU + dual worker rejected"]:::implemented
    INFER["One selected local inference service + protocol<br/>one active adapter; bounded complete units<br/>M7-M10.1 implemented"]:::implemented
    PROD["Supported product TTS profiles<br/>Piper CPU + gated Chatterbox GPU<br/>M010.1/M011 validated"]:::implemented
    BUFFER["Product narration coordinator<br/>bounded scheduler/FIFO, Web Audio player,<br/>accessible controls implemented"]:::implemented
    ...
    SAFE --> PREP
    PREP -->|"profile-specific ephemeral prepared text"| BUFFER
    PROFILE -.-> NEXT_PROFILE
    NEXT_PROFILE -.-> BATCH_PROBE
    BATCH_PROBE -.->|"selection-v5 + ADR-0015:<br/>one GPU demo only"| INFER
    PROD -->|"one compatible language-bound adapter"| INFER
...
The current product flow continues from `VISUAL` through `PREP`, `BUFFER`,
`INFER`, and `DEVICE`. The application coordinator starts at the active
narration locator, selects the preparation profile associated with the admitted
language/engine pair, retains at most one bounded prepared batch, dispatches one
synthesis at a time, and transfers sole complete-unit ownership into the
player. The historical M009 exact-host matrix proves synchronized quick and
one-minute prepared audio while measuring six segment transitions, one natural
underrun/refill, 190 ms cancellation, and 378.46 buffering seconds per playback
minute. Later packaged matrices exercise the same path with supported Piper and
compatibility-gated Chatterbox profiles.
...
repository/pull-request evidence pass. The Windows x64 local/portfolio package
supports bilingual Piper on admitted CPU hosts and Chatterbox only after its
published live gate and verified optional installation; Qwen remains a
development-only exact-host branch. None of those profiles creates an
uninterrupted-playback or all-hardware guarantee. The 30-minute value remains a
...
- Persisted reading and narration preference state is limited to versioned logical locators, exact-byte publication identity, closed display preferences, and bounded language/profile/start/rate choices. An explicitly downloaded Chatterbox runtime and model are stored separately as one integrity-checked, application-owned optional package with removal support. EPUB bytes, derived narration text, model output, and generated audio are not persisted by default.
- The durable reading position is a stable EPUB logical locator, never a rendered page number.
- Implemented ingestion, raster access, reader rendering, and persistence limits are documented in the [architecture overview](overview.md) and their accepted ADRs. Narration preparation retains the model-independent [base limits](narration-preparation-limits-v1.md), [bilingual normalization](narration-normalization-v2.md), and the profile-specific [Piper v2](piper-narration-preparation-profile-v2.md) and [Chatterbox v1](chatterbox-narration-preparation-profile-v1.md) boundaries.
...
- The only authorized end-user network flows are the embedded Microsoft WebView2 prerequisite bootstrapper when a compatible runtime is absent and explicit compatibility-gated Chatterbox acquisition. Neither flow carries EPUB bytes, narration text, or generated audio. Network TTS and remote book processing remain outside the product boundary.
- The supervised Python child has exact-profile, protocol, allocation,
  ...
  representative-host privacy evidence, and truthful release claim.
...
| Secure EPUB ingestion and locator model                 | [Milestone 3 completed plan](../plans/completed/M003-secure-epub-ingestion-and-document-model.md), [ADR-0007](decisions/ADR-0007-secure-epub-ingestion-boundary.md), and [ADR-0003](decisions/ADR-0003-stable-reading-locators.md) remain the completed EPUB 3 authority. Implemented [ADR-0048](decisions/ADR-0048-admit-bounded-epub2-and-ncx-compatibility.md) and the [completed Milestone 3.1 plan](../plans/completed/M003-001-bounded-epub2-and-ncx-compatibility.md) are the additive bounded OPF 2.0/NCX package, navigation, downstream-equivalence, and validation authority.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| MVP packaging and release                               | [Completed M011 ExecPlan](../plans/completed/M011-package-validate-and-release-mvp.md), frozen historical [release authority v1](mvp-release-authority-v1.md), superseding claim [authority v2](mvp-release-authority-v2.md), additive [Chatterbox acquisition authority v2](chatterbox-official-acquisition-authority-v2.md), accepted [ADR-0042](decisions/ADR-0042-freeze-mvp-release-authority.md) through [ADR-0050](decisions/ADR-0050-promote-ordinary-chatterbox-acquisition-and-retire-validation-overlay.md), [release security/distribution boundary](../development/release-security-and-distribution.md), exact [component inventory](../../services/tts/release/component-inventory-v1.json), [Piper package evidence](../../services/tts/release/core/core-package-evidence-v1.json), regenerable [Windows package evidence](../../apps/desktop/src-tauri/release/windows-package-evidence-v2.json), and current [Chatterbox runtime evidence](../../services/tts/release/optional/chatterbox/runtime-package-evidence-v3.json). The local package/acquisition paths are implemented. Milestone 6B is complete: ordinary Download requires renderer presentation and a native pre-network compatible-host recheck, and the release-locked runtime excludes development fallbacks. The renewed hash-bound lifecycle and representative journey pass install, first start, repair, cancellation, Spanish/English Chatterbox, offline restart, removal, Piper Spanish/English, reacquisition, and uninstall. Milestone 7's local decision and PR #207's required checks pass; M011 is archived, and trusted public signing remains pending external authorization. |
| Local-first desktop and local process direction         | [ADR-0001](decisions/ADR-0001-local-first-desktop.md); ADR-0015 permits a constrained one-GPU development demo, M010/M010.1 implement exact local profiles, and completed M011 owns their narrower distributable payload                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Roadmap status                                          | [Roadmap](../plans/roadmap.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |

## Milestone closeout and remaining external gate

The implementation milestones below are complete. The only remaining gate in
this MVP release sequence is external authorization for a trusted signed public
Windows installer; it does not block the accepted unsigned local/portfolio
package.
...
6. **Milestone 10.2 — Complete:** v1 authority and
   ...
   pass.
7. **Milestone 3.1 — Complete:** ADR-0048 and the completed ExecPlan
   establish the bounded OPF 2.0/NCX path through the existing semantic,
   locator, reader, restoration, and narration owners. Focused package,
   browser, and packaged-native matrices prove the additive profile without a
   new runtime owner, dependency, public schema, persistence field, network
   edge, or renderer/narration fork.
8. **Milestone 11 — Complete for the unsigned local/portfolio MVP; trusted public signing pending external authorization:** Milestone 1 freezes the exact minimum
   ...
   `chatterbox-runtime-v2`. Milestone 5 implements the exact unsigned local
   Windows candidate and external-credential signing path; Milestone 6A
   implements lifecycle feedback and bounded uninstall choices. Renewed
   Milestone 7 consumes completed Milestone 3.1 and accepts GO for the Piper
   local/portfolio core, Chatterbox runtime on systems that pass the published
   gate, and the optional package on a compatible host. Milestone 6B enables
   ordinary acquisition only after renderer
   presentation and native pre-network recheck of that gate, compiles
   development fallbacks out of release, and passes the hash-bound ordinary
   installed journey. Renewed Milestone 7 repeats the exact-artifact lifecycle
   and journey and records the independent local/portfolio decisions. PR #207's
   required checks pass and M011 is archived. Trusted public signing remains
   externally pending.
