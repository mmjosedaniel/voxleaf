...
| Bilingual narration and candidate screening        | **In progress; full evaluation complete, integration pending** | M010.1 Milestone 2 implements `narration-bilingual-v2`, the bounded Spanish/English preference, accessible explicit selection, exact profile/language admission, and identity-safe cancellation/cleanup. Milestone 3 admits Piper/joe English. Corrective Milestone 5 admits exact Chatterbox for both languages, retains Qwen/Serena Spanish, and admits Qwen/Aiden English after the complete bilingual machine matrix and fresh quality controls. MOSS remains deferred without rejection. The executable registry remains unchanged, so these newly admitted paths are not selectable or runtime-supported until Milestone 6 integrates and proves them.                                        |
