...
| Reader settings and playback controls              | **In progress; preferences implemented, shell/speed pending**    | M010.2 Milestones 1-3 are complete. English-default language v2 and narration-start v1 preferences are bounded, hydrated before use, and reset through identity-first cleanup. Valid saved Spanish/English is preserved. The separate boundary-deferred v3 passed its complete comparison and ADR-0040 selects repository WSOLA without a dependency or CSP expansion. Milestone 4 owns the Settings shell, Milestone 5 owns the six-rate connection, and runtime playback remains `1.00x`. |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            SETTINGS_SPEED["Reader-first Settings + playback<br/>M10.2 M4-M5 pending;<br/>WSOLA selected"]:::planned
    ...
    SETTINGS_SPEED -.->|"planned Settings composition"| COMPAT
...
The remaining dashed M010.2 node is not a current runtime relationship.
Milestone 1
...
5. **Milestone 10.1 — Complete:** Milestones 1/1A-6 freeze and execute the
   ...
   merged the closeout.
6. **Milestone 10.2 — In progress; Milestones 1-3 complete:** v1 authority and
   ...
   language/start preference runtime. Milestone 5 owns the selected backend
   connection after Milestone 4.
