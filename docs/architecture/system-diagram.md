...
- Milestone 5: ADR-0012 accepts the narration-preparation ownership, public result, continuation, range, language, dependency, cancellation, and lifecycle boundary, and Task 1.2 accepts the test-only deterministic normalization/Spanish corpus. Task 1.3 must still accept exact profile bounds before source projection, narration-only normalization, semantic segmentation, bounded continuation/cancellation, and stable locator-range mapping are implemented. Until production validation passes, the status remains approved planned.
