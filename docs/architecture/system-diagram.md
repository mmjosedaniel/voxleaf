...
| Local TTS profiles and release distribution        | **Local profiles implemented; distribution approved planned** | M010.1 implements supported Chatterbox Spanish/English plus language-matched Piper CPU profiles; Qwen Serena/Aiden remain development-only. M011 plans a small Windows x64/Piper Spanish-English core and a separately gated optional Chatterbox GPU download with explicit consent, integrity verification, atomic installation, offline use, and removal. Qwen remains outside the first distributable product.                                                                       |
...
| Release packaging                                  | **Approved planned**                                          | The active M011 ExecPlan freezes a Windows x64/Piper core, separate optional Chatterbox manifest/acquisition/removal boundary, exact shipped/downloadable dependency/licence/integrity closure, normal-user install/uninstall and clean-host validation, and independent Piper-core, Chatterbox-package, and signed-public decisions. No installer, acquisition flow, signing path, or complete-MVP release evidence is implemented yet.                                                |
...
flowchart LR
    ...
    PACKAGE["Windows x64 MVP core package<br/>M11 approved planned;<br/>Piper Spanish/English"]:::planned
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            ACQUISITION["Native optional-profile acquisition<br/>fixed manifest + explicit consent;<br/>bounded verify/install/remove; M11 planned"]:::planned
        end
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            PROD_TTS["Minimum distributable TTS family<br/>M11 approved planned:<br/>Piper Spanish/English"]:::planned
            CHATTERBOX_PACKAGE["Optional Chatterbox GPU package<br/>minimal runtime + bilingual model;<br/>M11 separately gated planned"]:::planned
...
The M011 package and acquisition nodes are approved planned, not runtime
edges. The core payload combines the implemented desktop/service boundary with
...
| MVP packaging and release                               | [Active M011 ExecPlan](../plans/active/M011-package-validate-and-release-mvp.md) and [release security/distribution boundary](../development/release-security-and-distribution.md). The Windows x64/Piper core, separately gated optional Chatterbox acquisition/removal package, shipped/downloadable dependency/licence/integrity closure, clean-host lifecycle, signing path, and independent core/optional/public decisions are approved planning only; no installer, acquisition flow, or release evidence exists yet.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
6. **Milestone 10.2 — Complete:** v1 authority and
   ...
   pass.
7. **Milestone 11 — Approved planned; next:** freeze the exact minimum
   Windows/Piper payload and threat model; close shipped dependencies,
