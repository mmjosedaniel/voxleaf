...
| Bilingual narration and candidate screening        | **In progress; runtime integration complete**                   | M010.1 Milestone 6 implements exact profile/language registry, matching, UI selection, pre-start configuration, native supervision, Piper English, Chatterbox bilingual, and Qwen Serena/Aiden bindings without changing protocol v1. Both Piper voices and Chatterbox are supported when exact gates pass; both Qwen voices remain development-only. The sequential six-arm exact-host service matrix passes. Milestone 7 still owns packaged portfolio journeys and final closeout.   |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            BILINGUAL["Bilingual preparation + exact selection<br/>M10.1 Milestones 2-6 integrated;<br/>portfolio closeout pending"]:::implemented
        end
    end
...
passes; packaged portfolio proof remains Milestone 7 work.
...
5. **Milestone 10.1 — In progress:** Milestones 1/1A-6 freeze and execute the
   ...
   Milestone 7 must still run packaged portfolio journeys, record final
   metrics/privacy evidence, and close the plan.
