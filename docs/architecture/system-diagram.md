...
| Hardware profiles, fallback, and resilience        | **In progress; recovery implemented**   | M010 Milestones 1-4 freeze, implement, and validate the privacy-safe report boundary, immutable three-entry evidence registry, result-blind matching, bounded profile-ID preference, compatibility UI, immediate pre-start check, and identity-safe one-attempt operational recovery. Exact Qwen/Serena remains development-only behind its native gate; rejected entries remain unavailable. No supported recommendation, admitted CPU fallback, renderer permission, raw-report persistence, automatic retry, or standard support claim exists. |
...
recommendation. The recovery edge is now solid. Milestone 4 implements the
...
automatic retry or second service tree is admitted. The CPU-fallback node
...
| Hardware profiles, fallback, and operational resilience | [M010 active ExecPlan](../plans/active/M010-hardware-profiles-fallback-and-operational-resilience.md), frozen [hardware/profile/recovery authority v1](hardware-profile-recovery-authority-v1.md), and accepted [ADR-0019](decisions/ADR-0019-privacy-safe-hardware-profiles-and-recovery.md). Milestones 1-3 implement the canonical privacy-safe report, bounded native detector, measured registry/matcher, preference, compatibility UI, and pre-start enforcement. Milestone 4 implements the desktop-local identity-safe recovery controller, verified zero-owner cleanup, one explicit restart, latest-heard resume, and closed containment paths. Standard support and fallback remain absent. |
...
3. **Milestone 10 — In progress:** Milestones 1-4 freeze the [hardware/profile/recovery authority v1](hardware-profile-recovery-authority-v1.md) and implement its bounded native Windows report boundary, measured matching, compatibility UI, pre-start enforcement, and identity-safe one-attempt operational recovery. Follow the [M010 ExecPlan](../plans/active/M010-hardware-profiles-fallback-and-operational-resilience.md) for fallback admission and remaining resilience validation without promoting the current exact-development profile.
