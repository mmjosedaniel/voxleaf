...
| Short-unit and dual-worker feasibility | **In progress; v4 rejected, v5 frozen** | Milestone 6.2's accepted `selection-v4` selects neither stopped placement. The separate result-blind `v5` authority now freezes one GPU-primary and one complete CPU-only float32 support worker, 8-16-second target units, and a five-minute bounded maximum. No `v5` runner, pilot, result, topology selection, or product runtime exists. |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            BATCH_PROBE["Short-unit and dual-worker probe<br/>In progress: M6.2<br/>v4 rejected; v5 authority frozen"]:::progress
...
1. **Milestone 6.2 — In progress:** Milestones 3 and 4 completed the stopped `v4` safety paths, Milestone 5 was not admitted, and Milestone 6 records the rejection and freezes separate `v5` authority. Milestones 7-10 may now implement and run the independent GPU-primary/CPU-support experiment, five-minute-bounded replay, and final decision without changing ADR-0014 meanwhile.
