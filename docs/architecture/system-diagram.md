...
| Standard GPU TTS profile and release distribution  | **Bilingual local profiles implemented; distribution deferred** | ADR-0013 remains the historical failed standard-profile decision. M010.1 separately admits and implements supported Chatterbox Spanish/English plus language-matched Piper CPU profiles; Qwen Serena/Aiden remain development-only. Compliant runtime/model distribution remains M011 work.                                                                                                                                                                                             |
...
| Reader settings and playback controls              | **In progress; WSOLA selected, integration pending**            | M010.2 Milestones 1-2D are complete. Immutable v1 and frozen v2 comparisons selected no backend. The separate boundary-deferred v3 passed Chromium, packaged WebView2, exact Piper contention, privacy/lifecycle, and bilingual listening. ADR-0040 selects repository WSOLA, retains no dependency or CSP expansion, and defers the actual six-rate product connection to Milestone 5. Runtime remains `1.00x`.                                                                        |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            SETTINGS_SPEED["Reader-first Settings + playback<br/>M10.2 M1-M2D complete; WSOLA selected;<br/>runtime not integrated"]:::planned
    ...
    SETTINGS_SPEED -.->|"planned six-rate controls"| PLAYBACK
...
The dashed M010.2 node is not a current runtime relationship. Milestone 1
...
results. Milestone 2 selected no backend and ADR-0034 retains `1.00x`; later
reader/Settings work may compose the reader, store, and compatibility nodes,
but there is still no rate edge into playback. ADR-0035 authorized and
...
| Reader settings and playback controls                   | [M010.2 active ExecPlan](../plans/active/M010-002-reader-settings-and-playback-controls.md), approved [product requirements](../product/reader-settings-and-playback-controls.md), immutable [authority v1](reader-settings-playback-authority-v1.md), frozen [authority v2](reader-settings-playback-authority-v2.md), frozen [authority v3](reader-settings-playback-authority-v3.md), [ADR-0033](decisions/ADR-0033-freeze-reader-settings-and-pitch-preserving-playback-authority.md), no-selection [ADR-0034](decisions/ADR-0034-retain-fixed-speed-after-playback-backend-evaluation.md), reduced-range decision [ADR-0035](decisions/ADR-0035-reopen-reduced-range-fee-free-playback-evaluation.md), v2 freeze [ADR-0036](decisions/ADR-0036-freeze-reduced-range-fee-free-playback-authority-v2.md), v2 no-selection [ADR-0037](decisions/ADR-0037-retain-fixed-speed-after-reduced-range-evaluation.md), boundary-deferred v3 decision [ADR-0038](decisions/ADR-0038-reopen-boundary-deferred-playback-evaluation.md), v3 freeze [ADR-0039](decisions/ADR-0039-freeze-boundary-deferred-playback-authority-v3.md), aggregate [v3 result](../../benchmarks/playback/boundary-deferred-v3-result.json), and WSOLA selection [ADR-0040](decisions/ADR-0040-select-repository-wsola-for-boundary-deferred-playback.md). Milestones 1-2D are complete. Current app-shell, default-language, preference, and playback remain unchanged, with `1.00x` retained until Milestone 5. |
...
5. **Milestone 10.1 — Complete:** Milestones 1/1A-6 freeze and execute the
   ...
   merged the closeout.
6. **Milestone 10.2 — In progress; Milestones 1-2D complete:** v1 authority and
   ...
   frozen v3 gate passes. No M010.2 production runtime behavior is implemented
   yet; Milestone 5 owns the selected backend connection after Milestones 3-4.
