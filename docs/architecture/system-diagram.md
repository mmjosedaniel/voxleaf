...
| Reader settings and playback controls              | **In progress; Settings implementation next**                   | M010.2 Milestones 1-2B are complete. Immutable v1 and frozen v2 comparisons both select no backend. Under local Piper contention, media exceeded the frozen RAM delta and incremental WSOLA exceeded frozen start latency; Signalsmith failed before its first trial. ADR-0037 removes every experiment and retains `1.00x`. Milestone 3 is next. Current runtime remains the completed M010.1 interface and Spanish fallback.                                   |
...
removed. No CSP, dependency, or runtime rate edge exists.
...
6. **Milestone 10.2 — In progress; Milestones 1-2B complete:** v1 authority and
   ...
   contention and removes every experiment. Milestone 3 is next. No M010.2
