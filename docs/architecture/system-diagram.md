...
| Bilingual narration and candidate screening        | **In progress; candidate routing recorded, integration pending** | M010.1 Milestone 2 implements `narration-bilingual-v2`, the bounded Spanish/English preference, accessible explicit selection, exact profile/language admission, and identity-safe cancellation/cleanup. Milestone 3 independently admits exact Piper/joe English after every frozen v8 baseline gate passes. Corrective Milestone 4 advances Chatterbox as the sole new-engine full-matrix survivor, defers MOSS without rejection, and retains Qwen Serena/Spanish plus Aiden/English as separate hardware-dependent constrained-buffer candidates. The executable registry remains unchanged, so English is not selectable or supported until Milestone 6 integrates and proves admitted paths. |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            BILINGUAL["Bilingual preparation + explicit selection<br/>M10.1 Milestones 2-4;<br/>candidate routing recorded"]:::progress
        end
    end
...
profile/language pair is rejected before child start. Its candidate branch now
has content-safe Piper English admission and corrective MOSS/Chatterbox screen
evidence, while Chatterbox's full matrix and the two Qwen quality decisions
remain dashed future work. No English engine is selectable and the existing
M010 matrix remains authoritative.
...
| Bilingual narration and candidate screening             | [M010.1 active ExecPlan](../plans/active/M010-001-bilingual-narration-and-candidate-screening.md), implemented [bilingual product authority v1](bilingual-narration-authority-v1.md) and [normalization v2](narration-normalization-v2.md), historical resultless [v7 evaluation profile](tts-feasibility-profile-v7.md), superseding [v8 evaluation profile](tts-feasibility-profile-v8.md), corrective [v9](tts-feasibility-profile-v9.md) and [v11](tts-feasibility-profile-v11.md) authorities, exact content-safe [Piper English v8](../../benchmarks/tts/piper-english-result-v8.json), [MOSS v9](../../benchmarks/tts/moss-bilingual-screen-result-v9.json), and [Chatterbox v11](../../benchmarks/tts/chatterbox-bilingual-screen-result-v11.json) results, accepted [selection v11](../../benchmarks/tts/selection-v11.md), and [ADR-0029](decisions/ADR-0029-advance-chatterbox-retain-qwen-defer-moss.md). Milestone 2 implements model-free preparation and selection; Milestone 3 admits Piper/joe English; Milestone 4 records candidate routing without changing runtime support. M010's current runtime support matrix remains authoritative until Milestone 6 integrates and proves admitted paths. |
...
5. **Milestone 10.1 — In progress:** Milestones 1/1A-4 freeze authority,
   implement bilingual preparation, admit the Piper English baseline, advance
   Chatterbox to the next full matrix, defer MOSS without rejection, and
   retain local Qwen/Serena Spanish plus Qwen/Aiden English as separate
   hardware-dependent constrained-buffer candidates. Later milestones must
   complete evaluation and integration without claiming support before
   evidence passes.
