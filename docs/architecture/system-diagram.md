...
| Shared contracts and fakes                         | **Implemented**                         | Versioned contracts, runtime decoders, conformance fixtures, deterministic fakes, the closed protocol-v1 control family, and the M010 privacy-safe host-profile compatibility report exist. The new report is authority for future native-to-desktop host facts; it does not implement probing, matching, support, or recovery.                                                                                                                                                              |
...
| Hardware profiles, fallback, and resilience        | **In progress; authority frozen**       | M010 Milestone 1 freezes the privacy-safe host report, immutable profile/evidence shape, result-blind resource margins, deterministic matching/preference rules, closed failure taxonomy, and identity-first explicit recovery. No runtime host probe, registry entry, product matcher, admitted CPU fallback, recovery controller, dependency, permission, or support claim exists yet.                                                                                                          |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            COMPAT["Privacy-safe host report authority<br/>M10 Milestone 1 frozen;<br/>probe + matcher planned"]:::progress
...
The dashed M010 edges remain planned runtime boundaries. Milestone 1 makes the
`COMPAT` node an authority-only in-progress foundation: the canonical
host-profile report, profile/evidence shape, result-blind margins,
matching/preference order, failure taxonomy, and recovery transitions are
frozen, but no host fact is collected or matched. The authority keeps
native-owned privacy-safe host facts separate from engine
...
3. **Milestone 10 — In progress:** Milestone 1 freezes the [hardware/profile/recovery authority v1](hardware-profile-recovery-authority-v1.md). Follow the [M010 ExecPlan](../plans/active/M010-hardware-profiles-fallback-and-operational-resilience.md) for runtime privacy-safe hardware reporting, measured profile selection, fallback admission, and identity-safe operational recovery without promoting the current exact-development profile.
