...
| Shared contracts and fakes                               | **Implemented**                         | Versioned contracts, runtime decoders, conformance fixtures, deterministic fakes, and the closed protocol-v1 control family exist. They do not implement native supervision, real TTS, playback, or synchronization.                                                                                                                                  |
| Tauri native shell                                       | **In progress**                         | M007 Milestone 1 adds one narrow model-free Rust-owned child/std-stream protocol probe and optimized binary-response command. No plugin or capability is granted; packaged WebView evidence passes through Tauri's internal-only IPC CSP sources.                                                                                                     |
...
| Constrained local TTS service and process protocol       | **In progress**                         | M007 Milestones 1 and 2 have frozen transport limits, packaged binary-response evidence, canonical cross-language control contracts, and a bounded model-free Python service. Native supervision, desktop integration, and the Qwen adapter remain unimplemented.                                                                                     |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            SHELL["Tauri shell<br/>one narrow synthetic protocol probe<br/>M7 in progress; no plugin/capability"]:::progress
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            TTS["Constrained local TTS service + protocol<br/>M7 in progress: transport + contracts +<br/>model-free service; no supervisor/model"]:::progress
...
flowchart TD
    ...
    BATCH_PROBE["Short-unit and dual-worker probe<br/>Complete: M6.2<br/>CPU + dual worker rejected"]:::implemented
    INFER["Constrained local inference + protocol<br/>M7 in progress: transport, contracts, model-free service<br/>native supervisor/model not implemented"]:::progress
...
The current user-visible flow ends at `VISUAL`. `PREP` is usable by package callers and tests but is not wired into the desktop. `NEXT_PROFILE` and `BATCH_PROBE` are completed development evidence. Accepted `selection-v5` rejects CPU-only and dual-worker scheduling and retains the exact GPU candidate only for ADR-0015's constrained demo. M007 is in progress; Milestones 1 and 2 complete the model-free native transport probe, accepted protocol authority, canonical contracts, and bounded fake Python service. No native supervisor, Qwen integration, audible product runtime, or selected production engine exists. Quick start uses approximately 15 playable seconds rather than a fixed wall-clock delay; an explicit prepared mode may target 1, 2, 5, or 10 playable minutes. The approximately 30-minute in-memory value is a simultaneous ceiling, not a startup target or uninterrupted-playback guarantee.
...
| Constrained local TTS service and process protocol     | [M007 active ExecPlan](../plans/active/M007-local-tts-service-and-process-protocol.md), accepted frozen [protocol v1 authority](tts-service-protocol-v1.md), and accepted [ADR-0016](decisions/ADR-0016-rust-owned-stdio-tts-protocol.md). Milestones 1 and 2 have transport, packaged WebView, canonical contract, and model-free Python-service evidence; native supervision and model work remain open.                                                                                                                                                                                                                                                                              |
...
1. **Milestone 7 — In progress; production blocked:** implement M007's native supervisor and one bounded GPU Qwen/Serena development-demo adapter on the completed transport, canonical protocol, and model-free service foundation. Do not claim passing performance, continuous playback, native model streaming, general hardware support, or production distribution.
