...
| Tauri native shell                                       | **In progress**                         | M007 Milestones 1, 3, 4, and 5 implement the narrow binary-response boundary, persistent child supervision, framed standard streams, fixed timeouts, process-tree termination, zero automatic restart, typed commands, native-only exact-service activation, measured handoff diagnostics, and exit cleanup. No plugin, general shell capability, or listener is granted. |
| Python TTS area                                          | **In progress**                         | M007 Milestones 2, 4, and 5 add the offline-validated framed service, bounded fake engine, common one-active adapter boundary, exact development-only Qwen/Serena adapter, and content-safe exact-host measurement runner. Default tests remain model-free.                                                                                                               |
...
| Constrained local TTS service and process protocol       | **In progress**                         | M007 Milestones 1-5 have frozen transport limits, canonical contracts, bounded Python service, native supervision, typed desktop consumption, one-unit ownership, exact Qwen/Serena integration, model-free packaged evidence, and measured exact-host handoff/cancellation/cleanup evidence. Product caller and closeout remain unimplemented.                           |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            SHELL["Tauri native supervisor<br/>model-free default or native-configured exact child<br/>M7 M3-M5 implemented; no plugin/capability"]:::progress
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            TTS["Constrained local TTS service + protocol<br/>M7 M1-M5 implemented and exact-host measured<br/>no product caller; closeout open"]:::progress
...
flowchart TD
    ...
    BATCH_PROBE["Short-unit and dual-worker probe<br/>Complete: M6.2<br/>CPU + dual worker rejected"]:::implemented
    INFER["Constrained local inference + protocol<br/>M7 M1-M5 implemented and exact-host measured<br/>product caller/closeout not implemented"]:::progress
...
The current user-visible flow ends at `VISUAL`. `PREP` is usable by package callers and tests but is not wired into the desktop. `NEXT_PROFILE` and `BATCH_PROBE` are completed development evidence. Accepted `selection-v5` rejects CPU-only and dual-worker scheduling and retains the exact GPU candidate only for ADR-0015's constrained demo. M007 is in progress; Milestones 1-5 complete the model-free transport, protocol authority, canonical contracts, Python service, native supervisor, typed client, one-unit handoff, exact development-only Qwen/Serena adapter, and measured exact-host handoff/cancellation/cleanup matrix. Audible product runtime still does not exist, and no production engine is selected. Quick start uses approximately 15 playable seconds rather than a fixed wall-clock delay; an explicit prepared mode may target 1, 2, 5, or 10 playable minutes. The approximately 30-minute in-memory value is a simultaneous ceiling, not a startup target or uninterrupted-playback guarantee.
...
| Constrained local TTS service and process protocol     | [M007 active ExecPlan](../plans/active/M007-local-tts-service-and-process-protocol.md), accepted frozen [protocol v1 authority](tts-service-protocol-v1.md), accepted [ADR-0016](decisions/ADR-0016-rust-owned-stdio-tts-protocol.md), and the schema-valid [exact-host handoff result](../../benchmarks/tts/service-handoff-result-v1-exact-host.json). Milestones 1-5 have transport, canonical contracts, Python service, native supervision, typed client, exact Qwen/Serena integration, model-free packaged evidence, and measured exact-host delivery/backpressure/invalidation/termination/cleanup/reload evidence; product caller and closeout remain open.                    |
...
1. **Milestone 7 — In progress; production blocked:** complete M007's repository/protocol closeout on the implemented and exact-host-measured one-GPU Qwen/Serena development adapter, transport, service, native supervisor, and typed-client foundation. Do not claim sustainable playback, continuous playback, native model streaming, general hardware support, or production distribution.
