...
Status applies to the text inside each node. The file-open boundary proves local selection, bounded byte transfer, package opening, safe metadata presentation, semantic text/image rendering, explicit navigation, semantic code-point/DOM range mapping, and passive normalized visual-position tracking, but it does not yet prove reflow or saved-position restoration.
...
flowchart LR
  ...
  subgraph device["User device — local-first boundary"]
    ...
    subgraph desktop["apps/desktop — desktop process"]
      ...
      reader["Semantic text/image DOM reader, navigation,<br/>layout/preferences, range mapping, and passive<br/>visual-position tracking implemented; restoration planned"]:::partial
...
The capability-free file-open flow now passes one bounded in-memory read to the UI-independent publication-session owner and presents only validated title/authors or fixed safe outcomes. The session provides cancellation, replacement, stale-result rejection, and publication cleanup; the desktop shell adds an immutable idle/opening/ready/empty/failure/closing surface, explicit close/reopen, fixed presentation-error containment, and one continuous responsive reader with closed in-memory display preferences. The reader consumes the bounded static-raster boundary through lazy serialized resource reads, accessible per-image fallback, and lifecycle-owned Blob URL release; the Tauri capability list remains empty. ADR-0011 approves bounded WebView `localStorage`, but no storage repository, preference write, or position save/restoration behavior exists yet. No remote runtime service or external network dependency is approved for normal reading; the exact local desktop-to-TTS transport remains undecided.
...
flowchart LR
  ...
  visualReader["Semantic text/images, navigation, layout/preferences,<br/>code-point/DOM mapping, and passive normalized<br/>visual-position tracking implemented; restoration planned"]:::partial
...
| Reader, file/raster boundaries, coordinator, and persistence | ADR-0009/Tasks 1.2 and 2.3 implement the capability-free WebView selection/read/open boundary and safe metadata presentation; Task 2.2 implements its desktop-to-EPUB publication-session owner; Task 2.4 implements the surrounding accessible lifecycle state, empty recovery, close/reopen, stale-view clearing, and presentation-error cleanup. Task 3.1 implements the exhaustive semantic text renderer and deterministic starting-spine selection through the first canonical located block. Task 3.2 implements canonical active document/locator state, package-resolved TOC/internal/chapter navigation, deterministic boundaries/explanations, and explicit-navigation focus. ADR-0010/Tasks 1.3 and 3.3 implement bounded static-raster metadata, decode, capacity, semantic presentation, cancellation, fallback, and object-URL lifecycle. Task 3.4 implements the closed app-local display-preference model, accessible controls, coordinator-owned content-free reflow intents, app-lifetime in-memory preference retention, and application CSS mappings for one responsive continuous layout. Task 3.6 implements the ADR-0008 amendment through recursive 10,000-block/80,000-projected-node preflight, one cancellable 250-block browser-yield scheduler, memoized structural rendering without window virtualization, a fixed no-partial-content fallback, last-valid-locator retention, and production React benchmark evidence. Task 4.1 implements a lifecycle-owned, content-free map from every rendered addressable block to its package locator plus bidirectional semantic-code-point/DOM-range conversion. It counts Unicode code points, one newline position per line break, and one object-replacement position per raster image while excluding alternative and application-only accessibility text; structural blocks remain at offset zero. Sparse checkpoints bound conversion metadata, and registration replacement, chapter change, unmount, clear, and close reject stale nodes and release DOM references. Task 4.2 adds a publication-scoped active visual-locator tracker: one application reading line selects the first crossing leaf or deterministic nearest visible block, browser caret ranges map through Task 4.1 with block-start fallback, and every candidate normalizes through `OpenedPublication.resolveLocator` before updating coordinator state. A source-ordered leaf index uses logarithmic probing plus nearby visible measurements instead of one native observation target per block, scroll/resize/render callbacks coalesce to one pending animation frame, and tokenized suspension prevents transient programmatic-navigation geometry from replacing its destination. The DOM emits no publisher markup, arbitrary attributes/styles, source IDs/fragments, resource identities, locators, URLs, or history entries, and passive tracking performs no focus or storage write. [ADR-0011](decisions/ADR-0011-bounded-web-storage-reader-state.md) approves two bounded versioned Web Storage envelopes, a 500 ms passive-save debounce, exact-identity restoration, content-free failures, and explicit desktop-owned migration, but those persistence modules remain unimplemented. [Roadmap Milestone 4](../plans/roadmap.md#milestone-4-deliver-the-reflowable-visual-reader-and-position-restoration) and the [active plan](../plans/active/M004-reflowable-visual-reader-and-position-restoration.md) retain implementation ownership. No storage adapter, native file access, or reflow-restoration implementation exists yet. File paths and native permissions remain outside the accepted boundary because `@voxleaf/epub` accepts bytes only. |
