...
| Release packaging                                  | **M1-4B deterministic foundation and runtime publication implemented; optional download withheld** | M011 release authority v1 freezes the Windows x64/private embedded Piper core and historical single-archive optional boundary. Milestones 2-3 close the production locks, audits, 400-component inventory, deterministic bilingual Piper payload, compliance bundle, measurements, and native verification. Milestone 4A adds the compiled withheld lifecycle. Milestone 4B freezes exact official Hugging Face model acquisition and publishes the reproducible three-part runtime, but Download remains withheld until clean-host gates pass. Installer integration, signing, and complete-MVP release evidence remain planned. |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            ACQUISITION["Native optional-profile lifecycle<br/>M11 M4B multi-artifact controller implemented;<br/>manifest withheld pending clean host"]:::progress
        end
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            CHATTERBOX_PACKAGE["Optional Chatterbox GPU profile<br/>official six-file model + published split runtime;<br/>withheld pending clean host"]:::progress
