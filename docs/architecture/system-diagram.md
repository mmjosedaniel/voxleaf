...
| Reader settings and playback controls              | **Local validation complete; closeout pending**                 | M010.2 Milestones 1-5 are complete. English-default language v2 plus narration-start v1 and playback-rate v1 preferences are bounded; the reader-first shell is implemented; and ADR-0040's repository WSOLA provides six boundary-deferred rates without restarting TTS, discarding source PCM, adding a dependency, or expanding CSP. Milestone 6's six-arm packaged and repository checks pass locally; intermediate-rate human confirmation and required PR checks still gate archival. |
...
5. **Milestone 10.1 — Complete:** Milestones 1/1A-6 freeze and execute the
   ...
   merged the closeout.
6. **Milestone 10.2 — In progress; local automated closeout passes:** v1 authority and
   ...
   Milestone 6's six packaged profile/language arms and complete local
   repository checks pass; human intermediate-rate confirmation and required
   PR checks still gate archival.
