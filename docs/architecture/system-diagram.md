...
| Adaptive audio scheduling and playback                   | **In progress**                         | M008 Milestones 1-3 provide the frozen authority, model-free scheduler, sole-owner payload FIFO, and dedicated Web Audio PCM player. Deterministic tests cover exact format/order, consumption, underruns, pause/resume, volume, end-of-range, and bounded cleanup. No publication/M007 product caller or playback UI exists.                                                   |
...
flowchart LR
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            PLAYBACK["Adaptive scheduler + payload FIFO<br/>Web Audio PCM player<br/>M8 Milestones 1-3 implemented; unconnected"]:::implemented
...
flowchart TD
    ...
    BUFFER["Bounded adaptive scheduler + payload FIFO<br/>Web Audio PCM player implemented<br/>not product-connected"]:::implemented
...
The current user-visible flow ends at `VISUAL`. `PREP` is usable by package callers and tests but is not wired into the desktop. `NEXT_PROFILE` and `BATCH_PROBE` are completed development evidence. Accepted `selection-v5` rejects CPU-only and dual-worker scheduling and retains the exact GPU candidate only for ADR-0015's constrained demo. Completed M007 provides the model-free transport, protocol authority, canonical contracts, Python service, native supervisor, typed client, one-unit handoff, exact development-only Qwen/Serena adapter, measured exact-host handoff/cancellation/cleanup matrix, and repository decision audit. M008 Milestones 1-3 now provide the model-independent authority, scheduler, payload-owning bounded FIFO, and low-level Web Audio player. The dashed TTS/player/device edges remain unconnected: no desktop product caller instantiates the path, so audible product runtime still does not exist and no production engine is selected. The 30-minute in-memory value is a simultaneous ceiling, not a startup target or uninterrupted-playback guarantee.
...
- Implemented publication, preparation, constrained inference, and low-level playback work supports cancellation and close within each current boundary. The M008 FIFO already makes stale audio ineligible before bounded cleanup; future product narration dispatch must propagate that identity-first transition across every component. Playback-only pause may continue bounded same-identity generation; explicit stop and invalidating actions still cancel obsolete work.
...
| Playable-audio startup and adaptive buffering policy   | [ADR-0004](decisions/ADR-0004-start-after-audio-lead.md), [ADR-0015](decisions/ADR-0015-bounded-adaptive-qwen-demo-buffering.md), frozen [adaptive buffer v1 authority](adaptive-buffer-authority-v1.md), and [M008](../plans/active/M008-bounded-adaptive-prebuffering.md). Milestones 1-3 provide authority, arithmetic, scheduler traces, sole-owner payload buffering, and deterministic low-level Web Audio playback evidence; the publication/M007 caller and UI remain unimplemented.                                                                                                                                                                      |
...
1. **Milestone 8 — In progress:** Milestones 1-3 freeze the authority and implement the model-free scheduler, bounded payload FIFO, and low-level player. Continue M008 with the product narration caller, quick/prepared UI, playback-only pause continuation, adaptive boundary waits, exact one-GPU integration, and truthful frontier buffering. Consume M007 protocol v1 without adding a service-side queue or automatic retry.
