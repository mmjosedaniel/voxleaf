...
| Local TTS profiles and release distribution        | **Local implementation complete; all release claims NO-GO**                                | M010.1 implements supported Chatterbox Spanish/English plus language-matched Piper CPU profiles; Qwen Serena/Aiden remain development-only. M011 builds the verified Piper core and withheld Chatterbox lifecycle. Milestone 7 records NO-GO for Piper portfolio readiness because the exact final hash lacks formal corrected clean-host evidence; NO-GO for optional Chatterbox because its compatible clean-GPU-host acquisition/restart/removal lifecycle is incomplete; and NO-GO for public publication because Piper has not passed and trusted signing authority is unavailable. Qwen remains outside the first distributable product.                                                                                                                                                                                                                                                                           |
...
| Release packaging                                  | **Implemented local candidate; release blocked after M7 NO-GO**                            | M011 Milestones 2-5 and 6A implement the closed release graph, deterministic bilingual Piper payload, withheld optional lifecycle, exact current-user NSIS candidate, truthful lifecycle feedback, and bounded uninstall data classes. The exact installer is `181,704,648` bytes with SHA-256 `56b3d0c0d991c8ded3989d6283fdca39e1071765eaf09530c4a59b9152fedc2d`. It is `unsigned-local`; the missing clean-host, clean-GPU-host, and trusted-signing evidence blocks the corresponding release claims.                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
flowchart LR
    ...
    SIGN["Trusted Windows signing authority<br/>External; currently unavailable<br/>public publication NO-GO"]:::external
    ...
    PACKAGE["Windows x64 per-user NSIS candidate<br/>VoxLeaf 0.1.0 + Piper ES/EN;<br/>unsigned local; portfolio/public NO-GO"]:::blocked
    ...
    subgraph DEVICE["User device / local-only trust boundary"]
        subgraph DESKTOP["apps/desktop"]
            ...
            ACQUISITION["Native optional-profile lifecycle<br/>M11 M4B controller implemented;<br/>normal manifest withheld after M7 NO-GO"]:::blocked
        end
        ...
        subgraph TTSAREA["services/tts and later local runtime"]
            ...
            CHATTERBOX_PACKAGE["Optional Chatterbox GPU profile<br/>official model + immutable split runtime;<br/>withheld; clean-GPU-host gate NO-GO"]:::blocked
            CPU_FALLBACK["Piper davefx/joe CPU family<br/>Spanish/English supported;<br/>package exists; release gate NO-GO"]:::implemented
...
observed for this exact hash, and clean-host behavior remains missing
...
this artifact remains an unsigned local validation candidate. Milestone 7
consumes the completed bounded EPUB 2/NCX evidence and records NO-GO for Piper
portfolio readiness, optional Chatterbox readiness, and signed public
publication. These status changes do not alter component topology or runtime
data flow; they constrain the claims made about the existing package edges.
...
8. **Milestone 11 — Blocked at release gates:** Milestone 1 freezes the exact minimum
   ...
   acquisition. Milestone 5 implements the exact unsigned local Windows
   ...
   completed Milestone 3.1 result and records NO-GO for Piper portfolio
   readiness, optional Chatterbox readiness, and signed public publication.
   Normal-user clean-host, compatible clean-GPU-host, and trusted-signing
   evidence remains unavailable, so M011 stays active and is not a completed
   distributable MVP.
