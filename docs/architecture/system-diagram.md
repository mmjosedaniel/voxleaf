...
| Reader settings and playback controls              | **In progress; speed path blocked**                             | M010.2 Milestones 1-2 freeze the reader-first shell, Settings, English migration, preferences, and playback comparison, then select no backend: WSOLA exceeds the CPU gate and packaged WebView2 rejects the media path under unchanged CSP. Current runtime remains the completed M010.1 interface, Spanish fallback, and `1.0x`; reader/Settings work can proceed independently.                                                               |
...
but a rate edge into playback requires a new explicit decision.
...
| Reader settings and playback controls                   | [M010.2 active ExecPlan](../plans/active/M010-002-reader-settings-and-playback-controls.md), approved [product requirements](../product/reader-settings-and-playback-controls.md), frozen [authority v1](reader-settings-playback-authority-v1.md), [ADR-0033](decisions/ADR-0033-freeze-reader-settings-and-pitch-preserving-playback-authority.md), and no-selection [ADR-0034](decisions/ADR-0034-retain-fixed-speed-after-playback-backend-evaluation.md). Milestones 1-2 are complete; current app-shell, default-language, preference, effective-lead, and playback remain unchanged, with `1.00x` retained. |
...
5. **Milestone 10.1 — Complete:** Milestones 1/1A-6 freeze and execute the
   ...
   merged the closeout.
6. **Milestone 10.2 — In progress; Milestone 1 complete:** result-blind
   authority now freezes the reader-first Settings shell, English fallback
   migration, bounded preferences, pitch-preserving playback rates,
   source/effective-duration arithmetic, candidates, and gates. Milestone 2 is
   next; no runtime behavior is implemented yet.
