...
Status applies to the text inside each node. The file-open boundary now proves local selection, bounded byte transfer, package opening, and safe metadata presentation, but it does not prove publication rendering or reading.
...
flowchart LR
  ...
  subgraph device["User device — local-first boundary"]
    ...
    subgraph desktop["apps/desktop — desktop process"]
      ...
      reader["Semantic text DOM renderer<br/>implemented; coordination<br/>and restoration planned"]:::partial
...
flowchart LR
  ...
  visualReader["Semantic text rendering implemented;<br/>navigation and restoration planned"]:::partial
